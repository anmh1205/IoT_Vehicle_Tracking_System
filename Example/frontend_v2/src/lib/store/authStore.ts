...
export const useAuthStore = create<AuthStoreState>()(persist((set, get) => ({
    ...
    login: async (username, password) => {
        ...
        try {
            ...
            // Backend now returns plain LoginResponse: { user, session }
            const sessionToken = (json as any)?.session?.token;
            const user = (json as any)?.user ?? null;

            if (sessionToken && user) {
                set({ user, token: sessionToken, isAuthenticated: true, isLoading: false, hasHydrated: true })
                return true
            }
            ...
            set({ error: (e as Error)?.message || 'Lỗi mạng', isLoading: false, hasHydrated: true })
            return false
        }
    },
    logout: () => {
        set({ user: null, token: null, isAuthenticated: false, error: null, isLoading: false, hasHydrated: true })
    },
    hasRole: (roles: Array<User['role']>): boolean => {
        const u = get().user
