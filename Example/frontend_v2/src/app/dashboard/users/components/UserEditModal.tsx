...
export function UserEditModal({ open, onClose, user, onSubmit, submitting, allDevices }: UserEditModalProps) {
  const { data: editUserAccess } = useUserDeviceAccess(user?.id ?? null);
  ...
  useEffect(() => {
    if (user && editUserAccess) {
      setForm((prev) => ({
        ...prev,
        deviceAccessMode: editUserAccess.mode || 'all',
        deviceIds: editUserAccess.deviceIds ?? []
      }));
    }
  }, [user, editUserAccess]);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Cập nhật thông tin và quyền hạn.</DialogDescription>
        </DialogHeader>
