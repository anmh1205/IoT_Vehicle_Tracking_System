...
export const getTripColumns = (actions: {
  ...
  return [
    ...
    { accessorKey: 'driverName', header: 'Tài xế' },
    {
      accessorKey: 'status',
      header: 'Trạng thái',
      cell: ({ row }) => TRIP_STATUS_LABELS[row.original.status] ?? row.original.status,
    },
    {
      id: 'actions',
      cell: ({ row }) => (
        <div className="flex gap-1">
          ...
          <Button size="sm" variant="outline" onClick={() => actions.onStart(row.original.id)}>
            Bắt đầu
          </Button>
          <Button size="sm" variant="outline" onClick={() => actions.onEnd(row.original.id)}>
            Kết thúc
          </Button>
