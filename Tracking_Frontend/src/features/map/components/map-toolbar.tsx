...
export function MapToolbar() {
  ...
  return (
    <div className="absolute right-3 top-3 z-[1000] flex gap-2">
      <Button size="sm" variant={followMode ? 'default' : 'outline'} onClick={toggleFollowMode}>Theo dõi</Button>
      <Button size="sm" variant={showGeofences ? 'default' : 'outline'} onClick={toggleGeofences}>Vùng giám sát</Button>
    </div>
  );
}
