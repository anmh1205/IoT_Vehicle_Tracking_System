...
const ViolationsPage = () => {
  const alerts = useQuery({
    queryKey: ['violations'],
    queryFn: () => alertServices.getList({ limit: 200, alertType: VIOLATION_TYPES.join(',') }),
  });
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: 'Loại vi phạm',
      cell: ({ row }) => VIOLATION_TYPE_LABELS[row.original.alertType] ?? row.original.alertType,
    },
    {
      accessorKey: 'severity',
      header: 'Mức độ',
      cell: ({ row }) => SEVERITY_LABELS[row.original.severity] ?? row.original.severity,
    },
