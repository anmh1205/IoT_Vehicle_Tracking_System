...
export default function MaintenancePage() {
  ...
  const maint = useQuery({ queryKey: ['maintenance'], queryFn: () => maintenanceServices.getList({ limit: 300 }) });
  const updateMutation = useMutation({ mutationFn: ({ id, ...payload }: any) => maintenanceServices.update(id, payload), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['maintenance'] }) });

  const columns: ColumnDef<any>[] = [
    { accessorKey: 'vehicleId', header: ({ column }) => <DataTableColumnHeader column={column} title="Phương tiện" /> },
    { accessorKey: 'maintenanceType', header: 'Loại', cell: ({ row }) => MAINTENANCE_TYPE_LABELS[row.original.maintenanceType] ?? row.original.maintenanceType },
    ...
    { id: 'actions', cell: ({ row }) => <Button size="sm" variant="outline" onClick={() => updateMutation.mutate({ id: row.original.id, status: 'completed', completedDate: new Date().toISOString() })}>Đánh dấu xong</Button> },
  ];
    ...
    <PageContainer pageTitle="Bảo trì" pageDescription="Quản lý lịch bảo trì phương tiện">
      <Tabs defaultValue="list" className="space-y-4">
        ...
        <TabsContent value="list">
          <DataTable columns={columns} data={rows} searchKey="vehicleId" searchPlaceholder="Tìm phương tiện..." isLoading={maint.isLoading} />
        </TabsContent>
