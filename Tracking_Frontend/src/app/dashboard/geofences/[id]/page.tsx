...
export default function GeofenceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const query = useQuery({ queryKey: ['geofence-detail', id], queryFn: () => geofenceServices.getById(Number(id)) });
