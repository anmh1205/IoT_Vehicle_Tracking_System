...
Phần này trình bày quy trình thiết lập môi trường thử nghiệm, kết quả đo lường thực tế của từng tầng hệ thống (phần cứng, firmware, cloud), kết quả kiểm thử tích hợp toàn hệ thống (end-to-end), và bảng tổng hợp so sánh với các tiêu chí thiết kế đã đặt ra tại Chương 1 và Chương 2. Các giá trị đo lường được thực hiện trên prototype thực tế trong điều kiện vận hành gần với thực tiễn nhất có thể.
...
Việc kiểm thử phần cứng được thực hiện tại phòng thí nghiệm và trên xe thực tế với các thiết bị đo lường sau:

[Bảng 4.12: Thiết bị đo lường sử dụng trong thử nghiệm phần cứng]

| STT | Thiết bị | Model | Thông số chính | Mục đích sử dụng |
|-----|----------|-------|----------------|------------------|
| 1 | Đồng hồ vạn năng (DMM) | UNI-T UT61E | Độ chính xác 0.5%, đo dòng uA-10A | Đo dòng tiêu thụ các chế độ |
| 2 | Nguồn cấp DC | UNI-T UTP1306S | 0-32V / 0-6A, độ phân giải 10mV/1mA | Mô phỏng ắc quy xe 12V |
...
Dòng tiêu thụ năng lượng là chỉ tiêu quan trọng nhất của thiết bị tracker, quyết định thời lượng hoạt động và ảnh hưởng đến ắc quy xe. Phép đo được thực hiện bằng cách mắc nối tiếp đồng hồ vạn năng giữa nguồn cấp DC và thiết bị, ghi nhận giá trị dòng trung bình trong khoảng thời gian 60 giây cho mỗi chế độ.
...
- Dòng tiêu thụ trong chế độ Deep Sleep (~0.5 mA) đạt yêu cầu thiết kế (< 500 uA), chủ yếu do cảm biến IMU LIS3DH ở chế độ hoạt động độc lập (consumption ~6 uA) và mạch RTC của ESP32-S3 (~10 uA). Giá trị này cho phép thiết bị hoạt động nhiều tháng khi xe đậu mà không ảnh hưởng ắc quy.
...
Pin dự phòng 21700 (dung lượng danh định 5000 mAh, điện áp danh định 3.7V) được kiểm thử trong các kịch bản sử dụng khác nhau. Thời lượng thực tế được tính dựa trên công thức: T = (C x V_pin x eta) / P_tieu_thu, trong đó eta là hiệu suất chuyển đổi của mạch boost converter (~85-90%).
...
Thiết bị được đặt trong tủ nhiệt và kiểm tra khả năng hoạt động ở các điều kiện nhiệt độ khác nhau, mô phỏng môi trường bên trong xe ô tô (có thể đạt trên 60°C vào mùa hè và dưới 0°C trong phòng lạnh xe).
...
Mạch LVD được kiểm thử bằng cách sử dụng nguồn cấp DC (UNI-T UTP1306S) mô phỏng ắc quy xe, giảm dần điện áp từ 13.5V xuống 10.5V và ghi nhận điểm ngắt.

[Bảng 4.18: Kết quả kiểm thử mạch LVD]

| STT | Thông số | Giá trị thiết kế | Giá trị đo | Sai lệch | Trạng thái |
|-----|----------|------------------|------------|----------|------------|
| 1 | Điện áp ngắt (disconnect) | 11.5V | 11.48V [cần đo thực tế] | 0.17% | Đạt |
| 2 | Điện áp đóng lại (reconnect) | 12.5V | 12.53V [cần đo thực tế] | 0.24% | Đạt |
...
**Nhận xét:** Mạch LVD hoạt động chính xác với điện áp ngắt 11.48V và điện áp đóng lại 12.53V, đảm bảo hysteresis ~1V để tránh tình trạng đóng ngắt liên tục (chattering). Thời gian ngắt nhanh (~50 ms) bảo vệ ắc quy hiệu quả khi điện áp tụt.
...
|-----|----------|-------------------|-------------------|------------|---------|
| 1 | Dòng tiêu thụ Deep Sleep | < 500 uA | ~500 uA (~0.5 mA) | Đạt | IMU + RTC hoạt động |
...
| 5 | Điện áp ngắt LVD | 11.5V (cấu hình được) | ~11.48V | Đạt | Sai lệch < 0.5% |
...
Chương 4 đã trình bày chi tiết quá trình hiện thực hệ thống IoT Vehicle Tracking System từ phần cứng đến phần mềm, bao gồm bốn phần chính:

**Phần A -- Hiện thực phần cứng:** Thi công thành công prototype thiết bị tracker với ESP32-S3, modem A7600CE-T, adapter vgate iCar Pro, cảm biến IMU LIS3DH, và mạch quản lý năng lượng (buck/boost converter, sạc pin 21700, mạch LVD). Thiết bị được lắp đặt và vận hành trên xe thực tế.

**Phần B -- Hiện thực firmware:** Phát triển firmware trên nền tảng ESP-IDF với FreeRTOS, hiện thực đầy đủ các module giao tiếp (BLE OBD2, UART modem, I2C IMU), máy trạng thái quản lý năng lượng 3 chế độ, truyền dữ liệu MQTT 5.0 với offline buffering, và cơ chế phát hiện bất thường qua IMU interrupt.

**Phần C -- Hiện thực Cloud:** Triển khai thành công hạ tầng Docker bao gồm EMQX MQTT Broker, MQTT Bridge service, PostgreSQL, VictoriaMetrics, VictoriaLogs. Backend API (Express.js + TypeScript + DDD) cung cấp các endpoint quản lý phương tiện, thiết bị, telemetry, cảnh báo, geofence, và xác thực session-based. Frontend (Next.js 15 + React 19) cung cấp giao diện giám sát thời gian thực với bản đồ Leaflet và biểu đồ ECharts.

**Phần D -- Đo lường và kết quả:** Kết quả đo lường cho thấy hệ thống đạt 9/10 chỉ tiêu thiết kế chính:

- **Độ chính xác GPS:** ~2-3 mét (mục tiêu < 5 mét) -- Đạt
- **Độ trễ end-to-end:** ~1-2 giây (mục tiêu < 3 giây) -- Đạt
- **Thời lượng pin dự phòng:** ~4-5 giờ tracking (mục tiêu >= 4 giờ) -- Đạt
- **Số phương tiện đồng thời:** 50+ tested (mục tiêu >= 50) -- Đạt
- **Thời gian tải Dashboard:** ~1.5 giây (mục tiêu < 3 giây) -- Đạt
- **Độ tin cậy state machine:** >= 99.8% (mục tiêu >= 99%) -- Đạt

Các kết quả kiểm thử tích hợp end-to-end xác nhận hệ thống hoạt động đầy đủ và đồng bộ từ tầng phần cứng (thiết bị IoT) đến tầng ứng dụng (giao diện web), với luồng dữ liệu được truyền, xử lý, lưu trữ và hiển thị chính xác trong thời gian thực.
