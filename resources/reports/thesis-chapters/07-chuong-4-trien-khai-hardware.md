...
Chương này trình bày chi tiết quá trình thiết kế, chế tạo và lắp ráp phần cứng của hệ thống theo dõi phương tiện IoT. Nội dung bao gồm sơ đồ nguyên lý mạch điện, bảng phân công chân GPIO, thiết kế mạch quản lý nguồn, thiết kế vỏ hộp bảo vệ, cũng như quy trình lắp ráp và lắp đặt thiết bị trong xe.
...
Hệ thống tracker được thiết kế xoay quanh vi điều khiển ESP32-S3 làm đơn vị xử lý trung tâm. ESP32-S3 giao tiếp với các ngoại vi thông qua nhiều giao thức khác nhau: UART để điều khiển modem 4G/GNSS, BLE để đọc dữ liệu OBD2 từ xe, I2C để giao tiếp với cảm biến gia tốc, và ADC để giám sát điện áp ắc quy. Toàn bộ hệ thống được cấp nguồn từ mạch quản lý nguồn thông minh với khả năng tự động chuyển đổi giữa ắc quy xe và pin dự phòng.
...
|     |       |         |          |          |              |
|  +-----+ +-----+ +----------+ +--------+ +--------+      |
|  |LIS3DH| |OBD2 | |A7600CE-T | |Power   | |U_batt  |     |
...
|  +---------------------------------------------+          |
|          |                        |                        |
|     Ắc quy xe 12V          Pin 21700 3.7V 5Ah             |
+-----------------------------------------------------------+
```
...
**Giao tiếp UART với modem A7600CE-T:** ESP32-S3 sử dụng một cổng UART (GPIO16 TX, GPIO17 RX) để giao tiếp với modem SIMCom A7600CE-T thông qua tập lệnh AT. Tốc độ baud mặc định là 115200 bps, dữ liệu 8-bit, 1 stop bit, không parity. Modem A7600CE-T là module LTE Cat-4 tích hợp GNSS (GPS, GLONASS, BeiDou, Galileo), hỗ trợ kết nối mạng cellular và định vị vệ tinh mà không cần module GPS riêng, giúp đơn giản hóa thiết kế và giảm chi phí.
...
```text
Sơ đồ kết nối UART giữa ESP32-S3 và A7600CE-T:
...
![Hình 4.2 - Sơ đồ kết nối giữa ESP32-S3 và modem A7600CE-T qua UART](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-2.png)

*Hình 4.2: Sơ đồ kết nối giữa ESP32-S3 và modem A7600CE-T qua UART*

> Nguồn: Hình vẽ của tác giả

#### c) Khối đo điện áp ắc quy

Để giám sát điện áp ắc quy xe (12V danh định), hệ thống sử dụng mạch chia áp (voltage divider) kết nối với kênh ADC của ESP32-S3. Bộ chia áp gồm hai điện trở R1 = 10 kOhm và R2 = 2.2 kOhm, tạo tỷ lệ chia áp 0.18 để đưa điện áp về dải đo của ADC (0-3.3V).

Công thức tính toán:

```
V_adc = U_batt x R2 / (R1 + R2)
      = U_batt x 2.2k / (10k + 2.2k)
      = U_batt x 0.18

Khi U_batt = 12V:
V_adc = 12V x 0.18 = 2.16V

Giá trị ADC 12-bit tương ứng:
ADC_value = (2.16V / 3.3V) x 4095 = 2680
```

Giá trị ADC đọc được sẽ được chuyển đổi ngược lại thành điện áp ắc quy trong firmware, cho phép hệ thống quyết định chuyển đổi nguồn khi điện áp giảm dưới ngưỡng 12.0V (với hysteresis 12.0V OFF, 12.2V ON để tránh dao động).
...
- **UART** (2 dây tín hiệu TX/RX): Kết nối với modem A7600CE-T để truyền nhận dữ liệu 4G và GNSS
...
| GPIO 4    | U_BATT_ADC    | Input (ADC)       | Đọc điện áp ắc quy xe qua voltage divider (R1=10k, R2=2.2k)                  |
| GPIO 5    | CHARGER_EN    | Output            | Điều khiển bật/tắt IC sạc IP2312 (HIGH = sạc, LOW = không sạc)               |
| GPIO 16   | MODEM_UART_TX | Output            | Truyền dữ liệu UART đến modem A7600CE-T                                      |
| GPIO 17   | MODEM_UART_RX | Input             | Nhận dữ liệu UART từ modem A7600CE-T                                         |
...
| GPIO 25   | MODEM_PWRKEY  | Output            | Điều khiển bật/tắt nguồn modem A7600CE-T                                     |
...
Mạch quản lý nguồn là thành phần thiết yếu của hệ thống tracker, đảm bảo thiết bị hoạt động liên tục ngay cả khi ắc quy xe yếu hoặc mất điện. Kiến trúc nguồn bao gồm năm khối chức năng chính:

1. **Mạch giảm áp Buck (LM2596):** Chuyển đổi 12V từ ắc quy xe xuống 5V
...
#### b) Mạch giảm áp Buck Converter (12V -> 5V)

Mạch buck converter sử dụng IC LM2596-5.0 (phiên bản cố định 5V) để chuyển đổi điện áp từ ắc quy xe 12V xuống 5V cấp cho toàn bộ hệ thống. Các thông số thiết kế:
...
| Điện áp vào     | 7-40 V DC                      | Từ ắc quy xe 12V         |
| Điện áp ra      | 5V                             | Cấp cho 5V Rail          |
| Dòng tối đa     | 3A                             | Đủ cho tracker + sạc pin |
| Hiệu suất       | ~85%                           | Tiêu biểu tại 12V -> 5V  |
...
```text
12V Input --+-- C1 (100uF/50V) --+-- LM2596 --+-- L1 (100uH) --+-- 5V Output
            |                     |             |                 |
            +-- GND               +-- GND       +-- D1 (1N5822)--+
                                                     |
                                                     +-- C2 (220uF/16V) -- GND
...
| Tụ ra C2          | 220 uF, 16V                 | Electrolytic                |
| Điện trở hồi tiếp | R1 = R2 = 10 kOhm           | Vout = 0.6V x (1 + R1/R2)   |
...
Kết quả cho thấy pin 21700 cần cung cấp dòng tối đa 3.18A khi đầu ra tải 2A. Với pin 21700 có dung lượng 5000 mAh và khả năng xả dòng cao, dòng này nằm trong phạm vi hoạt động an toàn.
...
| Ắc quy yếu          | OFF | < 12.0V | Pin 21700     | Không   | Có       |
| Ắc quy phục hồi     | OFF | > 12.2V | Ắc quy        | Không   | Có       |

Cơ chế hysteresis (12.0V OFF, 12.2V ON) được áp dụng để tránh hiện tượng dao động liên tục khi điện áp ắc quy nằm gần ngưỡng chuyển đổi.
...
- IGN ON và U_batt > 12V: Bật sạc (GPIO5 = HIGH)
- IGN OFF: Tắt sạc (GPIO5 = LOW) để bảo vệ ắc quy
- U_batt < 12V: Tắt sạc (GPIO5 = LOW) để bảo vệ ắc quy
...
Hệ thống sử dụng phương pháp giám sát điện áp bằng phần mềm (Software-based) thông qua ADC của ESP32-S3, kết hợp với mạch chia áp (voltage divider) R1 = 10 kOhm, R2 = 2.2 kOhm.
...
**Tản nhiệt:** Các module công suất (LM2596, MT3608) được đặt ở vị trí có lưu lượng khí tốt nhất, cách xa cảm biến nhiệt độ. Khe thông gió được bố trí ở mặt bên và mặt dưới vỏ hộp. Heatsink được gắn cho IC LM2596 khi hoạt động ở công suất cao.

**Anten:** Anten 4G/LTE và anten GNSS được đặt ở mặt trên vỏ hộp, hướng lên phía trên để đảm bảo thu sóng tốt nhất. Anten BLE (tích hợp trong ESP32-S3) hướng về phía cổng OBD2 để đảm bảo khoảng cách kết nối ngắn với adapter vgate iCar Pro. Các anten được đặt cách nhau tối thiểu 30 mm để giảm nhiễu tương hỗ.

**Chống nhiễu điện từ (EMI):** Mạch công suất (Buck, Boost) được đặt cách xa mạch tín hiệu (UART, I2C, ADC) để giảm nhiễu điện từ. Dây nguồn và dây tín hiệu được đi riêng, không chạy song song.
...
| 4   | SIMCom A7600CE-T Module        | Cái    | 1   | 300,000-500,000    | Kèm anten LTE + anten GNSS + khe SIM       |
...
| 8   | Module Buck LM2596 (12V->5V)   | Cái    | 1   | 15,000-25,000      | Dòng ra 3A, có chốn điều chỉnh             |
...
Quy trình lắp ráp mạch điện tử được thực hiện theo các bước sau:

**Bước 1 - Kiểm tra linh kiện:** Kiểm tra tất cả các module và linh kiện trước khi lắp ráp. Test riêng từng module (ESP32-S3, LM2596, MT3608, IP2312, A7600CE-T) để đảm bảo hoạt động đúng.

**Bước 2 - Lắp ráp mạch nguồn:** Kết nối module Buck LM2596 với nguồn 12V, điều chỉnh điện áp ra 5V. Kết nối module Boost MT3608 với pin 21700, điều chỉnh điện áp ra 5V. Lắp relay module làm Power MUX. Kết nối module sạc IP2312 với pin và BMS.
...
**Bước 4 - Kết nối ngoại vi:** Kết nối modem A7600CE-T qua UART (GPIO16, GPIO17). Kết nối cảm biến LIS3DH qua I2C (GPIO22, GPIO23). Kết nối mạch đo điện áp ắc quy (voltage divider) vào GPIO4 (ADC).
...
- **Kiểm tra chuyển nguồn:** Mô phỏng tình huống ắc quy yếu bằng cách giảm điện áp đầu vào từ 12V xuống dưới 12V, xác nhận hệ thống tự động chuyển sang pin dự phòng.
...
Chân 16: Nguồn +12V (Battery Power)     ---> Đầu vào mạch Buck LM2596
...
**Lưu ý:** Adapter vgate iCar Pro được cắm trực tiếp vào cổng OBD2 của xe. ESP32-S3 giao tiếp với adapter này qua BLE, không cần kết nối dây vật lý cho phần dữ liệu OBD2. Chỉ có nguồn điện 12V và GND được lấy từ cổng OBD2 thông qua dây nối riêng.
...
**Dây nguồn:** Dây nguồn 12V từ cổng OBD2 đến thiết bị tracker sử dụng dây điện tiết diện tối thiểu 0.5 mm2 (AWG 22), có bọc cách điện tốt. Cầu chì (fuse) 5A được đặt tại đầu dây nguồn gần cổng OBD2 để bảo vệ mạch.
...
1. **Kiểm tra nguồn điện:** Đo điện áp tại đầu vào thiết bị, xác nhận 12V từ ắc quy xe được cấp đầy đủ
2. **Kiểm tra kết nối BLE:** Xác nhận ESP32-S3 kết nối được với adapter vgate iCar Pro và đọc dữ liệu OBD2 (IGN, RPM, tốc độ)
3. **Kiểm tra GNSS:** Xác nhận modem A7600CE-T bắt được vệ tinh và trả về tọa độ GPS chính xác (sai số < 5 mét)
