...
Chương này trình bày toàn bộ quá trình phân tích, đề xuất và lựa chọn các giải pháp thiết kế cho hệ thống IoT giám sát phương tiện. Nội dung được tổ chức theo bốn tầng chính của hệ thống: (1) phần cứng thiết bị tracker, (2) firmware nhúng trên vi điều khiển, (3) hệ thống backend và cloud, và (4) giao diện người dùng frontend. Mỗi tầng được phân tích các phương án thay thế, đánh giá ưu nhược điểm bằng ma trận trọng số, từ đó lựa chọn phương án tối ưu về hiệu năng, chi phí và khả năng mở rộng.

## 3.1. Phân tích tổng hợp – General analysis

### 3.1.1. Phân tích và lựa chọn phần cứng

Chương này trình bày quá trình phân tích, lựa chọn và thiết kế các thành phần phần cứng cho thiết bị theo dõi xe IoT. Mục tiêu của hệ thống phần cứng là xây dựng một thiết bị tracker nhỏ gọn, tiêu thụ năng lượng thấp, có khả năng thu thập dữ liệu từ xe thông qua giao thức OBD2, xác định vị trí bằng GNSS, và truyền dữ liệu về máy chủ thông qua mạng di động 4G/LTE. Các tiêu chí thiết kế được đặt ra bao gồm: độ tin cậy cao, khả năng hoạt động liên tục với nguồn dự phòng, dễ tích hợp và phù hợp với ngân sách đồ án.

#### 3.1.1.1. Sơ đồ khối hệ thống

Hệ thống tracker được thiết kế theo kiến trúc module, bao gồm năm khối chức năng chính được kết nối với nhau thông qua vi điều khiển trung tâm ESP32-S3. Sơ đồ khối tổng thể được trình bày như sau:
...
Hệ thống hoạt động theo ba chế độ chính: (1) chế độ lái xe -- khi động cơ bật (IGN ON), hệ thống hoạt động đầy đủ với tất cả các module được kích hoạt; (2) chế độ đỗ xe -- khi động cơ tắt (IGN OFF), ESP32 vào trạng thái deep sleep, chỉ IMU LIS3DH duy trì hoạt động để phát hiện chuyển động; và (3) chế độ cảnh báo -- khi IMU phát hiện chuyển động bất thường, hệ thống tự động thức dậy và gửi cảnh báo qua mạng 4G.

Luồng dữ liệu trong hệ thống được tổ chức như sau: dữ liệu OBD2 (RPM, tốc độ, nhiên liệu) được đọc qua BLE từ adapter vgate iCar Pro, dữ liệu vị trí được lấy từ module GNSS tích hợp trong modem A7600CE-T, dữ liệu chuyển động được thu thập từ cảm biến IMU LIS3DH qua giao tiếp I2C. Tất cả dữ liệu này được ESP32-S3 xử lý, đóng gói và truyền lên máy chủ thông qua giao thức MQTT qua kết nối 4G/LTE.
...
**ESP32-S3** là vi điều khiển dual-core Xtensa LX7 hoạt động ở tần số 240 MHz, tích hợp sẵn WiFi 802.11 b/g/n và Bluetooth Low Energy (BLE) 5.0. Module này có 512 KB SRAM, 4-16 MB Flash, 3 cổng UART, 2 cổng I2C, ADC 12-bit với 20 kênh, và hỗ trợ external wakeup từ deep sleep với dòng tiêu thụ 10-15 uA [1].

**STM32L4** là dòng vi điều khiển siêu tiết kiệm năng lượng của STMicroelectronics, sử dụng nhân ARM Cortex-M4 hoạt động ở 80 MHz. Dòng này nổi tiếng với mức tiêu thụ cực thấp ở chế độ STOP2 (1-3 uA), tuy nhiên không tích hợp sẵn module truyền thông không dây [2].

**nRF52840** là vi điều khiển của Nordic Semiconductor với nhân ARM Cortex-M4 ở 64 MHz, tích hợp BLE 5.0 và 802.15.4. Module này có dòng deep sleep 1.5 uA nhưng bị hạn chế về số lượng UART (chỉ 2 cổng) và tài nguyên tính toán [3].

##### b) Bảng đánh giá đa tiêu chí

Để lựa chọn MCU phù hợp nhất, nhóm sử dụng phương pháp đánh giá đa tiêu chí với trọng số (weighted scoring matrix). Các tiêu chí và trọng số được xác định dựa trên yêu cầu cụ thể của hệ thống tracker.
...
**Tiêu thụ deep sleep (trọng số 0.15):** STM32L4 dẫn đầu với 1-3 uA ở chế độ STOP2, thấp hơn ESP32-S3 (10-15 uA) khoảng 3-5 lần. Tuy nhiên, sự chênh lệch này không ảnh hưởng đáng kể đến thời gian hoạt động tổng thể khi sử dụng pin 5000 mAh. Với dòng trung bình 7 mA ở chế độ heartbeat, sự khác biệt về dòng deep sleep chỉ tạo ra chênh lệch khoảng 2-3 ngày trong tổng thời gian hoạt động 20-35 ngày.
...
Dựa trên kết quả đánh giá đa tiêu chí, **ESP32-S3** được lựa chọn làm vi điều khiển chính cho hệ thống tracker với tổng điểm 9.20/10. Các lý do chính bao gồm:
...
- Tiêu thụ deep sleep 10-15 uA là chấp nhận được cho ứng dụng tracker với pin backup 5000 mAh.
...
| ADC              | 12-bit, 20 kênh                            |
| GPIO             | 45 chân                                    |
| Deep sleep       | 10-15 uA (với external wakeup)             |
| Active (BLE)     | 20-40 mA                                   |
...
Việc thu thập dữ liệu từ ECU xe thông qua cổng OBD2 (On-Board Diagnostics II) là một yêu cầu thiết kế cốt lõi của hệ thống tracker. Có hai phương pháp tiếp cận chính: sử dụng adapter OBD2 có dây (wired ELM327 qua UART) hoặc adapter OBD2 không dây (BLE ELM327). Nhóm lựa chọn phương pháp kết nối không dây qua BLE với các lý do sau:
...
Một điểm thiết kế quan trọng là ESP32-S3 không thể duy trì kết nối BLE khi ở trạng thái deep sleep. Do đó, chiến lược kết nối OBD2 được thiết kế phù hợp với từng chế độ hoạt động:
...
| Độ phân giải         | 16-bit                                          |
| Tiêu thụ (Active)    | 10-50 uA                                        |
| Tiêu thụ (Low-power) | 2-5 uA                                          |
...
Cấu hình I2C sử dụng địa chỉ 0x18 (khi SDO = LOW), tốc độ 400 kHz (Fast Mode), với điện trở kéo lên (pull-up) 4.7 kOhm đã có sẵn trên breakout board.

##### d) Ưu điểm của giải pháp

- **Tiết kiệm năng lượng tốt nhất:** Dòng tiêu thụ chỉ 2-5 uA ở chế độ low-power, trong khi vẫn duy trì khả năng phát hiện chuyển động. ESP32-S3 không cần chạy liên tục để kiểm tra cảm biến.
...
Hệ thống quản lý nguồn là thành phần phức tạp nhất trong thiết kế phần cứng, đảm nhận việc chuyển đổi điện áp, quản lý đường nguồn (power path), sạc pin dự phòng, và bảo vệ ắc quy xe khỏi tình trạng rút cạn quá mức. Hệ thống bao gồm năm khối chức năng chính.
...
##### a) Mạch buck converter (12V sang 5V)

Mạch buck converter có nhiệm vụ giảm điện áp từ ắc quy xe (12V) xuống 5V để cấp nguồn cho toàn hệ thống. IC LM2596-5.0 của Texas Instruments được lựa chọn vì các lý do sau:
...
Điện áp ra 5V được chọn làm bus nguồn chung vì phù hợp với đầu vào của LDO 3.3V (cấp nguồn cho ESP32), điện áp hoạt động của modem A7600CE-T, và điện áp đầu vào của mạch sạc IP2312. Việc sử dụng một mức điện áp chung 5V đơn giản hóa thiết kế và giảm số lượng converter cần thiết.

Dòng ra 3A của LM2596 đủ cho toàn hệ thống bao gồm: ESP32-S3 (80-120 mA), modem A7600CE-T (50-150 mA), mạch phụ trợ (10-20 mA), và dòng sạc pin 3A qua IP2312 (tuy nhiên dòng sạc chỉ hoạt động khi xe chạy, không đồng thời với toàn bộ tải).
...
**Giải pháp được chọn: Relay Module 5V.** Trong phạm vi đồ án, ưu tiên sự đơn giản và dễ triển khai. Relay module 1 kênh 5V (giá 5.000 - 10.000 VND) được điều khiển trực tiếp từ GPIO của ESP32-S3: GPIO HIGH bật relay (dùng ắc quy), GPIO LOW tắt relay (dùng pin backup). Relay có tiếp kết nối NO (Normally Open) cho ngõ ra buck và NC (Normally Closed) cho ngõ ra boost, đảm bảo khi mất điều khiển, hệ thống tự động chuyển sang pin backup [12].
...
**Phương pháp được chọn: Software-based (ADC ESP32-S3).** Điện áp ắc quy 12V được đo gián tiếp qua mạch chia áp (voltage divider) với R1 = 10 kOhm và R2 = 2.2 kOhm, đưa điện áp xuống mức an toàn cho ADC 12-bit của ESP32 (0-3.3V):

```
V_adc = U_batt x R2 / (R1 + R2) = U_batt x 2.2k / 12.2k = U_batt x 0.18

Khi U_batt = 12V:  V_adc = 12 x 0.18 = 2.16V
Giá trị ADC = (2.16 / 3.3) x 4095 = 2680
```

Logic chuyển nguồn sử dụng cơ chế hysteresis để tránh dao động khi điện áp gần ngưỡng:

- **Ngưỡng OFF (12.0V):** Chuyển sang pin backup khi U_batt < 12.0V
- **Ngưỡng ON (12.2V):** Chuyển lại ắc quy khi U_batt > 12.2V

[Bảng 3.11: Bảng trạng thái chuyển nguồn và cảnh báo]

| Trạng thái          | IGN | U_batt  | Nguồn tracker | Sạc pin | Cảnh báo              |
| ------------------- | --- | ------- | ------------- | ------- | --------------------- |
| Xe chạy bình thường | ON  | > 12V   | Ắc quy        | Có      | Không                 |
| Xe đỗ bình thường   | OFF | > 12V   | Ắc quy        | Không   | Không                 |
| Ắc quy yếu          | OFF | < 12V   | Pin 21700     | Không   | Cảnh báo chuyển nguồn |
| Ắc quy phục hồi     | OFF | > 12.2V | Ắc quy        | Không   | Cảnh báo phục hồi     |

Phương pháp software-based được chọn thay vì hardware-based (LM393 comparator) vì: không cần linh kiện ngoài (chi phí = 0), linh hoạt điều chỉnh ngưỡng trong firmware, và độ chính xác của ADC 12-bit (độ phân giải ~3 mV) là đủ cho ứng dụng này.
...
Mạch sạc chỉ được kích hoạt khi đồng thời thỏa mãn hai điều kiện: IGN ON và U_batt > 12V. ESP32-S3 điều khiển chân EN của IP2312 qua GPIO với điện trở hạn dòng 10 kOhm. Khi xe đỗ (IGN OFF), mạch sạc bị vô hiệu hóa để bảo vệ ắc quy khỏi bị rút năng lượng không cần thiết [13].
...
| 8   | Module Buck LM2596 (12V->5V)   | Cái    | 1   | 15.000 - 25.000    | Dòng ra 3A                           |
...
Tổng chi phí ước tính cho toàn bộ phần cứng hệ thống tracker nằm trong khoảng **872.000 - 1.635.000 VND**, phù hợp với ngân sách của một đồ án tốt nghiệp. Nhóm khuyến nghị sử dụng các module sẵn có (thay vì IC rời) trong giai đoạn prototype để rút ngắn thời gian phát triển và dễ dàng kiểm thử từng thành phần độc lập trước khi tích hợp.
