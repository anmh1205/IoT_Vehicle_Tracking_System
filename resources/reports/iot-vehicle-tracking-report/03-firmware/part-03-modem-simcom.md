...
SIM7600CE-T là lựa chọn runtime duy nhất cho tracker: **LTE Cat-4** + **GNSS tích hợp** (GPS/GLONASS/BeiDou) trong một module SIMCom duy nhất. Firmware chỉ dùng UART1 (GPIO17/18) để trao đổi AT command và đọc NMEA stream qua `AT+CGNSTST`. Hệ thống không còn UART GNSS phụ; GNSS được kích hoạt bằng lệnh AT trong module.
...
6. Nếu không phản hồi → reset qua PWRKEY pulse (GPIO4) và chờ ~10–30 giây để boot lại.
...
4. **Kích hoạt GNSS tích hợp, dùng `AT+CGNSPWR` / `AT+CGNSINF` và `AT+CGNSTST`:**
   ...
   - `AT+CGNSTST=1` stream NMEA (GGA/RMC) cho firmware GNSS parser qua cùng UART AT của SIM7600CE-T.
...
- `AT+CSCLK=1` để bật UART power save, RI nhanh chóng đánh thức modem khi cần.
- `AT+CPSMS=1,"","","01100001","00000001"` – PSM mặc định để giảm dòng standby.
- Khi deep sleep kéo dài: `AT+CGNSPWR=0` + `AT+CGNSTST=0` để tắt GNSS hoàn toàn.
- Wake up: `AT+CGNSPWR=1` → đợi `AT+CGNSINF` có fix (timeout ~3s hot start) → `AT+CGNSTST=1` để streaming.
...
- `mqtt_task` đọc dữ liệu GNSS qua `AT+CGNSTST=1` / parse GGA+RMC.
- Có thể giảm tần suất GNSS bằng cách đọc `CGNSINF` mẫu (5–30s) nếu cần tiết kiệm bộ nhớ.

**Chế độ 2 – Đỗ xe (IGN OFF, heartbeat):**

- Tắt GNSS (`CGNSPWR=0`) khi không cần fix, chỉ bật lại khi chuẩn bị gửi heartbeat.
- Modem ở trạng thái sleep (`CSCLK=1`) + PSM (`CPSMS=1,...`).
- Wake up mỗi 10–30 phút: `CGNSPWR=1`, `CGNSTST=1` (đợi ~1s hot start), đọc fix, gửi heartbeat, rồi `CGNSPWR=0` và `CSCLK=1` trở lại.

**Chế độ 3 – Cảnh báo (motion):**

- Wake ngay, bật GNSS + LTE, giữ `CGNSPWR=1`, `CGNSTST=1`, `CFUN=1` → track liên tục.
- Publish alert → sau đó bật lại `CSCLK`/`CPSMS` nếu cần.
...
- GNSS mất fix: `AT+CGNSINF` không trả về tọa độ → retry `CGNSPWR=1`, `CGNSTST=1`, dùng last known coordinate.
- Quá nhiệt: `AT+CFUN=4` → `AT+CFUN=1` sau khi nguội.

#### V.6 Tóm tắt driver

- UART1 (ESP32 GPIO17/18) tương thích trực tiếp với SIM7600CE-T.
- Modem xử lý cả giao tiếp mạng và GNSS, firmware chỉ cần một `modem_task` và các parser NMEA giống như trước.
