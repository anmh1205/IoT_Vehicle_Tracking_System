...
Phần này giữ nguyên trọng tâm của bản gốc nhưng lược bớt các sơ đồ phụ, bảng tra chi tiết và mô tả lặp. Mục tiêu là làm rõ hệ thống được hiện thực như thế nào ở ba lớp chính: phần cứng thiết bị, firmware nhúng và hạ tầng cloud/dashboard.

\subsubsection{4.1.1. Thiết kế chi tiết phần
cứng}\label{411-thiux1ebft-kux1ebf-chi-tiux1ebft-phux1ea7n-cux1ee9ng}

Thiết bị tracker được xây dựng quanh ESP32-S3, modem SIM7600CE-T, adapter BLE OBD2 và cảm biến gia tốc LIS3DSH. Cách tổ chức này giúp một bo mạch duy nhất vừa đọc được dữ liệu vận hành từ xe, vừa lấy được vị trí GNSS, vừa gửi được telemetry qua mạng 4G mà không cần thêm bộ xử lý phụ. Về bản chất, thiết kế phần cứng ưu tiên ba yêu cầu: tích hợp gọn, dễ lắp trong xe và đủ ổn định để hỗ trợ nhiều chế độ nguồn khác nhau.

Ở mức liên kết chức năng, ESP32-S3 dùng UART để điều khiển modem, BLE để giao tiếp với adapter OBD2, I2C để làm việc với IMU và ADC để giám sát điện áp ắc quy. Kiến trúc nguồn gồm buck cho bus chính, LDO cho 3.3V, nhánh dự phòng dùng pin 18650 và mạch power path để chuyển đổi nguồn an toàn khi xe mất điện hoặc điện áp xuống thấp. Nhờ đó, thiết bị không chỉ hoạt động khi xe đang chạy mà còn có thể duy trì cảnh báo ngắn hạn ở trạng thái đỗ.

\begin{center}
\begin{tabular}{|p{0.22\linewidth}|p{0.28\linewidth}|p{0.40\linewidth}|}
\hline
\textbf{Khối} & \textbf{Phần tử chính} & \textbf{Vai trò trong hệ thống} \\
\hline
Xử lý trung tâm & ESP32-S3 & Điều phối trạng thái, đọc cảm biến, đóng gói dữ liệu và điều khiển toàn bộ thiết bị \\
\hline
Truyền thông và định vị & SIM7600CE-T & Kết nối 4G/MQTT, lấy vị trí GNSS, nhận lệnh điều khiển từ xa \\
\hline
Dữ liệu xe & vgate iCar Pro BLE OBD2 & Đọc trạng thái khóa điện, tốc độ, RPM, nhiên liệu và các PID chẩn đoán cần thiết \\
\hline
Nguồn và dự phòng & Buck/LDO/Boost + pin 18650 & Cấp nguồn ổn định, hỗ trợ chuyển nguồn và duy trì vận hành ngắn hạn khi mất nguồn xe \\
\hline
\end{tabular}
\end{center}

Bản gốc có trình bày đầy đủ sơ đồ nguyên lý, mapping GPIO, cấu trúc enclosure và checklist lắp đặt. Trong bản rút gọn này, các nội dung đó được gom thành nhận định kỹ thuật: thiết kế phần cứng đã đạt yêu cầu tích hợp trên xe thật, phù hợp với nguồn xe 12V/24V và tạo được nền tảng rõ ràng cho phần firmware quản lý năng lượng, kết nối và dữ liệu.
...
Firmware được phát triển trên ESP-IDF 5.4 với FreeRTOS và NimBLE. Thay vì tách quá nhiều task ứng dụng, hệ thống dùng mô hình điều phối trung tâm kết hợp máy trạng thái \emph{Driving}, \emph{Parking}, \emph{Alert}. Cách tổ chức này giúp kiểm soát tốt hơn luồng đọc dữ liệu, phát hiện chuyển động, chu kỳ gửi telemetry, ngủ sâu và đánh thức theo sự kiện. Những phần bất đồng bộ thật sự cần thiết, như BLE stack, đồng bộ AT command và phản hồi OBD2, mới được giao cho các cơ chế hàng đợi, semaphore và mutex.

Ở tầng cloud, dữ liệu đi theo chuỗi thiết bị \(\rightarrow\) EMQX \(\rightarrow\) MQTT Bridge \(\rightarrow\) PostgreSQL/VictoriaMetrics/VictoriaLogs \(\rightarrow\) Backend API/WebSocket \(\rightarrow\) Frontend dashboard. Kiến trúc này giữ được sự phân vai rõ ràng: broker chịu trách nhiệm vận chuyển bản tin, bridge chịu trách nhiệm kiểm tra và ánh xạ dữ liệu, backend cung cấp nghiệp vụ và kênh thời gian thực, còn frontend tập trung vào hiển thị và vận hành. Việc tách lớp như vậy làm cho quá trình kiểm thử và chẩn đoán lỗi thuận lợi hơn đáng kể.

\begin{center}
\begin{tabular}{|p{0.23\linewidth}|p{0.29\linewidth}|p{0.38\linewidth}|}
\hline
\textbf{Lớp} & \textbf{Thành phần chính} & \textbf{Kết quả triển khai} \\
\hline
Firmware & ESP-IDF, FreeRTOS, NimBLE, MQTT client & Điều phối trạng thái, đọc dữ liệu OBD2/GNSS, quản lý nguồn và gửi telemetry \\
\hline
Cloud/ingest & EMQX, MQTT Bridge, PostgreSQL, Victoria stack & Thu nhận, phân tuyến, lưu trữ và giám sát dữ liệu theo thời gian thực \\
\hline
Ứng dụng điều khiển & Backend API, WebSocket, Next.js dashboard & Hiển thị trạng thái xe, bản đồ, cảnh báo, truy vấn lịch sử và quản trị đội xe \\
\hline
\end{tabular}
\end{center}

Điểm cần giữ lại của mục này là: các phương án của Chương 3 không dừng ở mức thiết kế lý thuyết mà đã được hiện thực thành một pipeline kỹ thuật hoàn chỉnh, chạy xuyên suốt từ thiết bị gắn trên xe đến dashboard giám sát.
...
Phần này tóm lược quá trình chuyển thiết kế thành sản phẩm thử nghiệm thực tế. Thay vì trình bày chi tiết từng bước thao tác, bản rút gọn tập trung vào đầu ra kỹ thuật và các quyết định triển khai quan trọng.
...
Bo mạch được lắp theo hướng tích hợp tối đa các khối xử lý, nguồn và giao tiếp vào cùng một tracker. Trọng tâm của giai đoạn này là bảo đảm ba yêu cầu: đường nguồn ổn định, tín hiệu UART/I2C/BLE không bị xung đột chức năng và việc kiểm tra sau lắp ráp có thể thực hiện nhanh trên từng module. Nhóm linh kiện chính gồm MCU, modem, cảm biến, khối nguồn và các đầu nối phục vụ nạp/debug.

\subsubsection{4.2.2. Lắp đặt trong xe (Vehicle
Installation)}\label{422-lux1eafp-ux111ux1eb7t-trong-xe-vehicle-installation}

Thiết bị được lắp đặt theo nguyên tắc hạn chế rung, tránh nguồn nhiệt cao và giữ đường dây tín hiệu/nguồn đủ sạch để không ảnh hưởng đến đo đạc. Ở mức vận hành, vị trí lắp không chỉ nhằm “gắn được thiết bị” mà phải hỗ trợ thu GNSS, kết nối nguồn xe ổn định và cho phép bảo trì nhanh khi cần kiểm tra tại hiện trường. Sau lắp đặt, nhóm thực hiện kiểm tra lại nguồn, GNSS, BLE OBD2, MQTT và khả năng chuyển chế độ nguồn.

\subsubsection{4.2.3. Triển khai Firmware}\label{423-triux1ec3n-khai-firmware}

Firmware được hiện thực bám sát mã nguồn ESP-IDF đang chạy trên thiết bị. Ở mức triển khai, phần quan trọng cần giữ lại từ bản gốc là firmware không phải một khối mã đơn lẻ, mà là tập hợp các module vận hành rõ vai trò: quản lý modem/GNSS, giao tiếp BLE-OBD2, phát hiện chuyển động bằng IMU, điều phối trạng thái hệ thống, đóng gói telemetry, lưu cấu hình trong NVS và phục hồi kết nối khi gặp lỗi mạng hoặc lỗi ngoại vi.

Môi trường phát triển xoay quanh ESP-IDF 5.4, FreeRTOS, NimBLE, CMake/Ninja và kênh log UART để nạp, theo dõi và debug. Phần trình bày rất dài của bản full về toolchain, cây thư mục và snippet được rút lại thành ý chính: nhóm đã xây dựng được một firmware có quy trình build-nạp-debug rõ ràng, có cấu trúc đủ tốt để tiếp tục mở rộng thay vì chỉ phục vụ demo ngắn hạn.

\begin{center}
\begin{tabular}{|p{0.24\linewidth}|p{0.30\linewidth}|p{0.34\linewidth}|}
\hline
\textbf{Khối} & \textbf{Nội dung đã hiện thực} & \textbf{Ý nghĩa kỹ thuật} \\
\hline
Toolchain & ESP-IDF 5.4, FreeRTOS, NimBLE, CMake/Ninja, log UART & Bảo đảm quy trình build, flash, debug và kiểm thử lặp lại được \\
\hline
Runtime chính & Modem/GNSS, BLE-OBD2, IMU wake, telemetry, quản lý cấu hình & Tạo được pipeline dữ liệu và điều khiển xuyên suốt trên thiết bị thật \\
\hline
Độ tin cậy & Watchdog, retry, phục hồi kết nối, lưu cấu hình bền vững & Giảm lỗi treo, lỗi mất mạng và tăng khả năng vận hành liên tục \\
\hline
\end{tabular}
\end{center}

Bản gốc có nhiều cây thư mục, snippet và bảng PID; bản rút gọn chỉ giữ lại kết luận cốt lõi: firmware đã đạt mức một nền tảng vận hành thật, có tổ chức nội bộ hợp lý và đủ cơ sở cho kiểm thử hệ thống ở Chương 4.3.

\subsubsection{4.2.4. Triển khai hệ thống Cloud}\label{424-triux1ec3n-khai-hux1ec7-thux1ed1ng-cloud}

Hệ thống cloud được dựng theo mô hình per-service Docker Compose với mạng dùng chung \texttt{tracking-network}. Phần cần giữ từ bản full là logic triển khai theo từng lớp: mỗi dịch vụ có compose riêng để dễ khởi động, dừng và debug độc lập, nhưng vẫn nhìn thấy nhau qua tên dịch vụ trên cùng mạng chia sẻ. Cách tổ chức này phản ánh rõ tư duy vận hành hệ thống thật, thay vì gom tất cả vào một cấu hình duy nhất khó bảo trì.

Trong đường đi dữ liệu, EMQX đóng vai trò broker trung tâm; MQTT Bridge tiếp nhận và chuẩn hóa bản tin; backend quản lý nghiệp vụ, xác thực và realtime; các kho dữ liệu tách vai trò giữa dữ liệu quan hệ, chuỗi thời gian và log vận hành. Việc tách lớp như vậy đặc biệt quan trọng vì nó giảm độ dính giữa các thành phần, giúp kiểm thử theo khối và truy vết lỗi thuận lợi hơn khi hệ thống bắt đầu có nhiều nguồn dữ liệu.

\begin{center}
\begin{tabular}{|p{0.22\linewidth}|p{0.28\linewidth}|p{0.36\linewidth}|}
\hline
\textbf{Khối cloud} & \textbf{Thành phần chính} & \textbf{Vai trò triển khai} \\
\hline
Hạ tầng container & Docker Compose theo dịch vụ + mạng dùng chung & Cho phép triển khai độc lập nhưng vẫn giao tiếp ổn định giữa các service \\
\hline
Ingest & EMQX + MQTT Bridge & Nhận, kiểm tra, phân tuyến và chuẩn hóa telemetry từ thiết bị \\
\hline
Nghiệp vụ & Backend API + WebSocket & Xác thực, truy vấn, cảnh báo và phân phối dữ liệu thời gian thực \\
\hline
Lưu trữ và giám sát & PostgreSQL, VictoriaMetrics, VictoriaLogs, Grafana & Tách riêng dữ liệu nghiệp vụ, telemetry, log và trực quan hóa vận hành \\
\hline
\end{tabular}
\end{center}

Cách triển khai này tạo ra một môi trường gần với vận hành thật nhưng vẫn đủ linh hoạt cho kiểm thử đồ án, đặc biệt khi cần đánh giá startup order, health endpoint và hành vi khôi phục dịch vụ.
...
Dashboard được triển khai như lớp vận hành của toàn bộ hệ thống chứ không chỉ là màn hình trình diễn. Các chức năng chính gồm hiển thị bản đồ thời gian thực, tổng quan đội xe, cảnh báo, lịch sử và các màn hình quản trị. Phần cần giữ từ bản full là dashboard được thiết kế theo đúng luồng sử dụng thực tế: người vận hành phải quan sát được xe đang ở đâu, trạng thái hiện tại là gì, sự cố nào vừa phát sinh và có thể truy vấn lại lịch sử khi cần.

Ở mức công nghệ, frontend dùng Next.js/React/TypeScript và được tổ chức theo hướng tách theo feature để dễ mở rộng. Bản rút gọn không liệt kê lại toàn bộ stack, nhưng vẫn giữ ý học thuật quan trọng: kiến trúc giao diện đã được xây theo hướng hỗ trợ realtime, quản trị và bảo trì dài hạn, thay vì chỉ dựng các trang đơn lẻ cho mục đích minh họa.
...
VictoriaMetrics, VictoriaLogs và Grafana được dùng để quan sát hiệu năng và hành vi vận hành của hệ thống. So với bản full, phần này được rút gọn mạnh nhưng vẫn cần giữ ba ý: hệ thống có lớp metrics, có lớp log tập trung, và có dashboard trực quan hóa để truy vết sự cố theo hướng có bằng chứng chứ không suy đoán cảm tính.

\begin{center}
\begin{tabular}{|p{0.24\linewidth}|p{0.28\linewidth}|p{0.34\linewidth}|}
\hline
\textbf{Trụ cột} & \textbf{Công cụ} & \textbf{Vai trò} \\
\hline
Metrics & VictoriaMetrics & Lưu telemetry và các chỉ số hiệu năng hệ thống để đo tải, độ trễ, trạng thái dịch vụ \\
\hline
Logs & VictoriaLogs & Tập trung log từ broker, bridge, backend và frontend để tra cứu lỗi \\
\hline
Visualization & Grafana & Tổng hợp dashboard, biểu đồ và ngưỡng giám sát phục vụ vận hành \\
\hline
\end{tabular}
\end{center}

Ngoài mục tiêu học thuật, lớp giám sát này có vai trò thực tế: giúp nhận biết lỗi ở broker, bridge, backend hoặc frontend sớm hơn, thay vì chỉ suy đoán từ hiện tượng ở dashboard. Đây là điểm cho thấy hệ thống được xây dựng theo tư duy vận hành chứ không chỉ ghép đủ chức năng.
...
Checklist hardening của bản gốc gồm nhiều chi tiết triển khai. Trong bản rút gọn, có thể gom thành bốn nhóm việc chính: bảo mật kết nối MQTT/API, quản lý secrets và biến môi trường, chuẩn hóa logging/monitoring, và quy trình phát hành có kiểm tra lại sau triển khai.

\begin{center}
\begin{tabular}{|p{0.24\linewidth}|p{0.32\linewidth}|p{0.30\linewidth}|}
\hline
\textbf{Nhóm hardening} & \textbf{Hạng mục đại diện} & \textbf{Mục tiêu} \\
\hline
Truyền thông và xác thực & TLS, device credential riêng, kiểm tra đầu vào API & Giảm nguy cơ nghe lén, giả mạo thiết bị và request độc hại \\
\hline
Độ tin cậy dịch vụ & Health check, restart policy, watchdog, retry/backoff & Tăng khả năng tự phục hồi khi dịch vụ hoặc thiết bị gặp lỗi \\
\hline
Dữ liệu và lưu trữ & Backup, retention, replay hàng đợi, kiểm soát ghi cục bộ & Hạn chế mất dữ liệu khi mạng chập chờn hoặc khi tải tăng cao \\
\hline
Quan sát và phát hành & Logging bảo mật, alert theo ngưỡng, kiểm tra sau triển khai & Giúp vận hành có thể phát hiện và xử lý sự cố sớm \\
\hline
\end{tabular}
\end{center}

Đây là những hạng mục chưa phải toàn bộ đều hoàn tất ở mức production, nhưng đã được xác định rõ như phần việc bắt buộc của giai đoạn tiếp theo.

\subsubsection{Tóm
tắt}\label{tuxf3m-tux1eaft}

Tóm lại, giai đoạn chế tạo và lắp ráp đã chuyển thiết kế thành nguyên mẫu vận hành được trên xe thật và trên hạ tầng cloud thật. Bản rút gọn không đi lại từng bước kỹ thuật chi tiết, nhưng vẫn giữ được kết luận cốt lõi: thiết bị, firmware, backend, dashboard và hệ giám sát đã được tích hợp thành một hệ thống làm việc đồng bộ.
...
Chương này được giữ theo đúng khung học thuật của bản gốc nhưng rút lại ở mức phản ánh rõ giá trị học tập và kinh nghiệm kỹ thuật, tránh lặp ý giữa các mục.

\subsubsection{6.1.1. Vi xử lý và Vi điều
khiển}\label{611-vi-xux1eed-luxfd-vuxe0-vi-ux111iux1ec1u-khiux1ec3n}

Kiến thức về vi điều khiển được áp dụng trực tiếp khi lựa chọn ESP32-S3, tổ chức ngoại vi, quản lý GPIO, ADC, UART, I2C và tối ưu chế độ ngủ sâu. Điểm quan trọng không chỉ là “dùng được MCU”, mà là biến các ràng buộc phần cứng thành thiết kế hệ thống vận hành được.

\subsubsection{6.1.2. Mạng máy tính và
IoT}\label{612-mux1ea1ng-muxe1y-tuxednh-vuxe0-iot}

Kiến thức mạng được thể hiện qua lựa chọn MQTT, WebSocket, phân lớp cloud và cách tổ chức luồng dữ liệu từ thiết bị đến dashboard. Nhờ nền tảng đó, hệ thống tránh được việc ghép nối tùy tiện giữa các lớp và có đường đi dữ liệu nhất quán.

\subsubsection{6.1.3. Cơ sở dữ
liệu}\label{613-cux1a1-sux1edf-dux1eef-liux1ec7u}

Kiến thức cơ sở dữ liệu được dùng để tách dữ liệu quan hệ, chuỗi thời gian và log theo đúng tính chất vận hành. Quyết định này giúp hệ thống vừa phục vụ nghiệp vụ quản lý xe, vừa hỗ trợ telemetry và giám sát theo thời gian thực hiệu quả hơn.

\subsubsection{6.1.4. Lập trình hướng đối
tượng}\label{614-lux1eadp-truxecnh-hux1b0ux1edbng-ux111ux1ed1i-tux1b0ux1ee3ng}

Tư duy mô-đun và tổ chức mã theo trách nhiệm rõ ràng được áp dụng xuyên suốt ở backend/frontend, đồng thời ảnh hưởng cả cách chia module firmware. Đây là yếu tố quan trọng để hệ thống vẫn giữ được khả năng bảo trì khi mở rộng.

\subsubsection{6.1.5. Điện tử số và
Analog}\label{615-ux111iux1ec7n-tux1eed-sux1ed1-vuxe0-analog}

Phần điện tử số và analog hiện diện ở khối nguồn, chia áp đo ắc quy, tín hiệu điều khiển modem và xử lý các đường giao tiếp ngoại vi. Đây là lớp kiến thức giúp phần cứng không chỉ đúng về sơ đồ mà còn đúng về hành vi trong điều kiện xe thật.

\subsubsection{6.1.6. Kỹ thuật phần
mềm}\label{616-kux1ef9-thuux1eadt-phux1ea7n-mux1ec1m}

Kiến thức kỹ thuật phần mềm được dùng khi thiết kế kiến trúc, kiểm thử, quản lý cấu hình và điều phối phát triển nhiều lớp công nghệ. Điểm rút ra là với hệ thống IoT nhiều tầng, chất lượng phần mềm phải được hiểu ở cấp hệ thống chứ không chỉ ở từng file mã nguồn.
...
Phần trước cho thấy kiến thức nền đã được vận dụng như thế nào. Phần này đi thêm một bước: tập trung vào các bài toán khó thật sự đã phát sinh trong quá trình hiện thực, qua đó làm rõ cách nhóm chuyển từ tư duy làm từng mô-đun riêng lẻ sang tư duy xử lý hệ thống nhiều ràng buộc.
...
Thách thức nằm ở việc kết nối hai thế giới khác nhau: chuẩn chẩn đoán xe và giao tiếp BLE. Khó khăn thực tế không chỉ là gửi được lệnh, mà là hiểu đúng phản hồi từ adapter bên thứ ba vốn có tài liệu hạn chế và có thể trả về dữ liệu đa khung. Vì vậy, nhóm phải chuẩn hóa rõ chuỗi xử lý từ quét, kết nối, gửi lệnh, nhận phản hồi đến phân tích dữ liệu, đồng thời kiểm thử lặp với các PID phổ biến và các bản tin nhiều khung.

Bài học quan trọng của mục này là khi làm việc với thiết bị bên thứ ba, reverse engineering có kiểm soát và kiểm thử theo ca sử dụng thực là bắt buộc nếu muốn hệ thống đủ tin cậy.

\subsubsection{6.2.2. Vấn đề 2: Đường ống dữ liệu thời gian thực với đảm
bảo phân phối}\label{622-vux1ea5n-ux111ux1ec1-2-ux111ux1b0ux1eddng-ux1ed1ng-dux1eef-liux1ec7u-thux1eddi-gian-thux1ef1c-vux1edbi-ux111ux1ea3m-buxe1o-phuxe2n-phux1ed1i}

Bài toán không dừng ở việc “gửi được dữ liệu”, mà là giữ dữ liệu đi qua nhiều tầng mà vẫn đủ kịp thời, đủ tin cậy và còn khả năng quan sát khi có lỗi. Trong bản full, vấn đề này được phân tích qua bài toán ghi kép sang PostgreSQL và VictoriaMetrics, theo dõi QoS, và nhu cầu truy vết bản tin khi chuỗi xử lý bị chậm hoặc đứt.

Đây là lý do hệ thống phải tách broker, bridge, backend và lớp hiển thị thay vì dồn tất cả vào một dịch vụ. Cách chia lớp đó làm tăng công triển khai ban đầu nhưng giảm đáng kể chi phí debug và mở rộng về sau.
...
Đây là bài toán điển hình của thiết bị gắn trên xe: vừa không được làm hao ắc quy, vừa phải phản ứng kịp khi có chuyển động hoặc lệnh điều khiển. Thực chất đây là bài toán giao thoa giữa mạch nguồn, ngưỡng điện áp, cơ chế ngủ sâu, đánh thức bằng IMU và logic chuyển trạng thái của firmware.

Cách giải là phối hợp chặt giữa phần cứng nguồn và firmware trạng thái, thay vì coi hai phần này độc lập. Nhờ đó thiết bị giữ được cân bằng giữa tiết kiệm năng lượng và khả năng phản hồi trong bối cảnh nguồn xe có thể thay đổi liên tục.

\subsubsection{6.2.4. Vấn đề 4: Kiến trúc đám mây có khả năng mở rộng
cho IoT}\label{624-vux1ea5n-ux111ux1ec1-4-kiux1ebfn-truxfac-ux111uxe1m-muxe2y-cuxf3-khux1ea3-nux103ng-mux1edf-rux1ed9ng-cho-iot}

Độ phức tạp của bài toán cloud không nằm ở từng dịch vụ riêng lẻ mà ở giao diện giữa chúng. Khi số lớp tăng lên, những quyết định tưởng nhỏ như tách storage, quy ước topic MQTT, health endpoint hay mạng dùng chung trong Docker đều ảnh hưởng trực tiếp đến khả năng vận hành và scale-out.

Bài học quan trọng là phải chốt rõ vai trò của broker, bridge, backend, storage và dashboard ngay từ đầu, nếu không chi phí tích hợp về sau sẽ rất lớn.
...
Một hệ thống theo dõi phương tiện không chỉ là bài toán kỹ thuật. Khi hệ thống bắt đầu thu thập dữ liệu vị trí, hành trình và trạng thái sử dụng xe, nó đồng thời chạm tới các câu hỏi về quyền riêng tư, trách nhiệm sử dụng dữ liệu và ảnh hưởng xã hội; vì vậy phần này cần được giữ lại như một lớp đánh giá độc lập chứ không phải phần phụ.
...
Hệ thống theo dõi phương tiện luôn chạm tới dữ liệu nhạy cảm. Vì vậy, việc bảo vệ dữ liệu phải được xem như một phần của thiết kế kỹ thuật chứ không phải phần phụ sau cùng. Trong triển khai hiện tại, tư duy bảo vệ dữ liệu đã xuất hiện qua các quyết định như tách lớp xác thực, chuẩn bị cho TLS, phân quyền truy cập và định hướng retention dữ liệu.

Trong triển khai tương lai, mã hóa, phân quyền và chính sách lưu trữ vẫn là các yêu cầu bắt buộc nếu muốn chuyển giải pháp từ mức đồ án sang mức triển khai thực tế có trách nhiệm.
...
Quyết định chỉ đọc dữ liệu OBD2 và không điều khiển xe từ xa là một lựa chọn kỹ thuật đúng hướng trong bối cảnh đồ án. Nó giảm đáng kể rủi ro ảnh hưởng đến an toàn vận hành của phương tiện trong khi vẫn giữ được giá trị giám sát.

\subsubsection{6.3.3. Tác động xã hội tích
cực}\label{633-tuxe1c-ux111ux1ed9ng-xuxe3-hux1ed9i-tuxedch-cux1ef1c}

Nếu triển khai đúng cách, hệ thống có thể hỗ trợ doanh nghiệp nhỏ quản lý đội xe tốt hơn, giảm thất thoát, tăng khả năng phản ứng khi có sự cố và mở đường cho tối ưu hóa vận hành dựa trên dữ liệu. Tác động tích cực ở đây không chỉ nằm ở giám sát, mà còn ở khả năng chuyển các quyết định vận hành từ cảm tính sang dựa trên bằng chứng.

\subsubsection{6.3.4. Những lo ngại cần xem
xét}\label{634-nhux1eefng-lo-ngux1ea1i-cux1ea7n-xem-xuxe9t}

Song song với lợi ích, hệ thống cũng đặt ra lo ngại về giám sát quá mức, phụ thuộc công nghệ và trách nhiệm dữ liệu. Vì vậy, giá trị xã hội của giải pháp chỉ bền vững khi đi kèm quy tắc sử dụng minh bạch, giới hạn truy cập rõ ràng và cơ chế quản trị dữ liệu đủ chặt để tránh lạm dụng.
...
Nếu Chương 5 đánh giá hệ thống bằng số liệu và các mục trước của Chương 6 nhìn lại kiến thức, thách thức và tác động xã hội, thì mục này là phần khép lại hành trình thực hiện đồ án. Phần rút gọn vẫn giữ tinh thần của bản full: không kể lại toàn bộ quá trình, mà chọn ra các bài học có giá trị cho những vòng phát triển tiếp theo.
...
Bài học lớn nhất của đồ án là phải chốt kiến trúc và giao diện giữa các lớp trước khi tối ưu chi tiết. Điều này đặc biệt đúng với hệ thống nhiều tầng, nơi lỗi tích hợp thường đắt hơn nhiều so với lỗi cục bộ.
...
Đồ án cho thấy một hệ thống IoT không phải là phép cộng đơn giản giữa phần cứng, firmware và web. Độ khó thực sự nằm ở cách các lớp phụ thuộc lẫn nhau và cách lỗi xuất hiện ở giao điểm giữa chúng.
...
Hệ sinh thái mã nguồn mở cho phép xây dựng một hệ thống khá hoàn chỉnh với chi phí phần mềm thấp, nhưng lợi ích thật sự nằm ở khả năng quan sát, tùy biến và học hỏi từ chính kiến trúc mình đang dùng. Bài học đi kèm là mã nguồn mở chỉ phát huy giá trị khi biết chọn thành phần đủ trưởng thành và ghép chúng theo ranh giới trách nhiệm rõ ràng.
...
Kiểm thử cần được hiểu như chuỗi hoạt động xuyên suốt từ phần cứng, firmware, cloud đến giao diện. Với hệ thống nhiều lớp, nếu chỉ kiểm thử cục bộ từng phần thì các lỗi tích hợp quan trọng vẫn sẽ bị bỏ sót.

\subsubsection{6.4.5. Tổng kết cá
nhân}\label{645-tux1ed5ng-kux1ebft-cuxe1-nhuxe2n}

Giá trị lớn nhất của đồ án không chỉ là một nguyên mẫu hoạt động, mà là quá trình đi qua trọn vòng đời của một hệ thống IoT: xác định yêu cầu, chọn kiến trúc, triển khai, đo lường, nhận ra giới hạn và đề xuất hướng phát triển. Đây là bước chuyển rõ rệt từ tư duy làm từng module sang tư duy làm sản phẩm hệ thống.

Điều cần giữ lại từ bản full là chiều sâu phản tư: thành quả của đồ án không nằm riêng ở phần cứng hay phần mềm, mà ở khả năng nối các lớp đó thành một hệ thống có thể giải thích, đo kiểm và tiếp tục cải tiến.

\subsection{Kết luận chương
6}\label{kux1ebft-luux1eadn-chux1b0ux1a1ng-6}

Chương 6 khép lại đồ án bằng góc nhìn phản tư: hệ thống không chỉ có giá trị kỹ thuật mà còn tạo ra bài học thực tế về kiến trúc, năng lượng, dữ liệu thời gian thực, kiểm thử tích hợp và trách nhiệm dữ liệu.
...
Phụ lục này giữ lại phần thông tin tài chính thiết yếu thay cho bảng kê rất dài của bản gốc. Mức chi phí tập trung vào các nhóm cấu thành giá của một thiết bị tracker mẫu.

\begin{center}
\begin{tabular}{|p{0.40\linewidth}|p{0.23\linewidth}|p{0.23\linewidth}|}
\hline
\textbf{Nhóm linh kiện} & \textbf{Chi phí ước tính} & \textbf{Ghi chú} \\
\hline
Khối xử lý và truyền thông & 700.000--900.000 VND & ESP32-S3, modem LTE/GNSS, anten và mạch phụ trợ \\
\hline
Khối nguồn và dự phòng & 350.000--450.000 VND & Buck, LDO, sạc pin, pin dự phòng, linh kiện bảo vệ \\
\hline
Cảm biến và kết nối ngoài & 150.000--250.000 VND & IMU, đầu nối, phụ kiện lắp đặt và hàn lắp \\
\hline
PCB/chế tạo mẫu & 250.000--400.000 VND & Phụ thuộc số lượng mẫu và mức hoàn thiện \\
\hline
\end{tabular}
\end{center}
...
Ở giai đoạn đồ án, chi phí cloud phụ thuộc chủ yếu vào VPS, kết nối mạng và lưu trữ dữ liệu, trong khi chi phí phần mềm được giảm mạnh nhờ dùng công nghệ mã nguồn mở. Điều này làm cho phương án triển khai thử nghiệm tương đối khả thi với ngân sách nhỏ.

\begin{center}
\begin{tabular}{|p{0.18\linewidth}|p{0.24\linewidth}|p{0.22\linewidth}|}
\hline
\textbf{Hạng mục} & \textbf{Thông số rút gọn} & \textbf{Chi phí ước tính} \\
\hline
VPS server & 4 vCPU, 8GB RAM, 100GB SSD & 300.000--500.000 VND/tháng \\
\hline
SIM 4G dữ liệu & 1 gói data cho thiết bị thử nghiệm & 70.000 VND/tháng \\
\hline
Tên miền & 1 domain phục vụ truy cập hệ thống & 25.000 VND/tháng quy đổi \\
\hline
Chứng chỉ TLS & Let's Encrypt & 0 VND \\
\hline
\textbf{Tổng hàng tháng} & & \textbf{395.000--595.000 VND} \\
\hline
\end{tabular}
\end{center}
...
Tổng hợp lại, chi phí của đồ án tập trung vào phần cứng mẫu, thời gian chế tạo và hạ tầng thử nghiệm. Đây là cấu trúc chi phí phù hợp với một đề tài phát triển nguyên mẫu hệ thống hơn là một dự án thương mại hóa ngay từ đầu.
...
15031--5)}\label{21-tiuxeau-chuux1ea9n-obd2-sae-j1979-iso-15031--5}

Tiêu chuẩn OBD2 được dùng làm cơ sở để lựa chọn nhóm PID đọc từ xe, giới hạn phạm vi truy cập ở mức \emph{read-only} và bảo đảm rằng giải pháp không can thiệp vào điều khiển vận hành của phương tiện.

\subsection{2.2. Tiêu chuẩn MQTT 5.0
(OASIS)}\label{22-tiuxeau-chuux1ea9n-mqtt-50-oasis}

MQTT 5.0 là nền tảng cho thiết kế topic, QoS, trạng thái kết nối và mô hình truyền dữ liệu nhẹ giữa thiết bị và server. Việc bám theo chuẩn này giúp hệ thống dễ mở rộng và tương thích tốt hơn với hạ tầng broker hiện đại.

\subsection{2.3. Bảo mật thông tin (ISO 27001 - tham
khảo)}\label{23-bux1ea3o-mux1eadt-thuxf4ng-tin-iso-27001---tham-khux1ea3o}

ISO 27001 không được hiện thực đầy đủ ở mức chứng chỉ trong phạm vi đồ án, nhưng các nguyên tắc cơ bản như phân quyền, quản lý bí mật, logging và kiểm soát truy cập vẫn được tham chiếu khi thiết kế giải pháp cloud/backend.
...
Thiết kế REST API của hệ thống dựa trên hướng chuẩn hóa tài nguyên, mã trạng thái HTTP, phân quyền theo phiên và phản hồi thống nhất. Điều này giúp lớp frontend và các công cụ tích hợp làm việc dễ dàng hơn với backend.
...
Kế hoạch thực hiện được chia thành các giai đoạn: khảo sát yêu cầu, thiết kế phần cứng và firmware, xây dựng cloud/backend, xây dựng dashboard, tích hợp hệ thống và đo kiểm. Cách chia này phản ánh đúng đặc thù của một đồ án nhiều tầng công nghệ.
...
\hline
\textbf{Giai đoạn} & \textbf{Thời lượng} & \textbf{Kết quả chính} \\
\hline
Khảo sát và chốt kiến trúc & Tuần 1--4 & Yêu cầu kỹ thuật, sơ đồ hệ thống, hướng chọn công nghệ \\
\hline
Thiết kế thiết bị và firmware & Tuần 5--10 & Sơ đồ phần cứng, nguồn, giao tiếp OBD2/GNSS, máy trạng thái \\
\hline
Cloud, backend, frontend & Tuần 11--16 & MQTT, API, realtime, dashboard quản lý \\
\hline
Tích hợp và đo kiểm & Tuần 17--22 & Kiểm thử toàn tuyến, đánh giá và hoàn thiện báo cáo \\
\hline
\end{tabular}
\end{center}
...
Về bản chất, tiến độ của đồ án không đi theo kiểu tuần tự cứng mà có nhiều vòng lặp giữa thiết kế, hiện thực và đo kiểm. Bài học rút ra từ kế hoạch này là phải tích hợp sớm giữa các tầng, nếu không thời gian cuối kỳ sẽ bị dồn quá mạnh vào sửa lỗi tích hợp.
...
Mã nguồn được quản lý theo monorepo, tách rõ firmware, backend, frontend, MQTT bridge và các thành phần hạ tầng. Phần cần giữ từ bản full là cấu trúc kho mã nguồn đã đủ rõ để người đọc lần theo từng lớp của hệ thống: các dịch vụ runtime nằm trong \texttt{iot-vehicle-tracking-system-cloud/}, còn báo cáo, kế hoạch và tài liệu nằm trong \texttt{resources/}.

Cách tổ chức này hỗ trợ tốt cho việc phát triển đồng thời nhiều lớp công nghệ trong cùng một dự án và cũng là cơ sở để người khác tái dựng lại hệ thống từ tài liệu đi kèm.
...
Phần hỗ trợ cơ sở dữ liệu của đồ án gồm schema quan hệ cho nghiệp vụ quản lý xe/người dùng/cảnh báo và lớp telemetry chuỗi thời gian cho vị trí, tốc độ, RPM và các đại lượng vận hành khác. Trong bản full, các file SQL khởi tạo được đặt dưới \texttt{iot-vehicle-tracking-system-cloud/Tracking\_PostgreSQL/init/}; bản rút gọn chỉ giữ lại thông tin đủ để người đọc biết nơi tra cứu và phạm vi dữ liệu chính.

Tài liệu đầy đủ được giữ ở kho mã nguồn và tài liệu thiết kế hệ thống, nên phụ lục này đóng vai trò điểm dẫn hướng thay vì lặp lại toàn bộ schema.

\subsection{4.3. Tài liệu API (API
Documentation)}\label{43-tuxe0i-liux1ec7u-api-api-documentation}

API tài liệu hóa các nhóm chức năng chính: xác thực, quản lý phương tiện, dữ liệu telemetry, cảnh báo, geofence và lệnh điều khiển. Trong phạm vi bản rút gọn, mục này chỉ khẳng định rằng tài liệu API đã tồn tại như một phần của hạ tầng backend, chủ yếu được truy cập qua lớp backend và tài liệu OpenAPI tương ứng, chứ không liệt kê lại toàn bộ endpoint.
...
Cấu hình môi trường được tổ chức riêng cho firmware, backend, frontend, bridge và các dịch vụ hạ tầng, thường đi kèm các file \texttt{.env} hoặc \texttt{.env.example} theo từng dịch vụ. Việc tách cấu hình theo dịch vụ là yêu cầu cần thiết để kiểm soát secrets, môi trường thử nghiệm và môi trường triển khai thực tế.
...
Hướng dẫn cài đặt và chạy hệ thống được xây dựng theo trình tự: chuẩn bị môi trường, tạo mạng Docker dùng chung, khởi động hạ tầng, chạy backend/frontend/bridge và xác nhận các endpoint chính. Điều quan trọng là toàn bộ tài liệu hỗ trợ đã đủ để người khác có thể đọc, dựng và kiểm tra lại hệ thống mà không cần dựa hoàn toàn vào mô tả trong bản báo cáo chính.
