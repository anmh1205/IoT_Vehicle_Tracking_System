...
| **Tên đề tài:**           | Thiết kế hệ thống IoT cho ứng dụng quản lý phương tiện giao thông trong lĩnh vực cho thuê xe tự lái |
| **Sinh viên thực hiện:**  | Lê Trọng An                                                                                         |
| **Lớp:**                  | K15-KTCĐT2                                                                                          |
...
- Địa điểm: .............................
- Họ và tên sinh viên: Lê Trọng An &emsp; Mã số SV: 21010389
- Lớp: K15-KTCĐT2 &emsp; Ngành: Kỹ thuật Cơ điện tử &emsp; Khóa: K15
- Tên đề tài: Thiết kế hệ thống IoT cho ứng dụng quản lý phương tiện giao thông trong lĩnh vực cho thuê xe tự lái
- Giảng viên hướng dẫn: TS. Nguyễn Đức Nam
...
|                           |                                                                                                     |
| ------------------------- | --------------------------------------------------------------------------------------------------- |
| **Trường:**               | Trường Kỹ thuật - Đại học Phenikaa                                                                  |
| **Khoa:**                 | Cơ khí - Cơ Điện tử                                                                                 |
...
Mã sinh viên: 21010389 &emsp; Lớp: K15-KTCĐT2  
...
Sự phát triển nhanh của dịch vụ cho thuê xe tự lái tại Việt Nam khiến nhu cầu giám sát phương tiện không còn dừng ở việc biết xe đang ở đâu, mà còn ở việc hiểu xe đang vận hành ra sao và có dấu hiệu bất thường hay không. Từ nhu cầu đó, đồ án này trình bày quá trình thiết kế và triển khai một hệ thống IoT giám sát phương tiện theo hướng end-to-end, bao gồm thiết bị gắn trên xe, hạ tầng cloud và bảng điều khiển web, nhằm hỗ trợ doanh nghiệp theo dõi vị trí, nắm bắt trạng thái kỹ thuật và phản ứng sớm với các rủi ro vận hành.

Ở tầng thiết bị, hệ thống được xây dựng trên bo mạch PCB tự thiết kế. Trong đó, ESP32-S3 giữ vai trò bộ điều khiển trung tâm; SIM7600CE-T đảm nhiệm truyền dữ liệu 4G và định vị vệ tinh GNSS; LIS3DSH theo dõi rung động để nhận biết chuyển động khi xe đỗ; còn adapter vgate iCar Pro cho phép đọc dữ liệu OBD2 từ xe qua BLE. Kiến trúc nguồn được tổ chức theo nhiều nhánh để thiết bị vẫn duy trì hoạt động khi xe tắt máy, đồng thời hạn chế ảnh hưởng tới ắc quy chính.

Ở tầng phần mềm, dữ liệu từ thiết bị được gửi bằng MQTT 3.1.1 tới EMQX. MQTT đóng vai trò lớp nhắn tin nhẹ phù hợp với đường truyền 4G, còn EMQX là broker trung tâm tiếp nhận và phân phối bản tin. Từ đây, MQTT Bridge subscribe bốn nhóm topic `v1/{device_id}/{rawdata|status|events|firmware}`, chuẩn hóa payload rồi phân luồng dữ liệu tới PostgreSQL, VictoriaMetrics và VictoriaLogs. Song song với luồng ingest, backend còn lắng nghe nhánh `internal/events/#` để đẩy cập nhật thời gian thực ra giao diện qua Socket.IO.

Bảng điều khiển web được phát triển bằng Next.js 15 và React 19, hiển thị bản đồ, biểu đồ và trạng thái phương tiện gần như theo thời gian thực thông qua Socket.IO. Nhờ đó, người vận hành không chỉ quan sát vị trí xe mà còn có thể theo dõi lịch sử hành trình, cảnh báo và trạng thái thiết bị trên cùng một giao diện thống nhất.

Kết quả đạt được là một hệ thống IoT giám sát phương tiện hoàn chỉnh từ phần cứng đến phần mềm, có khả năng theo dõi vị trí thời gian thực, đọc dữ liệu chẩn đoán OBD2, thiết lập vùng cho phép, phát sinh cảnh báo tự động, cập nhật firmware từ xa và hỗ trợ quản lý đội xe. Quan trọng hơn, hệ thống cho thấy một hướng triển khai phù hợp với bối cảnh doanh nghiệp cho thuê xe tự lái trong nước: chi phí hợp lý, có thể tùy biến và đủ dư địa để mở rộng trong các giai đoạn sau.

**Từ khóa:** IoT, giám sát phương tiện, GPS, OBD2, MQTT, ESP32-S3, thời gian thực, vùng cho phép
...
On the hardware side, the tracker is implemented as a custom integrated PCB rather than an assembly of off-the-shelf development boards. The main board integrates ESP32-S3 (MCU), SIMCom SIM7600CE-T (LTE+GNSS), LIS3DSH (IMU), DS3231M (RTC), W25Q128 (SPI Flash), and power blocks based on MP2482, TPS54231, AP2112, SX1308, and TP5100. The modem is configured to Auto mode (`AT+CNMP=2`) so it can switch between LTE/UMTS/GSM automatically, uses the default APN `internet`, and delivers GNSS data via `AT+CGNSINF` (with optional NMEA streaming through `AT+CGNSTST`) on the same UART1 channel. The device reads engine diagnostic data via OBD2 through a vgate iCar Pro adapter over Bluetooth Low Energy (BLE), where this adapter is an external peripheral outside the main PCB. The power architecture uses a 5V main rail, an approximately 4V modem rail, a 3.3V logic rail, and a 18650 1S backup cell charged through the TP5100 in 1-cell mode to maintain operation when the vehicle engine is off.

On the software side, device data is transmitted through MQTT 3.1.1 via EMQX with per-device ACL authorization. The MQTT Bridge subscribes to the device topics `v1/{deviceId}/{rawdata|status|events|firmware}`, validates the payloads, and routes them to PostgreSQL, VictoriaMetrics, and VictoriaLogs. The backend is built with Express.js and TypeScript in a domain-oriented structure, exposes REST/OpenAPI endpoints, and listens to internal MQTT events so Socket.IO channels can push realtime updates and OTA status back to operators.
...
Trước tiên, tôi xin gửi lời cảm ơn chân thành và sâu sắc nhất đến TS. Nguyễn Đức Nam — người đã trực tiếp hướng dẫn, định hướng và tận tình hỗ trợ tôi trong suốt quá trình thực hiện đồ án tốt nghiệp này. Những góp ý chuyên môn và sự động viên của thầy là nguồn động lực quan trọng giúp tôi hoàn thành đề tài.

Tôi xin trân trọng cảm ơn quý thầy cô trong Khoa Cơ khí - Cơ Điện tử, Trường Kỹ thuật - Đại học Phenikaa đã truyền đạt những kiến thức nền tảng vững chắc trong suốt quá trình học tập, tạo điều kiện thuận lợi để tôi có thể áp dụng vào thực tiễn thông qua đồ án này.
...
<br/>
<p align="right"><b>Lê Trọng An</b></p>
...
| 3.2.1        | Phân tích và lựa chọn giải pháp Firmware                                               | ...   |
| 3.2.2        | Giải pháp Firmware                                                                     | ...   |
| 3.3          | Phân tích và đề xuất kiến trúc Backend/Cloud — Backend/cloud analysis and solution     | ...   |
| 3.3.1        | Phân tích và lựa chọn kiến trúc Cloud                                                  | ...   |
| 3.3.2        | Giải pháp Backend & Cloud                                                              | ...   |
| 3.4          | Phân tích và đề xuất công nghệ Frontend — Frontend analysis and solution               | ...   |
| 3.4.1        | Phân tích và lựa chọn công nghệ Frontend                                               | ...   |
| 3.4.2        | Giải pháp Frontend                                                                     | ...   |
...
Từ những nhu cầu thực tiễn trên, dự án "IoT Vehicle Tracking System" được triển khai nhằm xây dựng một giải pháp đồng bộ, bao gồm thiết bị phần cứng IoT, firmware nhúng, hệ thống cloud và giao diện web. Mục tiêu là tạo ra một hệ thống có chi phí hợp lý, dễ tùy biến và có thể mở rộng theo quy mô doanh nghiệp cho thuê xe tự lái tại Việt Nam.
...
3. **Cảnh báo thông minh**: Tự động phát hiện và gửi thông báo khi có các sự kiện bất thường như: xe vượt ra khỏi vùng cho phép, vượt tốc độ quy định, chuyển động bất thường khi xe đang đỗ, hoặc mất kết nối thiết bị.
...
- Truyền dữ liệu qua giao thức MQTT 3.1.1 qua EMQX, phân tầng QoS 0/1 theo loại bản tin
...
4. **Giai đoạn 4 — Phát triển backend API**: Xây dựng REST API theo kiến trúc domain-oriented, hiện thực các domain chính như auth, vehicle, device, trip, alert, geofence, maintenance, firmware, telemetry, statistics, notification và violation, đồng thời tích hợp WebSocket cho dữ liệu thời gian thực.
...
- **PostgreSQL**: Lưu trữ dữ liệu quan hệ có cấu trúc — thông tin người dùng, xe, khách hàng, hành trình, cảnh báo, vùng cho phép (bảng `geofences`), lệnh điều khiển.
...
- Xây dựng thành công hạ tầng cloud bao gồm EMQX MQTT Broker, dịch vụ MQTT Bridge, PostgreSQL, VictoriaMetrics và VictoriaLogs, toàn bộ được container hóa bằng Docker.
- Backend API (Express.js + TypeScript) với kiến trúc Domain-Driven Design, cung cấp các endpoint quản lý xe, thiết bị, telemetry, cảnh báo, vùng cho phép, và xác thực người dùng.
...
7. **Tối ưu PCB cho sản xuất**: Nâng thiết kế PCB hiện tại theo hướng DFM/EMI, giảm kích thước, tăng độ bền cơ khí và chuẩn bị cho sản xuất hàng loạt.
...
Từ bối cảnh thực tiễn đó, đề tài tập trung vào bài toán thiết kế và xây dựng hệ thống IoT giám sát phương tiện với các vấn đề kỹ thuật cốt lõi sau:
...
Hệ thống phải truyền liên tục dữ liệu GPS và OBD2 qua mạng 4G/LTE với độ trễ thấp, đồng thời duy trì cơ chế reconnect ổn định khi mất sóng. Bên cạnh đó, kiến trúc truyền thông cũng cần chừa dư địa để bổ sung lưu đệm cục bộ nếu triển khai ở quy mô lớn hơn. Việc lựa chọn giao thức phù hợp như MQTT, HTTP hay CoAP ảnh hưởng trực tiếp đến hiệu suất và độ tin cậy toàn hệ thống.
...
1. Thiết kế thiết bị tracker tích hợp GPS/GNSS và OBD2 dựa trên vi điều khiển ESP32-S3, tối ưu tiêu thụ năng lượng khi xe đậu (mục tiêu dòng deep sleep toàn thiết bị dưới 500 µA trong phiên bản phần cứng hiện tại và tiếp tục giảm ở các vòng tối ưu PCB tiếp theo).
...
Bảng so sánh cho thấy phương án đề xuất giữ được các năng lực quan trọng của hệ thống thương mại như độ chính xác vị trí, khả năng đọc OBD2 và phát hiện bất thường, nhưng chi phí đầu tư thấp hơn và mức độ tùy biến cao hơn. Với bài toán của doanh nghiệp cho thuê xe trong nước, đây là hướng lựa chọn phù hợp hơn về cả kỹ thuật lẫn chi phí vận hành.
...
- **Deep sleep 10–15 µA ở mức MCU**: Mặc dù cao hơn STM32L4 (1–2 µA), mức này vẫn là nền tảng phù hợp cho một thiết kế low-power. Trong hệ thống hoàn chỉnh, dòng ngủ sâu còn phụ thuộc mạch nguồn, cảm biến và linh kiện phụ trợ; do đó giá trị đo trên toàn thiết bị sẽ cao hơn thông số riêng của vi điều khiển.
...
- **Phân tách trách nhiệm rõ ràng**: PostgreSQL xử lý dữ liệu quan hệ (người dùng, phương tiện, cảnh báo, vùng cho phép), VictoriaMetrics xử lý dữ liệu chuỗi thời gian (tọa độ GPS, dữ liệu OBD2, telemetry cảm biến). Mỗi hệ thống được tối ưu cho loại dữ liệu của mình.
...
- **Tương thích Grafana**: Cả hai đều hỗ trợ Grafana native, thuận tiện cho việc xây dựng dashboard giám sát thống nhất mà không cần thêm công cụ trung gian.
...
- Ưu tiên các linh kiện có datasheet rõ ràng, chất lượng ổn định và nguồn cung trong nước dễ tiếp cận
...
Hệ thống IoT Vehicle Tracking System liên quan đến nhiều nhóm sử dụng với nhu cầu khác nhau. Vì vậy, việc tách riêng yêu cầu của từng bên là cần thiết để tránh thiết kế dàn trải và giữ trọng tâm vào các nhu cầu vận hành thực tế.
...
1. _Giám sát và an ninh_: Theo dõi vị trí phương tiện 24/7, phát hiện và cảnh báo ngay khi có bất thường (xe bị di chuyển trái phép, vượt ra khỏi vùng cho phép, tốc độ vượt ngưỡng). Hệ thống cần gửi thông báo qua nhiều kênh (web dashboard, Telegram bot, email).
...
Chương 2 đã làm rõ các bài toán kỹ thuật chính của hệ thống giám sát phương tiện, từ phần cứng, truyền thông và xử lý dữ liệu đến kiến trúc tổng thể. Trên cơ sở khảo sát và đối sánh các giải pháp hiện có, chương đã xác định hướng tiếp cận phù hợp, đồng thời chốt các yêu cầu kỹ thuật, ràng buộc thiết kế và ma trận truy xuất yêu cầu. Đây là nền tảng để chuyển sang bước lựa chọn giải pháp ở Chương 3.

# CHƯƠNG 3. CÁC GIẢI PHÁP THIẾT KẾ – DESIGN SOLUTIONS

Chương này trình bày quá trình phân tích và lựa chọn các giải pháp thiết kế cho hệ thống IoT giám sát phương tiện. Nội dung được tổ chức theo bốn tầng: phần cứng thiết bị theo dõi, firmware nhúng trên vi điều khiển, hệ thống backend và cloud, cùng giao diện người dùng. Ở mỗi tầng, các phương án được so sánh theo yêu cầu kỹ thuật, thông số chính và mức độ phù hợp với điều kiện triển khai thực tế trước khi chốt phương án sử dụng.
...
![Hình 3.1a - Phân rã chi tiết các khối chức năng của tracker](./assets/figures/03-chuong-3-giai-phap-phan-cung-hinh-3-1a.svg)
...
| Chi phí triển khai phần cứng tham chiếu | ~100.000–200.000 VND (mặt bằng BOM dự án)                                  | ~150.000–300.000 VND (mặt bằng BOM dự án)                                                          | Thường cao hơn ESP32-S3 và ít phổ biến hơn trong bối cảnh dự án                      |
...
- Đáp ứng đầy đủ kiến trúc hiện tại: 1 kết nối BLE cho OBD2, UART cho modem và một đường debug riêng, đồng thời vẫn còn dư địa UART cho mở rộng.
...
- Chi phí triển khai phần cứng với ESP32-S3 thấp hơn STM32L4 khoảng 30–50% theo mặt bằng linh kiện đang dùng trong đồ án.
- Dòng deep-sleep của ESP32-S3 cao hơn STM32L4 theo tài liệu hãng, nhưng vẫn nằm trong giới hạn chấp nhận được của bài toán pin backup khi xét toàn bộ duty-cycle của modem, GNSS và các chu kỳ wake-up [19], [53], [54].
...
| Flash              | Flash ngoài (biến thể WROOM thường 4–16 MB) [19], [20]            |
...
| Biến thể linh kiện | ESP32-S3-WROOM-1                                                  |
| Phương án tích hợp | Tích hợp trực tiếp vào bo mạch chính                              |
...
| Tác động tới BOM phần cứng thực tế          | Bộ modem + GNSS đang chốt 330.000–500.000 VND (BOM dự án)  | Chưa phải cấu hình BOM chính của repo                      | Giảm số module phần cứng rời, thuận lợi cho đi dây và lắp ráp |
...
#### 3.1.2.1. Thiết kế mô-đun thu thập dữ liệu OBD2
...
**Kết luận:** Phương pháp BLE được chọn vì phù hợp hơn với kiến trúc hiện tại của tracker: tận dụng BLE tích hợp của ESP32-S3, không chiếm thêm UART phần cứng, giảm ràng buộc đi dây, đồng thời vgate iCar Pro đã có công bố rõ về BLE 4.0, auto-sleep và danh sách giao thức OBD-II. Với các thông số hãng chưa công bố (như dòng active/standby), báo cáo chỉ ghi nhận là chưa có số liệu chính thức thay vì đưa ra giá trị ước lượng.
...
- **Tiêu thụ năng lượng:** Ở thời điểm rà soát nguồn, trang hãng được kiểm tra không công bố dòng active/standby chính thức cho vgate iCar Pro BLE [64]. Vì vậy báo cáo chỉ ghi nhận đây là thông số chưa được công bố đầy đủ, không đưa ra giá trị mA cố định khi chưa có datasheet chính thức đúng mẫu.
- **Phổ biến và giá hợp lý:** Có nguồn cung khá phổ biến trên thị trường trong nước, với giá khoảng 150.000–300.000 VND.
...
#### 3.1.2.2. Thiết kế mô-đun cảm biến chuyển động IMU
...
LIS3DSH được cấu hình ở chế độ low-power với tần suất lấy mẫu 1 Hz (ODR = 1 Hz) và chỉ bật chức năng motion detection. Khi phát hiện gia tốc vượt ngưỡng 0.2g trên bất kỳ trục nào (X, Y, Z), cảm biến phát tín hiệu interrupt qua chân INT1 để firmware kích hoạt nhánh canh rung. Với wiring hiện tại, INT1 đi vào GPIO41 nên nhánh này dùng GPIO wake trong light sleep; heartbeat định kỳ vẫn dùng timer wake ở deep sleep [9].
...
![Hình 3.3a - Sơ đồ chi tiết chân kết nối LIS3DSH với ESP32-S3](./assets/figures/03-chuong-3-giai-phap-phan-cung-hinh-3-3a.svg)

_Hình 3.3a: Sơ đồ chi tiết chân kết nối LIS3DSH với ESP32-S3_

> Nguồn: Hình vẽ UML kỹ thuật của tác giả

Cấu hình I2C sử dụng địa chỉ `0x18`, tốc độ `400 kHz`, với hai điện trở kéo lên `10 kΩ` trên các net `IMU-SCL` và `IMU-SDA`. Ở runtime hiện tại, bus I2C được ánh xạ về `GPIO1` (SCL) và `GPIO2` (SDA), chân `INT1` đi vào `GPIO41`, còn `INT2` đi vào `GPIO42` để dành cho các tình huống mở rộng sau này.
...
#### 3.1.2.3. Thiết kế hệ thống quản lý nguồn

Hệ thống quản lý nguồn là khối phức tạp nhất trong thiết kế phần cứng, vì nó phải đồng thời giải quyết ba bài toán vốn dễ xung đột với nhau: tạo đúng các mức điện áp cho từng tải, duy trì đường nguồn dự phòng khi cần và bảo vệ ắc quy xe khỏi bị rút cạn quá mức. Vì vậy, khối này được tách thành năm nhánh chức năng để người đọc dễ theo dõi vai trò của từng đường nguồn thay vì chỉ nhìn một sơ đồ điện phức tạp.
...
![Hình 3.4a - Kiến trúc nguồn và phân phối điện áp trong hệ thống](./assets/figures/03-chuong-3-giai-phap-phan-cung-hinh-3-4a.svg)

_Hình 3.4a: Kiến trúc nguồn và phân phối điện áp trong hệ thống_

> Nguồn: Hình vẽ UML kỹ thuật của tác giả

##### a) LDO 3.3V cho ESP32-S3 (AP2112-3.3)

Nhánh 3.3V dùng **AP2112-3.3** như một LDO để hạ từ **bus 5V chính** xuống 3.3V, cấp cho ESP32-S3 và các tải logic 3.3V. Cách tổ chức này phù hợp với kiến trúc nguồn thực tế của bo mạch: MP2482 đảm nhiệm bước hạ áp đầu tiên từ 12–24V xuống 5V, còn AP2112-3.3 đảm nhiệm bước ổn áp cuối cho miền logic.
...
##### f) Mạch sạc pin 1S (TP5100)

Khối sạc dùng **TP5100** ở cấu hình **1 cell**, nhận **5V từ nhánh MP2482** và sạc pin 18650 1S tới mức **4.2V**. So với phương án TP4056 trước đây, TP5100 phù hợp hơn với nhánh nguồn dự phòng vì đây là bộ sạc chuyển mạch, giảm tổn hao nhiệt khi nạp.

[Bảng 3.13: Thông số khối sạc pin]

| Khối    | IC     | Input        | Output | Tải          |
| ------- | ------ | ------------ | ------ | ------------ |
| Sạc pin | TP5100 | 5V từ MP2482 | 4.2V   | Pin 18650 1S |

Dòng sạc TP5100 được cấu hình bằng điện trở thiết lập dòng sạc và còn phụ thuộc điều kiện nhiệt của mạch, vì vậy không dùng một giá trị dòng cố định cho mọi điều kiện vận hành.

##### g) Pin dự phòng 18650 1S Li-ion

Pin 18650 1S Li-ion được chọn làm nguồn dự phòng với cấu hình 1 cell đơn giản, giữ nguyên kiến trúc 1S Li-ion + TP5100 + BMS + SX1308 của hệ thống.
...
#### 3.1.2.4. Bảng tổng hợp linh kiện (Bill of Materials)
...
#### 3.2.1.1. Đặt vấn đề cho giải pháp Firmware
...
#### 3.2.1.2. So sánh các phương án nền tảng Firmware
...
#### 3.2.1.3. Chọn giải pháp Firmware

Từ kết quả so sánh, đồ án chọn **PA-FW2: ESP-IDF + FreeRTOS** làm nền tảng firmware chính.

Lý do lựa chọn:

- **Khớp yêu cầu điều phối thời gian thực của hệ thống**: ESP-IDF + FreeRTOS cho phép tổ chức state machine trung tâm, các driver bất đồng bộ và primitive đồng bộ ở mức module, phù hợp với bài toán IoT chạy trên tài nguyên hạn chế.
...
##### b) Cơ chế điều phối với FreeRTOS

Firmware sử dụng FreeRTOS theo hướng tối giản: `app_main()` khởi tạo hệ thống, nạp cấu hình từ NVS, xác định trạng thái ban đầu theo nguyên nhân đánh thức rồi gọi `state_machine_run()` theo chu kỳ ngắn. Cách tổ chức này giúp luồng xử lý chính dễ đọc, dễ debug và bám sát đặc thù của một thiết bị nhúng chuyên dụng hơn là tách thành quá nhiều task nghiệp vụ.
...
Các primitive FreeRTOS vẫn xuất hiện ở các mô-đun cần đồng bộ truy cập tài nguyên dùng chung. Cụ thể, NimBLE host chạy trong một task nền riêng; `ble_mgr` dùng queue và mutex để nhận kết quả khám phá/kết nối; `ble_obd` dùng semaphore cho phản hồi từ adapter; còn `modem_at` dùng mutex để tuần tự hóa lệnh AT trên UART chung.
...
1. **Khởi tạo và nạp cấu hình**: `app_main()` khởi tạo NVS, nạp `config_t`, ghi nhận wakeup cause, label OTA partition và bộ đếm boot trong ngữ cảnh RTC.
2. **Bring-up subsystem**: `state_machine_init()` lần lượt khởi tạo ADC, IMU, power manager, modem LTE/GNSS, MQTT client, command handler, RTC DS3231M và hàng đợi cục bộ trên microSD.
3. **Xác định trạng thái đầu tiên**: Timer wake đi vào `HEARTBEAT`, ngắt IMU EXT0 đi vào `ALARM`, còn khởi động thường đi theo nhánh `INIT -> CHECK_IGN`.
4. **Điều phối vận hành**: State machine chạy qua các trạng thái `CHECK_IGN`, `DRIVING`, `PARKED`, `ALARM`, `HEARTBEAT`, đồng thời xử lý reconnect mạng, subscribe topic `commands`, OTA pending-confirm và replay dữ liệu từ `offline_queue`.
5. **Chuẩn bị ngủ sâu**: Khi đủ điều kiện, firmware snapshot ngữ cảnh RTC, tắt BLE/GNSS/LTE, chốt đường nguồn rồi chuyển sang `SLEEP`.
...
Kiến trúc module BLE OBD2 được tổ chức theo bốn tầng logic: lớp điều phối ứng dụng trong `state_machine.c`, lớp giao thức OBD tại `ble_obd.c`, lớp quản lý BLE tại `ble_mgr.c` và lớp BLE stack của NimBLE. Cách tách lớp này giúp firmware cô lập xử lý PID OBD2 khỏi phần GATT/BLE thuần túy, thuận lợi cho debug và mở rộng.
...
##### c) Điều khiển khối sạc (TP5100)

Khối sạc TP5100 được điều khiển qua chân GPIO5 (CHARGER_EN). Logic sạc được thiết kế để bảo vệ cả ắc quy xe lẫn pin dự phòng:
...
Ngoài ra, hệ thống có thêm kênh giám sát LVD độc lập từ khối LVD phần cứng (GPIO19). Đây là kênh dự phòng cho ADC, cho phép kiểm tra nhanh trạng thái nguồn với cơ chế trễ (hysteresis): profile 12V dùng Switch_OFF=12.0V, Switch_ON=12.2V; profile 24V dùng Switch_OFF=24.0V, Switch_ON=24.4V. Khối LVD xử lý hysteresis ở mức phần cứng, tránh hiện tượng dao động (oscillation) quanh ngưỡng.
...
Dữ liệu từ thiết bị được đóng gói theo định dạng JSON và truyền lên broker qua nhóm topic `v1/{device_id}/...`. Trong hệ thống hiện tại, thiết bị phát ba nhóm payload chính là `rawdata`, `status` và `events`; ngoài ra còn có luồng `firmware` để báo trạng thái OTA và luồng `commands` theo chiều ngược lại từ máy chủ.
...
**Thông điệp Firmware (trạng thái OTA)**

Luồng `firmware` được dùng để ghi nhận vòng đời cập nhật firmware từ lúc gán tác vụ đến khi xác nhận thành công sau reboot hoặc rollback:
...
| ---------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------- |
| `update_config`                    | Cập nhật cấu hình thiết bị               | `heartbeat_interval_s`, `tracking_interval_s`                             |
| `request_location`                 | Yêu cầu gửi vị trí ngay ở chu kỳ kế tiếp | Không                                                                     |
| `enable_tracking`                  | Bật hoặc tắt phát telemetry định kỳ      | `enabled` (boolean)                                                       |
| `reboot`                           | Khởi động lại thiết bị                   | Không                                                                     |
| `ota_update`                       | Kích hoạt cập nhật firmware OTA          | `jobId`, `version`, `url`, `size`, `sha256`, `force`, `confirmTimeoutSec` |
| `manual_rollback` / `ota_rollback` | Yêu cầu quay về firmware trước đó        | Không bắt buộc                                                            |
...
#### 3.3.1.1. Đặt vấn đề cho kiến trúc Cloud
...
- **Lưu trữ dữ liệu hỗn hợp**: vừa có dữ liệu chuỗi thời gian (GPS, nguồn, IMU; sẵn sàng mở rộng thêm OBD2), vừa có dữ liệu quan hệ nghiệp vụ (xe, người dùng, cảnh báo, vùng cho phép).
- **Chi phí và năng lực vận hành phù hợp đồ án**: ưu tiên self-hosted, dễ triển khai, dễ mở rộng theo từng dịch vụ.

#### 3.3.1.2. So sánh các phương án kiến trúc Cloud
...
#### 3.3.1.3. Chọn giải pháp kiến trúc Cloud
...
Phần này trình bày chi tiết giải pháp thiết kế và triển khai hệ thống Backend/Cloud theo phương án đã lựa chọn ở mục 3.3.1.

#### 3.3.2.1. Kiến trúc tổng quan hệ thống Cloud (Cloud Architecture Overview)
...
![Hình 3.12a - Luồng dữ liệu chi tiết từ thiết bị đến dashboard](./assets/figures/05-chuong-3-giai-phap-backend-hinh-3-12a.svg)
...
1. **Thiết bị IoT** gửi dữ liệu telemetry chính (GNSS, trạng thái nguồn, mức rung, ignition, uptime) lên EMQX Broker thông qua giao thức MQTT 3.1.1 theo topic có cấu trúc `v1/{device_id}/rawdata`.
...
3. **MQTT Bridge** (dịch vụ Node.js độc lập) subscribe các topic `v1/{device_id}/{rawdata|status|events|firmware}`, xác thực dữ liệu đầu vào bằng Zod schema, sau đó thực hiện ghi kép (dual write):
   ...
   - Publish các sự kiện nội bộ qua nhánh `internal/events/#` để backend realtime listener tiếp nhận.
4. **API Server** (Express.js) cung cấp REST API cho Frontend, truy vấn dữ liệu từ PostgreSQL và VictoriaMetrics, đồng thời gửi lệnh điều khiển hoặc OTA ngược về thiết bị qua topic `v1/{device_id}/commands`.
...
Hệ thống áp dụng mô hình triển khai per-service Docker Compose theo quy ước IVM26. Mỗi dịch vụ có một file `docker-compose.yml` riêng và cùng tham gia mạng Docker `tracking-network` (external network). Cách tổ chức này có các ưu điểm sau:
...
#### 3.3.2.2. MQTT Broker - EMQX (Message Broker Selection & Configuration)
...
- **Hiệu năng phù hợp**: Theo tài liệu công bố của EMQX, broker đáp ứng tốt tải message lớn với độ trễ thấp, đủ cho quy mô triển khai hiện tại và còn dư địa mở rộng.
- **Khả năng mở rộng**: Hỗ trợ clustering nhiều node, thuận lợi nếu hệ thống tăng số lượng thiết bị trong giai đoạn sau.
- **Rules Engine tích hợp**: Thuận tiện cho các tác vụ lọc hoặc chuyển đổi dữ liệu đơn giản ngay tại broker; tuy nhiên trong kiến trúc hiện tại, đây không phải lớp xử lý nghiệp vụ chính.
...
EMQX Rules Engine là lớp xử lý dữ liệu nhẹ ngay tại broker. Trong hệ thống hiện tại, phần lớn nghiệp vụ cảnh báo được xử lý ở MQTT Bridge; tuy vậy, Rules Engine vẫn phù hợp để lọc nhanh các điều kiện đơn giản hoặc đẩy dữ liệu sang pipeline phụ mà không làm tăng tải cho backend.
...
Ngoài ra, hệ thống có thể mở rộng thêm rule cho vùng cho phép cơ bản, thống kê traffic theo topic, hoặc phát hiện thiết bị im lặng bất thường để phục vụ vận hành.

#### 3.3.2.3. Thiết kế cơ sở dữ liệu (Database Architecture Design)
...
| Commands    |                |
| Vùng cho phép | Lưu trữ: 30d  | Lưu trữ: 7d
```
...
| **Vùng địa lý**            | geofences, vehicle_geofences             | Vùng địa lý và liên kết với xe          |
...
#### 3.3.2.4. API Server và kiến trúc phần mềm (API Server & Software Architecture)
...
MQTT Bridge đã được tách thành dịch vụ Node.js độc lập (`Tracking_MqttBridge/`) với `package.json`, `docker-compose.yml`, logger, connection pool và cấu hình riêng. Cách tách này giúp dịch vụ dễ mở rộng hơn, cô lập lỗi tốt hơn và giữ kiến trúc tổng thể gọn hơn.

#### 3.3.2.5. Thiết kế API Endpoints (REST API Design)
...
|                    | POST   | /api/v1/devices/:id/ota          | Gửi lệnh OTA đến thiết bị       | Có    |
|                    | POST   | /api/v1/devices/:id/ota/rollback | Yêu cầu rollback firmware       | Có    |
...
|                    | GET    | /api/v1/firmware/:id/download    | Tải file firmware để OTA        | Có    |

![Hình 3.14a - Luồng điều phối OTA giữa Backend, EMQX, thiết bị và MQTT Bridge](./assets/figures/05-chuong-3-giai-phap-backend-hinh-3-14a.svg)

_Hình 3.14a: Luồng điều phối OTA giữa Backend, EMQX, thiết bị và MQTT Bridge_

> Nguồn: Hình dựng từ kiến trúc triển khai thực tế của tác giả
...
#### 3.3.2.6. WebSocket và truyền dữ liệu thời gian thực (Real-time Communication)

Hệ thống sử dụng **Socket.IO 4.8** làm giải pháp truyền dữ liệu thời gian thực từ Backend đến Frontend. Ở phạm vi triển khai này, hệ thống định nghĩa năm namespace chính: `/dashboard`, `/devices`, `/notifications`, `/exports` và `/firmware`. Tất cả namespace đều đi qua middleware xác thực; riêng namespace `/devices` hỗ trợ join room theo từng thiết bị (`device:{id}`) để nhận sự kiện chi tiết.

Các sự kiện tiêu biểu gồm `device:status`, `device:position`, `command:ack`, `stats:update`, `alert:new`, `geofence:enter`, `geofence:exit`, `export:ready` và `firmware:assignment`. Trong triển khai hiện tại, namespace `/firmware` chủ yếu dùng để phát sự kiện gán hoặc cập nhật tác vụ OTA từ backend; còn trạng thái chi tiết của quá trình OTA được MQTT Bridge ghi vào `firmware_update_log` và VictoriaLogs để phục vụ truy vết, theo dõi và đồng bộ với giao diện quản trị.

#### 3.3.2.7. Bảo mật hệ thống (Security Design)
...
#### 3.4.1.1. Đặt vấn đề cho giải pháp Frontend
...
- **Khả năng mở rộng theo tính năng nghiệp vụ**: quản lý xe, chuyến đi, vùng cho phép, cảnh báo, báo cáo cần dễ mở rộng.
...
#### 3.4.1.2. So sánh các phương án công nghệ Frontend
...
#### 3.4.1.3. Chọn giải pháp Frontend
...
Phần này trình bày các giải pháp thiết kế, lựa chọn công nghệ và kiến trúc ứng dụng web theo phương án đã chọn ở mục 3.4.1.

#### 3.4.2.1. Lựa chọn công nghệ Frontend (Technology Selection)
...
- **Sẵn sàng cho triển khai:** Có sẵn các cơ chế tối ưu cho môi trường sản xuất như Image Optimization, Code Splitting và Standalone Output cho Docker.
...
| Vẽ vùng cho phép           | @geoman-io/leaflet-geoman-free | mapbox-gl-draw               | Drawing Manager        |

**Kết luận:** Leaflet.js được chọn vì hoàn toàn miễn phí (MIT license), hỗ trợ tự host tile server cho môi trường không có Internet công cộng, và có hệ sinh thái plugin phong phú (MarkerCluster, Geoman cho vẽ vùng cho phép). Điều này đặc biệt phù hợp với hệ thống IoT có thể triển khai trong môi trường private network.

#### 3.4.2.2. Kiến trúc ứng dụng web (Web Application Architecture)
...
![Hình 3.15a - Luồng dữ liệu một chiều giữa Next.js, Query, Zustand và Backend](./assets/figures/06-chuong-3-giai-phap-frontend-hinh-3-15a.svg)
...
#### 3.4.2.3. Thiết kế các trang chức năng chính (Main Feature Pages)
...
Trang quản lý chuyến đi cung cấp lịch sử và chi tiết từng chuyến, gồm tuyến đường trên bản đồ, biểu đồ tốc độ theo thời gian và timeline sự kiện. Trang cảnh báo tổng hợp cảnh báo theo badge mức độ nghiêm trọng và hỗ trợ xử lý hàng loạt. Trang vùng cho phép cho phép vẽ hình tròn, đa giác hoặc hình chữ nhật trực tiếp trên bản đồ bằng Leaflet Geoman.
...
| Vùng địa lý | `/dashboard/geofences`     | Tạo, sửa vùng cho phép trên bản đồ      | Không                    |
...
#### 3.4.2.4. Tích hợp bản đồ thời gian thực (Real-time Map Integration)
...
![Hình 3.19a - Phân rã lớp bản đồ React Leaflet và kênh Socket.IO](./assets/figures/06-chuong-3-giai-phap-frontend-hinh-3-19a.svg)
...
| GeofenceMap | `components/map/geofence-map.tsx` | Vẽ và chỉnh sửa vùng cho phép (polygon, circle, rectangle)                 |
...
#### 3.4.2.5. Thiết kế responsive và trải nghiệm người dùng (Responsive Design & UX)
...
- **Bản đồ (Maps):** Custom markers với màu theo trạng thái, popup thông tin, drawing tools cho vùng cho phép.
...
Dựa trên phân tích các giải pháp đề xuất cho từng tầng hệ thống ở các mục 3.1 đến 3.4, bảng tổng hợp dưới đây đánh giá và so sánh các phương án đã xem xét:
...
Từ kết quả phân tích và đánh giá ở mục 3.5, phương án thiết kế tối ưu cho hệ thống IoT giám sát phương tiện được tổng hợp như sau:
...
- **Tầng truyền thông (Communication Layer):** MQTT 3.1.1 qua 4G LTE, QoS 0/1 theo loại bản tin, kết hợp hàng đợi cục bộ microSD và cơ chế replay khi kết nối phục hồi
...
![Hình 4.1a - Bố trí chi tiết các khối phần cứng trong tracker](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-1a.svg)
...
**Giao tiếp I2C với cảm biến LIS3DSH:** Cảm biến gia tốc 3 trục LIS3DSH được kết nối qua bus I2C (GPIO2 SDA, GPIO1 SCL). Cảm biến này đảm nhiệm chức năng phát hiện chuyển động (motion detection) khi xe đang đỗ. Với pin mapping hiện tại, tín hiệu INT1 đi vào GPIO41 nên nhánh canh rung dùng GPIO wake trong light sleep; heartbeat định kỳ vẫn dùng timer wake ở deep sleep.
...
![Hình 4.2a - Sơ đồ chân kết nối SIM7600CE-T với ESP32-S3](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-2a.svg)
...
- **Wakeup cho nhánh IMU** hiện dùng GPIO wake trong light sleep. Với wiring production hiện tại, chân GPIO 41 không thuộc nhóm RTC GPIO nên không thể dùng EXT0/EXT1 để đánh thức từ deep sleep theo ngắt IMU.

#### c) Sơ đồ đấu nối tổng thể

Sơ đồ đấu nối mô tả cách ánh xạ chân giữa ESP32-S3-WROOM-1 trên PCB chính và các khối phần cứng tích hợp trên bo mạch. Hình 4.5 đã được chuẩn hóa theo pin mapping firmware hiện tại, trong đó `GPIO1/2` dành cho I2C IMU, `GPIO41` nhận `INT1`, `GPIO26` điều khiển `PWR-KEY`, `GPIO19` nhận `LVD_STATUS`, và `GPIO4` đo `U_batt`.
...
_Hình 4.5: Sơ đồ đấu nối tổng thể giữa ESP32-S3 và các khối phần cứng tích hợp_
...
1. **LDO 3.3V (AP2112-3.3):** Hạ từ bus 5V chính xuống 3.3V cấp ESP32-S3
...
6. **Mạch sạc TP5100 + LVD (khối LVD/ADC):** Sạc pin 1S và giám sát ngưỡng điện áp bảo vệ ắc quy
...
![Hình 4.6a - Kiến trúc power path giữa nguồn xe và pin dự phòng](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-6a.svg)

_Hình 4.6a: Kiến trúc power path giữa nguồn xe và pin dự phòng_

> Nguồn: Hình vẽ UML kỹ thuật của tác giả

#### b) LDO 3.3V cho ESP32-S3 (AP2112-3.3)

Khối 3.3V dùng **AP2112-3.3** như một LDO để hạ từ bus 5V chính xuống 3.3V cấp cho ESP32-S3.
...
#### f) Mạch sạc pin TP5100

Khối sạc pin dùng **TP5100** ở cấu hình **1 cell**, nhận **5V từ MP2482** và sạc pin 18650 1S ở mức **4.2V**.

**Bảng 4.7: Khối sạc TP5100**

| Khối    | IC     | Input        | Output | Tải          |
| ------- | ------ | ------------ | ------ | ------------ |
| Sạc pin | TP5100 | 5V từ MP2482 | 4.2V   | Pin 18650 1S |

![Hình 4.10 - Kiến trúc sạc pin TP5100 và đường cấp nguồn dự phòng](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-10.svg)

_Hình 4.10: Kiến trúc sạc pin TP5100 và đường cấp nguồn dự phòng_

> Nguồn: Hình vẽ của tác giả

Dòng sạc của TP5100 được cấu hình bằng điện trở thiết lập dòng sạc và còn phụ thuộc điều kiện nhiệt của mạch, không cố định một giá trị trong mọi điều kiện.

#### g) Mạch giám sát điện áp Low Voltage Disconnect (LVD)

Hệ thống dùng ADC (R1 = 100 kΩ, R2 = 10 kΩ) kết hợp khối LVD phần cứng để giám sát điện áp.
...
Bố cục bên trong được chia thành ba vùng rõ ràng: cụm xử lý và truyền thông (`ESP32-S3`, `SIM7600CE-T`), cụm nguồn (`AP2112-3.3`, `MP2482`, `TPS54231`, `SX1308`, `TP5100`, `khối LVD`) và cụm lưu trữ năng lượng (`pin 18650 1S + BMS`). Hình 4.11 thể hiện cách nhóm các khối phần cứng này trong vỏ hộp cùng vị trí anten và cổng OBD2.
...
**Tản nhiệt:** Các khối công suất (MP2482, SX1308) được bố trí tại vị trí thông thoáng, tách khỏi cảm biến nhiệt độ; khe thông gió đặt ở mặt bên và mặt dưới vỏ hộp. IC MP2482 được gắn heatsink khi vận hành công suất cao.
...
| --------------------------------------------- | ------------- |
| Thành phần chính (MCU, cảm biến, modem, OBD2) | 1.231.000     |
| Hệ thống quản lý nguồn                        | 88.000        |
| Linh kiện phụ trợ                             | 53.000        |
| PCB (nếu tự thiết kế)                         | 60.000        |
| Vỏ bảo vệ                                     | 100.000       |
| **Tổng cộng**                                 | **1.514.000** |
...
**Bước 1 - Kiểm tra linh kiện:** Kiểm tra toàn bộ IC, linh kiện và bán thành phẩm trước khi lắp ráp. Test riêng từng khối chức năng chính (ESP32-S3-WROOM-1, AP2112-3.3, MP2482, TPS54231, SX1308, TP5100, SIM7600CE-T, khối LVD + ADC) để bảo đảm hoạt động đúng.

**Bước 2 - Lắp ráp khối nguồn trên PCB:** Hàn và kiểm tra MP2482 để tạo bus 5V từ nguồn ắc quy xe (12V hoặc 24V). Lắp AP2112-3.3 cấp rail 3.3V cho ESP32-S3 từ bus 5V chính, TPS54231 tạo rail ~4V cho modem SIM7600CE-T, SX1308 tạo 5V backup từ pin 18650 1S, diode OR giữa nhánh 5V chính và nhánh 5V backup, cùng TP5100 kết nối với pin và BMS.

**Bước 3 - Lắp ráp khối xử lý và điều khiển:** Hàn ESP32-S3-WROOM-1, LIS3DSH, khối LVD + ADC và các linh kiện liên quan lên PCB chính. Kiểm tra các chân GPIO theo bảng phân công (Bảng 4.1) và cấp nguồn theo từng rail chức năng (3.3V logic, ~4V modem, 5V bus/backup).

**Bước 4 - Tích hợp các thành phần còn lại:** Lắp modem SIM7600CE-T lên bo mạch, kiểm tra UART1 (GPIO16, GPIO17) và chân `PWR-KEY` (GPIO26), cấu hình lấy dữ liệu GNSS qua `AT+CGNSINF`/`AT+CGNSTST` trên cùng UART, kiểm tra cảm biến LIS3DSH trên I2C (GPIO2, GPIO1), và hiệu chuẩn mạch đo điện áp ắc quy tại GPIO4 (ADC).
...
![Hình 4.12 - Quy trình lắp ráp phần cứng trên PCB](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-12.svg)

_Hình 4.12: Quy trình lắp ráp phần cứng trên PCB_

> Nguồn: Hình dựng minh họa bởi tác giả, bám theo quy trình lắp ráp thực tế

![Hình 4.13 - Sơ đồ bố trí linh kiện sau khi lắp ráp](./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-13.svg)

_Hình 4.13: Sơ đồ bố trí linh kiện sau khi lắp ráp_

> Nguồn: Hình dựng minh họa bởi tác giả, bám theo bố cục phần cứng đã chế tạo
...
- **Kiểm tra sạc pin:** Xác nhận khối sạc TP5100 hoạt động đúng: sạc khi IGN ON, ngừng sạc khi IGN OFF hoặc U_batt thấp.
...
Firmware được tổ chức quanh bốn nhóm chức năng chính, phản ánh đúng cách chia mô-đun trong mã nguồn hiện tại:

**Bảng 4.6: Kiến trúc phân tầng firmware**

| Nhóm | Thành phần                                                                              | Chức năng                                                              |
| ---- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 1    | `adc_reader`, `imu_lis3dh`, `modem_at`, `modem_lte`, `modem_gnss`, `ble_mgr`, `ble_obd` | Truy cập phần cứng và giao thức ngoại vi                               |
| 2    | `power_mgr`, `nvs_config`, `app_state`                                                  | Quản lý nguồn, ngưỡng LVD, cấu hình lưu bền và ngữ cảnh qua deep sleep |
| 3    | `state_machine`, `command_handler`                                                      | Điều phối trạng thái vận hành, lệnh điều khiển, OTA và rollback        |
| 4    | `data_formatter`, `mqtt_client`, `util`                                                 | Đóng gói payload, giao tiếp MQTT, tải firmware OTA và kiểm tra SHA-256 |

Việc đồng bộ giữa các mô-đun được thực hiện có chọn lọc bằng mutex, semaphore và queue ở cấp driver. Cách tiếp cận này giúp firmware vẫn tận dụng được FreeRTOS nhưng tránh phân mảnh logic sang quá nhiều task ứng dụng khó kiểm soát.
...
Trong triển khai hiện tại, modem SIM7600CE-T đảm nhiệm việc đăng ký mạng và kích hoạt PDP context bằng AT command; sau khi đường truyền dữ liệu sẵn sàng, firmware sử dụng `esp_mqtt_client` của ESP-IDF để mở phiên MQTT, publish telemetry và subscribe topic lệnh điều khiển. Cách làm này giúp phần truyền MQTT tách biệt rõ khỏi phần điều khiển modem, dễ debug hơn và tận dụng trực tiếp event loop của ESP-IDF.

```c
static void tracker_mqtt_build_topics(void) {
    snprintf(s_topic_rawdata, sizeof(s_topic_rawdata), "v1/%s/rawdata", s_cfg.device_id);
    snprintf(s_topic_status, sizeof(s_topic_status), "v1/%s/status", s_cfg.device_id);
    snprintf(s_topic_events, sizeof(s_topic_events), "v1/%s/events", s_cfg.device_id);
    snprintf(s_topic_firmware, sizeof(s_topic_firmware), "v1/%s/firmware", s_cfg.device_id);
    snprintf(s_topic_commands, sizeof(s_topic_commands), "v1/%s/commands", s_cfg.device_id);
}

static void tracker_mqtt_event_handler(..., int32_t event_id, void *event_data) {
...
Trong đó, `rawdata` được publish với QoS 0 để tối ưu băng thông cho luồng telemetry tần suất cao; còn `status`, `events`, `firmware` và `commands` dùng QoS 1 để tăng độ tin cậy cho heartbeat, cảnh báo, OTA và lệnh điều khiển.
...
Firmware lắng nghe lệnh từ server thông qua topic `v1/{device_id}/commands`. Payload lệnh được `command_handler.c` phân tích theo cấu trúc JSON, sau đó cập nhật cấu hình hoặc đặt cờ hành động để `state_machine.c` xử lý ở chu kỳ kế tiếp.
...
#### e) Luồng cập nhật firmware OTA

Tính năng OTA mới được tích hợp xuyên suốt từ backend đến firmware. Backend tạo `jobId`, tra cứu firmware theo `firmwareVersion`, sinh URL tải file dạng `/api/v1/firmware/{id}/download`, rồi gửi lệnh `ota_update` qua topic `v1/{device_id}/commands`. Firmware kiểm tra đầy đủ các tham số `jobId`, `version`, `url`, `size`, `sha256`, `force` và `confirmTimeoutSec` trước khi bắt đầu cập nhật.
...
if (util_ota_apply_update(&s_config, s_current_version, &cmd, &report) == ESP_OK) {
    ...
    util_copy_string(g_rtc_context.ota_target_version, sizeof(g_rtc_context.ota_target_version), cmd.version);
    util_copy_string(g_rtc_context.ota_previous_version, sizeof(g_rtc_context.ota_previous_version), s_current_version);
...
Hàm `util_ota_apply_update()` sử dụng HTTPS để tải file firmware, ghi trực tiếp xuống OTA partition, tính và đối chiếu SHA-256, sau đó đặt phân vùng boot mới bằng `esp_ota_set_boot_partition()`. Sau lần khởi động kế tiếp, firmware phát trạng thái `confirming`, gọi `esp_ota_mark_app_valid_cancel_rollback()` để xác nhận image mới, rồi mới phát `success`. Nếu có lỗi trong quá trình xác nhận hoặc người vận hành gửi lệnh `manual_rollback`, hệ thống sẽ quay về phân vùng trước đó và phát trạng thái `failed` hoặc `rolled_back` tương ứng.

![Hình 4.16a - Chuỗi bước OTA trên firmware từ nhận lệnh đến xác nhận sau reboot](./assets/figures/08-chuong-4-trien-khai-firmware-hinh-4-16a.svg)

_Hình 4.16a: Chuỗi bước OTA trên firmware từ nhận lệnh đến xác nhận sau reboot_

> Nguồn: Hình dựng từ mã nguồn `command_handler.c`, `state_machine.c` và `util.c`
...
Module quản lý nguồn thực hiện đọc điện áp ắc quy qua ADC, điều khiển nhánh nguồn theo kiến trúc diode OR + EN, và điều khiển khối sạc TP5100.
...
void enter_deep_sleep(uint32_t sleep_duration_sec)
{
    ...
    //    - IMU interrupt trên GPIO41: dùng GPIO wake ở light sleep
    gpio_wakeup_enable(PIN_LIS3DH_INT, GPIO_INTR_HIGH_LEVEL);
...
![Hình 4.17 - Lưu đồ thuật toán chính của firmware thiết bị theo dõi xe](./assets/figures/08-chuong-4-trien-khai-firmware-hinh-4-17.svg)

_Hình 4.17: Lưu đồ thuật toán chính của firmware thiết bị theo dõi xe_

#### c) Định dạng dữ liệu telemetry

Payload telemetry thực tế của thiết bị hiện nay sử dụng một envelope gọn, trong đó metadata nằm ở cấp gốc và dữ liệu đo nằm trong trường `data`. Cấu trúc này giúp MQTT Bridge kiểm tra payload nhanh và phân luồng trực tiếp sang VictoriaMetrics/VictoriaLogs.
...
Kiến trúc cloud triển khai thực tế gồm thiết bị IoT gửi MQTT vào EMQX, MQTT Bridge phân luồng dữ liệu sang VictoriaMetrics, VictoriaLogs và PostgreSQL, còn Backend/API cùng Frontend Dashboard đảm nhiệm truy vấn, điều khiển và hiển thị thời gian thực.
...
Hệ thống áp dụng mô hình triển khai per-service Docker Compose theo quy ước IVM26. Theo đó, mỗi dịch vụ có một file `docker-compose.yml` riêng thay vì gom toàn bộ vào một file lớn. Cách tổ chức này có một số lợi điểm rõ ràng:
...
![Hình 4.19 - Cấu trúc thư mục hệ thống theo mô hình triển khai](./assets/figures/09-chuong-4-trien-khai-cloud-hinh-4-16.svg)

_Hình 4.19: Cấu trúc thư mục hệ thống theo mô hình triển khai_
...
services:
  emqx:
    ...
    # EMQX stays internal-only on the Docker network.
    # NPM exposes MQTT TLS :8883 via TCP stream and the dashboard via HTTPS reverse proxy.
    volumes:
...
MQTT Bridge (`Tracking_MqttBridge/`) là dịch vụ Node.js/TypeScript độc lập, làm nhiệm vụ nhận dữ liệu từ EMQX rồi ghi về các lớp lưu trữ phía sau như PostgreSQL, VictoriaMetrics và VictoriaLogs. Việc tách dịch vụ này khỏi Backend API giúp đường ingest telemetry gọn hơn, dễ scale riêng và tránh trộn lẫn với lớp nghiệp vụ web.
...
Trong các handler này, `handleFirmware()` là mắt xích mới đáng chú ý sau khi bổ sung OTA. Handler này không chỉ kiểm tra payload trạng thái firmware mà còn ghi tiến trình cập nhật vào `firmware_update_log`, đồng thời phát bản ghi sang VictoriaLogs để thuận tiện cho việc truy vết khi cập nhật thất bại hoặc rollback.

Cần phân biệt rõ hai chiều dữ liệu: MQTT Bridge xử lý chiều **thiết bị -> cloud**; còn chiều **cloud -> thiết bị**, đặc biệt với lệnh OTA, do backend publish trực tiếp vào topic `v1/{device_id}/commands` thông qua `device-command.service.ts`. MQTT Bridge chỉ tiếp nhận topic `firmware` khi thiết bị gửi ngược tiến trình cập nhật.
...
Ngoài schema cho telemetry, MQTT Bridge còn dùng `firmwareStatusSchema` để kiểm tra chặt các trường `jobId`, `status`, `progress`, `targetVersion`, `currentVersion`, `partition` và `error`. Nhờ vậy, luồng OTA có một lớp bảo vệ riêng trước khi dữ liệu được ghi vào cơ sở dữ liệu.
...
- **PostgreSQL**: Cập nhật trạng thái hiện tại của thiết bị (`last_seen_at`, `current_status`), tạo bản ghi phiên hoạt động (session), và lưu các thực thể nghiệp vụ như alert, trip và vùng cho phép.
...
| Vùng cho phép  | `/api/v1/geofences` | Có                 | CRUD vùng địa lý                         |
...
/firmware       // Sự kiện gán/cập nhật tác vụ OTA (yêu cầu xác thực)
/exports        // Tiến trình xuất báo cáo (yêu cầu xác thực)
/notifications  // Thông báo đẩy (yêu cầu xác thực)
```

Các sự kiện chính được phát từ server đến client bao gồm: `device:status`, `device:position`, `device:session_start`, `device:session_end`, `command:ack`, `stats:update`, `alert:new`, `geofence:enter`, `geofence:exit`, `export:ready` và `firmware:assignment`. Trong triển khai hiện tại, `firmware:assignment` phản ánh bước khởi tạo hoặc cập nhật tác vụ OTA từ backend; tiến trình thực thi chi tiết vẫn được theo dõi chủ yếu qua `firmware_update_log` và lớp log quan trắc.
...
Hiển thị tất cả các xe trên bản đồ Leaflet với vị trí cập nhật thời gian thực thông qua WebSocket. Hỗ trợ: hiển thị marker cho từng xe với popup trạng thái, lọc theo xe hoặc trạng thái, hiển thị vùng cho phép, và phát lại hành trình (route replay).
...
Cho phép tạo và quản lý các vùng cho phép trên bản đồ. Khi xe ra khỏi hoặc vào vùng đã định nghĩa, hệ thống sẽ tự động tạo cảnh báo.
...
| Phase 3: Advanced Features   | Allowed-zone management, maintenance, notification settings (Telegram + Email), user management, settings   | Hoàn thành      |
...
Phần này trình bày thiết lập thử nghiệm, kết quả đo theo từng tầng hệ thống (phần cứng, firmware, cloud), kết quả kiểm thử tích hợp end-to-end và đối chiếu với các tiêu chí thiết kế ở Chương 1 và Chương 2. Toàn bộ giá trị được ghi nhận trên thiết bị phần cứng đã chế tạo và lắp đặt thực tế trong điều kiện vận hành bám sát bối cảnh khai thác.

> **Ghi chú:** Các giá trị đo lường trong chương này là kết quả thu được trên môi trường thử nghiệm cụ thể (xem mục 4.3.1). Một số chỉ số được trình bày dưới dạng ước tính ban đầu hoặc cần đo lặp lại trên nhiều mẫu để tăng độ tin cậy thống kê.
...
| 4   | Tủ nhiệt               | Tủ nhiệt phòng thí nghiệm   | -20°C đến +80°C                     | Kiểm thử nhiệt độ hoạt động                        |
| 5   | OBD2 Simulator         | ELM327 OBD2 Simulator Board | Hỗ trợ tất cả giao thức OBD2        | Mô phỏng dữ liệu xe                                |
| 6   | Điểm đo GPS tham chiếu | Thử nghiệm ngoài trời       | Tọa độ chuẩn đối chiếu              | Kiểm thử độ chính xác GPS                          |
...
- Giai đoạn hiện tại đã hoàn tất đo trên 2 xe; xe thứ ba sẽ được bổ sung ở vòng kiểm thử tương thích mở rộng.
...
| 4   | Deep Sleep Mode             | MCU deep sleep + IMU wake | ~0.5 mA         | ~2 mA (ước tính, cần xác nhận thêm) | Chỉ IMU + RTC hoạt động          |
...
| 6   | 70°C                  | Cảnh báo             | Có      | Có (không ổn định) | Có       | Module 4G bắt đầu không ổn định, cần kiểm tra lặp lại |
...
| 3   | Độ trễ ngắt                  | < 100 ms                               | ~50 ms (đo bước đầu)                                  | —        | Đạt            |
| 4   | Độ trễ đóng lại              | < 500 ms                               | ~200 ms (đo bước đầu)                                 | —        | Đạt            |
| 5   | Hysteresis                   | 1.0V                                   | 1.05V (đo bước đầu)                                   | 5%       | Đạt            |
...
| 3   | Trong phố (urban canyon) | ~5.8 mét                    | ~15 mét (cần xác nhận thêm) | ~10 mét (cần xác nhận thêm) |
| 4   | Bãi đỗ xe có mái che     | ~8.5 mét (ước tính ban đầu) | ~20 mét (cần xác nhận thêm) | ~15 mét (cần xác nhận thêm) |
...
Khác với phần reconnect được đánh giá bằng các phép đo định lượng, luồng OTA ở giai đoạn này được đối chiếu theo mã nguồn và kiểm tra tích hợp giữa backend, firmware và MQTT Bridge. Mục tiêu là xác nhận toàn bộ đường đi của lệnh cập nhật, trạng thái phản hồi và cơ chế lưu vết đã được nối thông.

[Bảng 4.25A: Đối chiếu luồng OTA theo mã nguồn triển khai]

| Thành phần  | Điểm kiểm tra                                                                                                           | Kết quả      |
| ----------- | ----------------------------------------------------------------------------------------------------------------------- | ------------ |
| Backend     | `POST /api/v1/devices/:id/ota` tạo `jobId`, tra cứu firmware theo phiên bản, sinh URL download và gửi lệnh `ota_update` | Đã hiện thực |
| Backend     | `POST /api/v1/devices/:id/ota/rollback` gửi lệnh `manual_rollback`                                                      | Đã hiện thực |
| Firmware    | Parse đủ `jobId`, `version`, `url`, `size`, `sha256`, `force`, `confirmTimeoutSec`                                      | Đã hiện thực |
| Firmware    | Tải firmware qua HTTPS, kiểm tra SHA-256, đổi boot partition và xác nhận sau reboot                                     | Đã hiện thực |
| Firmware    | Hỗ trợ rollback thủ công về phân vùng trước đó                                                                          | Đã hiện thực |
| MQTT Bridge | Validate payload OTA, upsert `firmware_update_log`, ghi VictoriaLogs                                                    | Đã hiện thực |

**Nhận xét:** Luồng OTA cơ bản đã khép kín từ backend đến thiết bị và quay lại lớp giám sát. Ở phạm vi triển khai hiện nay, firmware phát các mốc quan trọng như `assigned`, `rebooting`, `confirming`, `success`, `failed` hoặc `rolled_back`; trong khi đó MQTT Bridge đã sẵn sàng tiếp nhận và lưu vết đầy đủ cho các lần cập nhật.
...
| 5   | 200                  | 5                   | ~400 msg/s     | ~55%       | ~3.5 GB    | 0.1% (ước tính ở tải biên) | ~350 ms (ước tính ở tải biên) |
...
| --- | ------------------------------ | --------------------------------------- | ---------- | ---------- |
| 1   | Lighthouse Performance Score   | 87/100 (đo trên bản dựng thử nghiệm)    | > 85       | Đạt        |
| 2   | First Contentful Paint (FCP)   | ~1.2 giây (đo trên bản dựng thử nghiệm) | < 1.5 giây | Đạt        |
| 3   | Largest Contentful Paint (LCP) | ~2.1 giây (đo trên bản dựng thử nghiệm) | < 2.5 giây | Đạt        |
| 4   | Total Blocking Time (TBT)      | ~120 ms (đo trên bản dựng thử nghiệm)   | < 200 ms   | Đạt        |
| 5   | Cumulative Layout Shift (CLS)  | ~0.05 (đo trên bản dựng thử nghiệm)     | < 0.1      | Đạt        |
| 6   | Time to Interactive (TTI)      | ~2.5 giây (đo trên bản dựng thử nghiệm) | < 3.5 giây | Đạt        |
| 7   | Bundle Size (gzipped)          | ~380 KB (đo trên bản dựng thử nghiệm)   | < 500 KB   | Đạt        |
...
![Hình 4.42 - Minh họa giao diện Dashboard hiển thị vị trí xe đang di chuyển trên bản đồ Leaflet](./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-33.svg)

_Hình 4.42: Minh họa giao diện Dashboard hiển thị vị trí xe đang di chuyển trên bản đồ Leaflet_

> Nguồn: Hình dựng từ dữ liệu đo và kịch bản thử nghiệm của tác giả

![Hình 4.43 - Minh họa khung dữ liệu OBD2 thời gian thực trên giao diện (RPM, Speed, Coolant Temp)](./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-34.svg)

_Hình 4.43: Minh họa khung dữ liệu OBD2 thời gian thực trên giao diện (RPM, Speed, Coolant Temp)_

> Nguồn: Hình dựng từ dữ liệu đo và kịch bản thử nghiệm của tác giả

#### 4.3.5.2. Kiểm thử cảnh báo vượt vùng cho phép

**Kịch bản kiểm thử:** Tạo vùng cho phép bán kính 1 km quanh một điểm xác định -> Lái xe đi ra khỏi vùng cho phép -> Hệ thống phát cảnh báo -> Dashboard hiển thị thông báo.

[Bảng 4.32: Kết quả kiểm thử cảnh báo vượt vùng cho phép]

| STT | Sự kiện                          | Kết quả                                             | Thời gian phát hiện        | Ghi chú                   |
| --- | -------------------------------- | --------------------------------------------------- | -------------------------- | ------------------------- |
| 1   | Xe đi vào vùng cho phép          | Hiển thị trạng thái "INSIDE"                        | ~2 giây                    | Cập nhật khi nhận GPS mới |
| 2   | Xe đi ra khỏi vùng cho phép      | Cảnh báo "GEOFENCE_EXIT"                            | ~5 giây                    | Phụ thuộc chu kỳ gửi GPS  |
| 3   | Cảnh báo hiển thị trên Dashboard | Popup notification xuất hiện                        | ~1 giây (từ lúc alert tạo) | Qua WebSocket             |
| 4   | Xe quay lại vùng cho phép        | Hiển thị trạng thái "INSIDE", cảnh báo tự động đóng | ~3 giây                    | Reset tự động             |

![Hình 4.44 - Minh họa màn hình cảnh báo vượt vùng cho phép với bản đồ và vùng theo dõi](./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-35.svg)

_Hình 4.44: Minh họa màn hình cảnh báo vượt vùng cho phép với bản đồ và vùng theo dõi_

> Nguồn: Hình dựng từ dữ liệu đo và kịch bản thử nghiệm của tác giả

**Nhận xét:** Hệ thống phát hiện xe vượt ra khỏi vùng cho phép trong vòng ~5 giây (phụ thuộc chu kỳ gửi GPS, mặc định 5 giây). Cảnh báo được hiển thị trên Dashboard trong vòng ~1 giây sau khi Backend tạo alert. Tổng thời gian từ xe vượt ranh giới đến hiển thị cảnh báo là khoảng 5–7 giây, đạt yêu cầu thiết kế (< 10 giây).
...
![Hình 4.45 - Minh họa luồng gửi lệnh điều khiển từ Dashboard và phản hồi từ thiết bị](./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-36.svg)

_Hình 4.45: Minh họa luồng gửi lệnh điều khiển từ Dashboard và phản hồi từ thiết bị_
...
_Hình 4.46: Minh họa quá trình bản đồ cập nhật lại hành trình sau khi phục hồi kết nối_
...
| STT | Tiêu chí                       | Chỉ tiêu thiết kế | Kết quả đạt được            | Trạng thái | Ghi chú                 |
| --- | ------------------------------ | ----------------- | --------------------------- | ---------- | ----------------------- |
| 1   | Lighthouse Performance         | > 85              | ~87 (đo trên desktop)       | Đạt        | Desktop mode            |
| 2   | First Contentful Paint         | < 1.5 giây        | ~1.2 giây (đo trên desktop) | Đạt        | Next.js SSR             |
...
**Tổng kết:** Hệ thống đã đáp ứng phần lớn các chỉ tiêu cốt lõi của cấu hình đang triển khai, nổi bật ở các tiêu chí GPS, độ trễ end-to-end, khả năng xử lý đồng thời, vùng cho phép, cảnh báo bất thường và reconnect MQTT. Những điểm cần tiếp tục tối ưu chủ yếu nằm ở dòng tiêu thụ khi phát 4G liên tục, ngưỡng hiệu chuẩn LVD profile 12V và hạng mục replay telemetry từ flash nếu muốn bảo toàn đầy đủ dữ liệu trong các khoảng mất sóng kéo dài.
...
| --- | --------------------------------------------------- | -------------------------------- | ------------------------------------------- |
| 1   | Tỷ lệ uptime Backend API (7 ngày)                   | 99.6%                            | Đạt, phù hợp môi trường triển khai hiện tại |
...
Trên cơ sở quá trình hiện thực đã trình bày, Chương 4 tổng hợp kết quả theo bốn nhóm chính: phần cứng, firmware, cloud và đo lường. Thiết bị tracker đã được chế tạo thành một bộ phần cứng hoàn chỉnh và đưa vào vận hành thử; firmware ESP-IDF/FreeRTOS hiện thực đầy đủ các mô-đun giao tiếp, máy trạng thái năng lượng và cơ chế tự phục hồi kết nối; hạ tầng cloud cùng backend/frontend cũng vận hành đồng bộ theo kiến trúc đề xuất.

Về đo lường, hệ thống đã đáp ứng phần lớn chỉ tiêu cốt lõi của cấu hình đang triển khai. Các tiêu chí quan trọng như độ chính xác GPS, độ trễ end-to-end, thời lượng pin dự phòng, khả năng xử lý đồng thời, thời gian tải dashboard, vùng cho phép, cảnh báo bất thường và độ ổn định của state machine đều đạt hoặc vượt mục tiêu tổng hợp. Dù vậy, một số hạng mục vẫn cần tiếp tục hoàn thiện ở vòng tối ưu tiếp theo, đáng chú ý là ngưỡng LVD profile 12V, mức dòng ở pha truyền 4G liên tục và cơ chế replay telemetry từ flash cho các tình huống mất sóng kéo dài.
...
Firmware xây dựng trên nền tảng ESP-IDF với FreeRTOS được đánh giá theo ba khía cạnh: mô hình điều phối thực thi, độ ổn định của máy trạng thái và khả năng tự phục hồi kết nối kèm OTA.

**Điều phối thực thi với FreeRTOS:**

Mã nguồn hiện tại không tổ chức thành nhiều task nghiệp vụ cấp cao như các kiến trúc mẫu thường dùng trong hệ nhúng. Thay vào đó, firmware giữ một vòng điều khiển trung tâm gọi `state_machine_run()` theo chu kỳ; còn FreeRTOS được dùng có chọn lọc cho các phần cần nền bất đồng bộ hoặc đồng bộ truy cập tài nguyên, như NimBLE host task, mutex của lớp AT command, queue/mutex trong BLE manager và semaphore phản hồi OBD2. Cách tổ chức này giúp logic firmware tập trung hơn, dễ đọc và thuận lợi khi truy vết lỗi trên thiết bị hiện tại.
...
| --------------------------------- | -------------------------------------------------- | ------------------------------------------------------------ | ----------------------------- |
| Mô hình điều phối chính           | State machine trung tâm, số task ứng dụng tối giản | `app_main` + `state_machine_run`; NimBLE chạy task nền riêng | Phù hợp mã nguồn              |
...
| Luồng OTA cơ bản                  | Có gán job, cập nhật, xác nhận và rollback         | Đã hiện thực; chưa có watchdog                               | Đạt ở mức triển khai hiện tại |
...
EMQX xử lý tin nhắn MQTT từ các thiết bị IoT với độ tin cậy cao. Trong triển khai hiện tại, broker chạy EMQX 5.x nhưng đường truyền của firmware đang dùng MQTT 3.1.1 cùng ACL theo từng thiết bị, QoS 0/1 và cây topic `v1/{device_id}/*`. Phần phân luồng và xử lý dữ liệu chủ yếu do MQTT Bridge đảm nhiệm; EMQX giữ vai trò broker trung tâm, còn Rules Engine chỉ là khả năng bổ trợ chứ không phải tuyến xử lý nghiệp vụ chính.
...
PostgreSQL 16 lưu trữ dữ liệu quan hệ (người dùng, xe, khách hàng, hành trình, cảnh báo, vùng cho phép) với hiệu suất truy vấn ổn định. VictoriaMetrics lưu trữ dữ liệu chuỗi thời gian (tọa độ GPS, dữ liệu OBD2) với hiệu suất ghi và đọc cao hơn so với InfluxDB truyền thống, đồng thời tiêu thụ ít tài nguyên hơn.
...
Toàn bộ ngăn xếp công nghệ của hệ thống đều là mã nguồn mở, giúp loại bỏ chi phí bản quyền phần mềm:
...
Phần này đánh giá rủi ro của hệ thống bằng ma trận xác suất - tác động (Probability-Impact Matrix) gồm 5 mức độ. Các rủi ro được xác định từ đặc điểm kỹ thuật của thiết bị hiện tại và các tình huống dễ phát sinh trong quá trình triển khai thực tế.
...
Phiên bản phần cứng hiện có sử dụng bo mạch 2 lớp do nhóm thiết kế và được lắp ráp thủ công, đủ để triển khai và đánh giá ở quy mô đồ án. Để tiến tới sản xuất hàng loạt, các cải tiến phần cứng tiếp theo nên tập trung vào tối ưu DFM, EMI, độ gọn cơ khí và độ tin cậy:
...
Kết quả đánh giá cho thấy hệ thống đã đáp ứng được các mục tiêu kỹ thuật quan trọng của một thiết bị giám sát phương tiện hoàn chỉnh có thể vận hành thực tế. Phần cứng giữ được mức tiêu thụ điện phù hợp, firmware vận hành ổn định với state machine rõ ràng và đã bổ sung được luồng OTA cơ bản, còn hạ tầng cloud duy trì độ trễ thấp cho bài toán theo dõi thời gian thực. Về chi phí, BOM 870.000–1.630.000 VND cùng chi phí vận hành thấp tạo ra lợi thế rõ rệt so với nhiều giải pháp thương mại cùng phân khúc. Phần còn lại cần ưu tiên ở giai đoạn tiếp theo không nằm ở việc thay đổi kiến trúc tổng thể, mà ở việc làm sâu hơn các lớp an toàn như watchdog, bảo mật MQTT/TLS và kiểm thử vận hành dài hạn.

# CHƯƠNG 6. PHẢN HỒI VÀ BÀI HỌC KINH NGHIỆM – REFLECTION AND LESSONS LEARNED

## 6.1. Ứng dụng kiến thức kỹ thuật – Application of prior coursework

Dự án "IoT Vehicle Tracking System" là kết quả của quá trình tổng hợp kiến thức từ nhiều học phần và nhiều lớp công nghệ khác nhau. Từ phần cứng, firmware đến backend và frontend, mỗi quyết định triển khai đều đòi hỏi người thực hiện chuyển kiến thức nền tảng thành lựa chọn kỹ thuật cụ thể. Phần này làm rõ quá trình chuyển hóa đó trong bối cảnh của dự án.
...
2. _Low Voltage Disconnect (LVD)_: Hiện thực LVD bằng khối LVD phần cứng kết hợp ADC firmware, dùng ngưỡng profile: 12V (OFF=12.0V, ON=12.2V) và 24V (OFF=24.0V, ON=24.4V), bảo vệ ắc quy không bị rút cạn quá mức.
3. _Bộ sạc pin dự phòng_: Tích hợp IC sạc TP5100 (input 5V từ MP2482, output 4.2V ở chế độ 1 cell) để sạc pin 18650 1S khi điều kiện nguồn cho phép.
...
Hệ thống IoT giám sát phương tiện tạo ra một số tác động xã hội tích cực có thể nhận thấy:
...
- Cảnh báo vượt vùng cho phép giúp đảm bảo xe hoạt động trong phạm vi an toàn đã thỏa thuận.
...
Dự án sử dụng hoàn toàn công nghệ mã nguồn mở. Quá trình triển khai cho thấy các thành phần này đã đủ ổn định để kết hợp thành một hệ thống IoT hoàn chỉnh:
...
- **PostgreSQL 16**: Cơ sở dữ liệu quan hệ mã nguồn mở mạnh mẽ, hỗ trợ PostGIS cho các bài toán dữ liệu không gian như vùng cho phép.
- **EMQX**: MQTT broker ổn định, hỗ trợ clustering và có Rules Engine để xử lý các tác vụ lọc đơn giản ở tầng broker.
...
Chi phí giấy phép phần mềm của toàn bộ hệ thống là 0 VND, nên ngân sách có thể tập trung cho phần cứng và hạ tầng máy chủ.
...
Dự án "IoT Vehicle Tracking System" đưa người thực hiện đi trọn một vòng từ ý tưởng, thiết kế mạch, đặt mua linh kiện, chế tạo phần cứng, tích hợp và đánh giá. Giá trị lớn nhất của quá trình này không nằm ở việc ghép đủ các thành phần, mà ở chỗ mỗi quyết định kỹ thuật đều phải được kiểm chứng lại bằng số liệu, bằng thử nghiệm và bằng mức độ phù hợp với điều kiện triển khai thật.

Kinh nghiệm rút ra từ dự án, đặc biệt ở các mảng kiến trúc hệ thống, quản lý năng lượng cho thiết bị IoT, bảo mật backend và tổ chức luồng dữ liệu thời gian thực, là nền tảng quan trọng cho công việc chuyên môn sau này.
...
[23] STMicroelectronics, "LIS3DSH - MEMS Digital Output Motion Sensor Ultra-Low-Power High-Performance 3-Axis 'Nano' Accelerometer Datasheet," DocID 17530, Rev. 3, 2021. [Online]. Available: https://www.st.com/resource/en/datasheet/lis3dsh.pdf

[24] STMicroelectronics, "AN3393: LIS3DSH 3-axis digital output accelerometer." [Online]. Available: https://www.st.com/resource/en/application_note/dm00026768-lis3dsh-3axis-digital-output-accelerometer-stmicroelectronics.pdf
...
[30] NanJing Top Power ASIC Corp., "TP5100 2A Switching Buck Charger IC for 8.4V/4.2V Lithium-Ion Battery," Datasheet, Rev. 2.4, 2024. [Online]. Available: https://www.toppwr.com/uploadfile/file/20230304/6402f53e7e8d9.pdf. [Accessed: Apr. 18, 2026].
...
Bảng dưới đây liệt kê chi tiết các linh kiện chính sử dụng trong thiết bị tracker IoT, bao gồm đơn giá và tổng chi phí tham chiếu. Đơn giá được rà soát theo mặt bằng giá ngày 12/04/2026 từ các kênh bán lẻ như Điện tử Á Châu và Điện tử Đức Huy.
...
| ------------------------------------------ | ----------------------- | ---------------------------- |
| Phần cứng (1 bộ tracker)                   | 1.514.000               | Theo BOM ở bảng PL-1.1       |
...
| Công cụ phát triển (khác)                  | 200.000                 | USB-UART, mỏ hàn, dây đo     |
| **Tổng chi phí dự án**                     | **4.504.000–5.704.000** |                              |

> **Nhận xét:** Tổng chi phí dự án vẫn dưới 6.000.000 VND, trong đó chi phí phần cứng cho mỗi bộ tracker khoảng 1.514.000 VND, thấp hơn đáng kể so với các giải pháp thương mại tương đương (2.000.000–5.000.000 VND/thiết bị). Khi sản xuất số lượng lớn (>50 bộ), chi phí linh kiện có thể giảm thêm 15–25% nhờ mua sỉ.
...
**Mô tả:** MQTT vẫn là giao thức nhắn tin phù hợp cho IoT vì gọn, ít overhead và hoạt động tốt trên kết nối 4G. Trong hệ thống này, EMQX chạy phiên bản 5.x ở vai trò broker, nhưng firmware trên thiết bị hiện cấu hình **MQTT 3.1.1**. Vì vậy, phần trình bày dưới đây tập trung vào các đặc tính thực sự đang dùng trong đường truyền thiết bị thay vì liệt kê các khả năng mở rộng của MQTT 5.
...
v1/{device_id}/commands            # Lệnh điều khiển từ server (QoS 1)
v1/{device_id}/status              # Trạng thái phiên/heartbeat (QoS 1)
v1/{device_id}/firmware            # Tiến trình OTA (QoS 1)
internal/events/device/status      # Sự kiện nội bộ cho backend realtime
internal/events/device/alert       # Cảnh báo nội bộ
internal/events/device/session     # Trạng thái phiên nội bộ
internal/events/device/data        # Vị trí / telemetry cho realtime
```
...
| 3    | Thiết kế PCB và đặt làm mạch | PCB layout / bo mạch hoàn chỉnh |
| 4    | Lắp ráp và kiểm tra cơ bản   | Thiết bị phần cứng hoạt động    |
...
| 7    | Dịch vụ MQTT Bridge            | Dữ liệu từ EMQX đến DB |
...
- **Repository:** `https://github.com/anmh1205/IoT_Vehicle_Tracking_System`
- **Branch chính:** `main` (nhánh ổn định để triển khai)
- **Branch phát triển:** `feature/system-coding` (nhánh phát triển đang sử dụng)
- **License:** Chưa khai báo license riêng trong repository; cần chốt trước khi công khai mã nguồn
...
| 8   | `geofences`  | Vùng cho phép                        | N:N với vehicles               |
...
- **Domain (cloud/UAT):** `api.thingdock.dev` (đường dẫn Swagger: `/api-docs`)
- **URL (development):** `http://localhost:4000/api-docs`
- **Tài liệu thiết kế:** `resources/plans/cloud/21-backend-api-endpoints.md`
...
| `FIRMWARE_PUBLIC_BASE_URL` | Backend             | Base URL công khai để thiết bị tải file OTA | Có cho OTA |
...
```bash
# 1. Clone repository
git clone https://github.com/anmh1205/IoT_Vehicle_Tracking_System.git
