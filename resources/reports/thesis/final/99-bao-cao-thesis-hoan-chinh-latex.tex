...
\newcommand{\thesislongtableorignewline}{}
\newcommand{\thesislongtableorigtabularnewline}{}
\AtBeginEnvironment{longtable}{%
  \normalsize
  \let\thesislongtableorignewline\\
  \let\thesislongtableorigtabularnewline\tabularnewline
  \renewcommand{\\}{\thesislongtableorignewline\hline}%
  \renewcommand{\tabularnewline}{\thesislongtableorigtabularnewline\hline}%
}
\AtEndEnvironment{longtable}{%
  \let\\\thesislongtableorignewline
  \let\tabularnewline\thesislongtableorigtabularnewline
}
\usepackage{graphicx}
\makeatletter
...
\newcommand{\thesisfigurecaption}[1]{%
  \begin{center}\emph{#1}\end{center}%
}
\newcommand{\thesistabletitle}[1]{%
...
\textbf{ĐẠI HỌC PHENIKAA}\\[0.25cm]
\rule{0.55\linewidth}{0.6pt}
\vspace*{2.2cm}

{\Large\textbf{ĐỒ ÁN TỐT NGHIỆP}}\\[0.9cm]
...
\textbf{FACULTY OF MECHANICAL ENGINEERING AND MECHATRONICS}\\[0.25cm]
\rule{0.55\linewidth}{0.6pt}
\vspace*{1.8cm}
...
\hline \textbf{STT} & \textbf{Nội dung yêu cầu chỉnh sửa} & \textbf{Nội dung đã chỉnh sửa} & \textbf{Ghi chú} \\
\hline
\endhead
\hline
\endlastfoot
...
\hline \textbf{} \textbf{STT} & \textbf{Nội dung} & \textbf{Trang} \\
\hline
\endhead
\hline
\endlastfoot
...
\textbf{Chương 1} & \textbf{GIỚI THIỆU DỰ ÁN - PROJECT OVERVIEW} & ... \\
...
3.2 & Đề xuất các giải pháp --- Proposed solutions & ... \\
...
\hline \textbf{} \textbf{Bảng} & \textbf{Mô tả} & \textbf{Trang} \\
\hline
\endhead
\hline
\endlastfoot
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}{|C{0.15\linewidth}|L{0.67\linewidth}|C{0.12\linewidth}|}
\hline \textbf{} \textbf{Hình} & \textbf{Mô tả} & \textbf{Trang} \\
\hline
\endhead
\hline
\endlastfoot
...
\hline \textbf{} \textbf{Từ viết tắt} & \textbf{Tiếng Anh} & \textbf{Tiếng Việt} \\
\hline
\endhead
\hline
\endlastfoot
...
\thesisfigurecaption{Hình 1.1: Sơ đồ tổng quan vấn đề và giải pháp đề xuất}
...
\thesistabletitle{Bảng 1.1: Tóm tắt phạm vi dự án theo tầng (layer)}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 1.3.1A: Tổng hợp thông tin về STT, Tiêu chí, Mục tiêu cụ thể}
...
\thesistabletitle{Bảng 1.2: Tiêu chí phần cứng}

\subsubsection{1.3.2. Tiêu chí
firmware}\label{132-tiuxeau-chuxed-firmware}

\thesistabletitle{Bảng 1.2A: Tiêu chí firmware}
...
\thesistabletitle{Bảng 1.2B: Tiêu chí cloud/backend}
...
\thesistabletitle{Bảng 1.2C: Tiêu chí frontend}
...
\thesisfigurecaption{Hình 1.2: Quy trình phát triển dự án theo các giai đoạn}
...
\thesisfigurecaption{Hình 1.3: Kiến trúc tổng thể hệ thống IoT Vehicle Tracking}
...
\thesisfigurecaption{Hình 1.4: Sơ đồ chuyển đổi giữa các chế độ năng lượng}

\subsubsection{1.4.4. Công nghệ sử
dụng}\label{144-cuxf4ng-nghux1ec7-sux1eed-dux1ee5ng}

\thesistabletitle{Bảng 1.3: Tổng hợp công nghệ sử dụng trong dự án}
...
\thesisfigurecaption{Hình 1.5: Lộ trình phát triển dự án theo các giai đoạn (Roadmap)}
...
\thesistabletitle{Bảng 2.1: So sánh các giải pháp giám sát phương tiện}
...
\thesistabletitle{Bảng 2.2: So sánh các giao thức truyền thông IoT}
...
\thesistabletitle{Bảng 2.3: So sánh các vi điều khiển cho ứng dụng IoT tracker}
...
\thesistabletitle{Bảng 2.4: So sánh các giải pháp cơ sở dữ liệu cho hệ thống IoT}
...
\thesisfigurecaption{Hình 2.1: Sơ đồ kiến trúc dữ liệu của hệ thống - Data Architecture
Diagram}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.3.4A: Tổng hợp thông tin về Tiêu chuẩn, Mô tả, Áp dụng trong hệ thống}
...
(Cat-4) \\
NMEA 0183 & Chuẩn dữ liệu GPS & Phân tích tọa độ từ khối GNSS tích hợp \\
...
\thesistabletitle{Bảng 2.5: Ma trận yêu cầu các bên liên quan}
...
\thesistabletitle{Bảng 2.6: Ma trận truy xuất yêu cầu các bên liên quan với chức năng
hệ thống}
...
\thesisfigurecaption{Hình 3.1: Sơ đồ khối tổng thể hệ thống tracker}

\pandocbounded{\safeincludesvg{./assets/figures/thesis-99-bao-cao-thesis-hoan-chinh-01.svg}}

\thesisfigurecaption{Hình 3.1a: Phân rã chi tiết các khối chức năng của tracker}
...
\thesistabletitle{Bảng 3.1: Đối chiếu yêu cầu kỹ thuật khi lựa chọn vi điều khiển}
...
\thesistabletitle{Bảng 3.2: Thông số kỹ thuật ESP32-S3 được chọn}
...
\thesistabletitle{Bảng 3.3: Đối chiếu các phương án LTE/GNSS theo tác động tích hợp}
...
{[}59{]}, {[}61{]} & 1 khối tích hợp LTE + GNSS (SIM7600CE-T) {[}60{]} \\
...
modem & Cao hơn do LTE/GNSS chung khối tích hợp \\
...
& Giảm số khối phần cứng rời, thuận lợi cho đi dây và lắp ráp \\
\end{longtable}
}
...
\thesistabletitle{Bảng 3.4: Thông số kỹ thuật phương án SIM7600CE-T}
...
\thesistabletitle{Bảng 3.5: So sánh phương pháp kết nối OBD2 theo thông số triển
khai}
...
\thesisfigurecaption{Hình 3.2: Sơ đồ kết nối BLE giữa ESP32-S3 và vgate iCar Pro}
...
\thesistabletitle{Bảng 3.6: Chiến lược kết nối BLE OBD2 theo chế độ hoạt động}
...
\thesistabletitle{Bảng 3.7: Thông số kỹ thuật cảm biến LIS3DSH}
...
\thesisfigurecaption{Hình 3.3: Sơ đồ kết nối LIS3DSH với ESP32-S3 qua I2C}

\pandocbounded{\safeincludesvg{./assets/figures/thesis-99-bao-cao-thesis-hoan-chinh-02.svg}}

\thesisfigurecaption{Hình 3.3a: Sơ đồ chi tiết chân kết nối LIS3DSH với ESP32-S3}
...
\thesisfigurecaption{Hình 3.4: Sơ đồ khối hệ thống quản lý nguồn}

\pandocbounded{\safeincludesvg{./assets/figures/thesis-99-bao-cao-thesis-hoan-chinh-03.svg}}

\thesisfigurecaption{Hình 3.4a: Kiến trúc nguồn và phân phối điện áp trong hệ thống}
...
\thesistabletitle{Bảng 3.8: Thông số khối Buck 3.3V}
...
\hline
\endlastfoot
Buck 3.3V & AP2112-3.3 & 12--24V & 3.3V & ESP32-S3 \\
\end{longtable}
}
...
\thesistabletitle{Bảng 3.9: Thông số khối Buck 5V}
...
\thesistabletitle{Bảng 3.10: Thông số khối Buck 3.8V/4V cho modem}
...
\thesistabletitle{Bảng 3.11: Thông số khối Boost 5V dự phòng}
...
\thesistabletitle{Bảng 3.12: Bảng trạng thái chuyển nguồn và cảnh báo}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}{|p{0.06\linewidth}|p{0.14\linewidth}|p{0.17\linewidth}|p{0.17\linewidth}|p{0.14\linewidth}|p{0.24\linewidth}|}
\hline \textbf{Trạng thái} & \textbf{IGN} & \textbf{U} \_batt & Nguồn tracker & Sạc pin & Cảnh báo \\
\hline
\endhead
\hline
\endlastfoot
...
\thesistabletitle{Bảng 3.13: Thông số khối sạc pin}
...
\thesistabletitle{Bảng 3.14: Thông số kỹ thuật pin dự phòng 18650 1S}
...
\thesistabletitle{Bảng 3.14A: Bảng tổng hợp linh kiện hệ thống tracker (BOM)}
...
12--24V -\textgreater{} ~4V cấp modem SIM7600CE-T \\
...
\thesistabletitle{Bảng 3.15: Tổng hợp chi phí theo nhóm}
...
khối LVD, pin 1S) & 80.000--155.000 \\
...
\thesistabletitle{Bảng 3.5A: So sánh các phương án nền tảng firmware}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2A: Tổng hợp thông tin về Lớp, Thành phần chính, Vai trò}
...
\thesisfigurecaption{Hình 3.5: Sơ đồ kiến trúc phân lớp của firmware}
...
\thesisfigurecaption{Hình 3.6: Sơ đồ tương tác giữa các FreeRTOS task}
...
\thesisfigurecaption{Hình 3.7: Lưu đồ thuật toán luồng hoạt động chính của firmware}
...
\thesisfigurecaption{Hình 3.8: Lưu đồ thuật toán quy trình kết nối và đọc dữ liệu BLE
OBD2}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2B: Tổng hợp thông tin về Tình huống lỗi, Xử lý, Thời gian timeout}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2C: Tổng hợp thông tin về Bước, Lệnh AT, Mục đích chính}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2D: Tổng hợp thông tin về Tình huống, Biện pháp}
...
\thesisfigurecaption{Hình 3.9: Lưu đồ điều phối modem SIM7600CE-T theo trạng thái mạng
và chế độ vận hành}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2E: Tổng hợp thông tin về GPIO, Chức năng, Hướng}
...
phòng) \\
19 & LVD\_STATUS & Input & Đọc trạng thái LVD từ khối LVD phần cứng \\
...
\thesisfigurecaption{Hình 3.10: Lưu đồ thuật toán điều khiển power path}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2F: Tổng hợp thông tin về Điều kiện, Trạng thái charger, Lý do}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2G: Tổng hợp thông tin về Chế độ, Trạng thái ESP32, Trạng thái modem}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2H: Tổng hợp thông tin về Topic, Hướng, QoS}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2I: Tổng hợp thông tin về Lệnh, Mục đích, Tham số chính}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2J: Tổng hợp thông tin về Trạng thái, Mã, Mô tả}
...
\thesistabletitle{Bảng chuyển đổi trạng thái:}
...
\thesisfigurecaption{Hình 3.11: Sơ đồ máy trạng thái của thiết bị theo dõi}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.2K: Tổng hợp thông tin về Cảm biến, Phương pháp hiệu chuẩn, Lưu trữ}
...
\thesistabletitle{Bảng 3.15A: So sánh các phương án kiến trúc Cloud}
...
\thesisfigurecaption{Hình 3.12: Sơ đồ kiến trúc tổng quan hệ thống Cloud}

\pandocbounded{\safeincludesvg{./assets/figures/thesis-99-bao-cao-thesis-hoan-chinh-04.svg}}

\thesisfigurecaption{Hình 3.12a: Luồng dữ liệu chi tiết từ thiết bị đến dashboard}
...
\thesistabletitle{Bảng 3.16: Phân bổ tài nguyên Docker cho các dịch vụ}
...
\thesistabletitle{Bảng 3.17: So sánh các MQTT Broker phổ biến}
...
\thesistabletitle{Bảng 3.18A: So sánh các phương án kiến trúc lưu trữ dữ liệu}
...
\thesisfigurecaption{Hình 3.13: Sơ đồ chiến lược lưu trữ kép}
...
\thesistabletitle{Bảng 3.18: So sánh các cơ sở dữ liệu trong hệ thống}
...
\thesistabletitle{Bảng 3.19: Các nhóm bảng chính trong PostgreSQL}
...
\thesisfigurecaption{Hình 3.14: Sơ đồ quan hệ cơ sở dữ liệu (ER Diagram)}
...
\thesistabletitle{Bảng 3.20: So sánh VictoriaMetrics và InfluxDB}
...
\thesistabletitle{Bảng 3.21: So sánh framework Backend}
...
\thesistabletitle{Bảng 3.22: Tóm tắt các nhóm API Endpoints chính}
...
\thesisfigurecaption{Hình 3.14a: Luồng điều phối OTA giữa Backend, EMQX, thiết bị và
MQTT Bridge}
...
\thesistabletitle{Bảng 3.23: So sánh JWT và Session-based authentication}
...
\thesistabletitle{Bảng 3.23A: So sánh các phương án công nghệ frontend tổng thể}
...
\thesistabletitle{Bảng 3.24: So sánh framework Frontend}
...
\thesistabletitle{Bảng 3.25: Chiến lược quản lý trạng thái}
...
\thesistabletitle{Bảng 3.26: So sánh thư viện bản đồ}
...
\thesisfigurecaption{Hình 3.15: Sơ đồ kiến trúc Feature-Sliced Design của ứng dụng
Frontend}
...
\thesisfigurecaption{Hình 3.15a: Luồng dữ liệu một chiều giữa Next.js, Query, Zustand
và Backend}
...
\thesisfigurecaption{Hình 3.16: Biểu đồ trình tự (Sequence Diagram) luồng xác thực
người dùng}
...
\item
  ...
  bản đồ thu nhỏ.
\item
  \thesistabletitle{Bảng cảnh báo gần đây:} Danh sách 10 cảnh báo mới nhất với
...
\thesisfigurecaption{Hình 3.17: Giao diện trang Dashboard tổng quan}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.4A: Tổng hợp thông tin về Cột, Mô tả, Chức năng}
...
\thesisfigurecaption{Hình 3.18: Giao diện trang quản lý phương tiện}
...
\thesistabletitle{Bảng 3.27: Ma trận các trang chức năng chính}
...
\thesisfigurecaption{Hình 3.19a: Phân rã lớp bản đồ React Leaflet và kênh Socket.IO}

\pandocbounded{\safeincludesvg{./assets/figures/06-chuong-3-giai-phap-frontend-hinh-3-19.svg}}

\thesisfigurecaption{Hình 3.19: Kiến trúc tích hợp bản đồ thời gian thực}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.2.4B: Tổng hợp thông tin về Component, File, Chức năng}
...
\thesisfigurecaption{Hình 3.20: Giao diện trang bản đồ thời gian thực với các marker
xe}
...
\thesistabletitle{Bảng 3.28: Breakpoint và tương ứng giao diện}
...
\thesisfigurecaption{Hình 3.21: Wireframe bố cục giao diện Dashboard}
...
\tightlist
\item
  \thesistabletitle{Bảng dữ liệu (Data Tables):} Sắp xếp, lọc, phân trang phía
...
\thesisfigurecaption{Hình 3.22: Các mẫu thiết kế UI của hệ thống - Data Table, Form,
Charts}
...
\thesistabletitle{Bảng 3.29: Ma trận đánh giá tổng hợp các phương án thiết kế}
...
\thesisfigurecaption{Hình 3.23: Sơ đồ kiến trúc tổng thể phương án tối ưu}
...
\thesisfigurecaption{Hình 4.1a: Bố trí chi tiết các khối phần cứng trong tracker}

\pandocbounded{\safeincludesvg{./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-1.svg}}

\thesisfigurecaption{Hình 4.1: Sơ đồ khối tổng thể hệ thống tracker IoT}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.1.1A: Tổng hợp thông tin về Thông số, Giá trị}
...
\thesisfigurecaption{Hình 4.2a: Sơ đồ chân kết nối SIM7600CE-T với ESP32-S3}

\pandocbounded{\safeincludesvg{./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-2.svg}}

\thesisfigurecaption{Hình 4.2: Sơ đồ kết nối giữa ESP32-S3 và modem SIM7600CE-T qua
UART}
...
\thesisfigurecaption{Hình 4.3: Sơ đồ mạch đo điện áp ắc quy bằng voltage divider và ADC
ESP32-S3}
...
\thesisfigurecaption{Hình 4.4: Sơ đồ nguyên lý mạch điện tổng hợp của hệ thống tracker}
...
\thesistabletitle{Bảng 4.1: Phân công chân GPIO của ESP32-S3}
...
\thesisfigurecaption{Hình 4.5: Sơ đồ đấu nối tổng thể giữa ESP32-S3 và các khối phần
cứng tích hợp}
...
\thesisfigurecaption{Hình 4.6a: Kiến trúc power path giữa nguồn xe và pin dự phòng}

\pandocbounded{\safeincludesvg{./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-6.svg}}

\thesisfigurecaption{Hình 4.6: Kiến trúc tổng thể mạch quản lý nguồn}
...
\thesistabletitle{Bảng 4.2: Khối Buck 3.3V}
...
\thesistabletitle{Bảng 4.3: Khối Buck 5V}
...
\thesisfigurecaption{Hình 4.7: Kiến trúc nhánh buck 5V chính dùng MP2482}
...
\thesistabletitle{Bảng 4.4: Khối Buck 3.8V/4V cho modem}
...
\thesistabletitle{Bảng 4.5: Khối Boost 5V dự phòng}
...
\thesisfigurecaption{Hình 4.8: Kiến trúc nhánh nguồn dự phòng 5V từ pin 18650 qua
SX1308}
...
\thesistabletitle{Bảng 4.6: Logic chuyển nguồn tự động}
...
\thesisfigurecaption{Hình 4.9: Sơ đồ power path runtime giữa nhánh chính và nhánh
backup}
...
\thesistabletitle{Bảng 4.7: Khối sạc TP4056}
...
\thesisfigurecaption{Hình 4.10: Kiến trúc sạc pin TP4056 và đường cấp nguồn dự phòng}
...
lượng (\texttt{pin\ 18650\ 1S\ +\ mạch\ bảo\ vệ\ 1S}). Hình 4.11 thể hiện cách nhóm
...
\thesisfigurecaption{Hình 4.11: Sơ đồ bố cục bên trong vỏ hộp bảo vệ}
...
\thesistabletitle{Bảng 4.6: Danh sách vật liệu đầy đủ cho hệ thống tracker}
...
\thesistabletitle{Bảng 4.7: Tổng chi phí ước tính}
...
\thesisfigurecaption{Hình 4.12: Quy trình lắp ráp phần cứng trên PCB}

\pandocbounded{\safeincludesvg{./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-13.svg}}

\thesisfigurecaption{Hình 4.13: Sơ đồ bố trí linh kiện sau khi lắp ráp}
...
\thesisfigurecaption{Hình 4.14: Minh họa vị trí cổng OBD2 trên xe và cách kết nối}
...
\thesisfigurecaption{Hình 4.15: Minh họa lắp đặt thiết bị tracker trong xe và đi dây}
...
\thesisfigurecaption{Hình 4.16: Checklist kiểm tra hệ thống sau khi lắp đặt trong xe}
...
\thesistabletitle{Bảng 4.5: Công cụ phát triển firmware}
...
\thesistabletitle{Bảng 4.6: Kiến trúc phân tầng firmware}
...
\thesistabletitle{Bảng 4.7: Bảng ánh xạ chân GPIO}
...
19 & LVD\_STATUS & Input & Trạng thái từ khối LVD phần cứng \\
21 & LIS3DSH\_INT & Input & Ngắt từ cảm biến gia tốc IMU \\
...
\thesistabletitle{Bảng 4.8: Các PID OBD2 được đọc định kỳ (Mode 0x01 - Current
Data)}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.8A: Bảng bổ sung chỉ mục (4.2.3. Triển khai Firmware)}
...
\thesisfigurecaption{Hình 4.16a: Chuỗi bước OTA trên firmware từ nhận lệnh đến xác nhận
sau reboot}
...
\thesistabletitle{Bảng 4.9: So sánh các chế độ ngủ}
...
\thesistabletitle{Bảng 4.10: Các trạng thái của firmware}
...
\thesistabletitle{Bảng 4.11: Chuyển đổi trạng thái}
...
\thesisfigurecaption{Hình 4.17: Lưu đồ thuật toán chính của firmware thiết bị theo dõi
xe}
...
\thesisfigurecaption{Hình 4.18: Kiến trúc tổng thể hệ thống Cloud và luồng dữ liệu}
...
\thesisfigurecaption{Hình 4.19: Cấu trúc thư mục hệ thống theo quy ước IVM26}
...
\thesistabletitle{Bảng 4.5: Danh sách các dịch vụ Docker và cổng truy cập}
...
\thesisfigurecaption{Hình 4.20: Luồng xử lý dữ liệu của MQTT Bridge}
...
\thesistabletitle{Bảng 4.6: So sánh framework backend cho hệ thống IoT}
...
\thesisfigurecaption{Hình 4.21: Cấu trúc thư mục Backend theo kiến trúc DDD}
...
\thesistabletitle{Bảng 4.7: Danh sách các domain modules chính}
...
\thesistabletitle{Bảng 4.8: Tổng hợp các nhóm API endpoint chính}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.2.5A: Tổng hợp thông tin về Công nghệ, Phiên bản, Vai trò}
...
\thesisfigurecaption{Hình 4.22: Cấu trúc thư mục Frontend theo kiến trúc
Feature-Sliced}
...
\thesisfigurecaption{Hình 4.23: Giao diện trang Dashboard tổng quan}
...
\thesisfigurecaption{Hình 4.24: Giao diện trang quản lý xe}
...
\thesisfigurecaption{Hình 4.25: Giao diện bản đồ thời gian thực với vị trí các xe}
...
\thesisfigurecaption{Hình 4.26: Giao diện trang quản lý cảnh báo}
...
\thesistabletitle{Bảng 4.9: Các giai đoạn triển khai Frontend}
...
\thesistabletitle{Bảng 4.10: Ba trụ cột giám sát hệ thống}
...
\thesisfigurecaption{Hình 4.27: Grafana dashboard hiển thị tổng quan hiệu năng hệ
thống}
...
\thesisfigurecaption{Hình 4.28: EMQX Dashboard hiển thị trạng thái kết nối thiết bị}
...
\thesistabletitle{Bảng 4.11: Checklist hardening production theo mức ưu tiên}
...
\thesistabletitle{Bảng 4.11A: Tổng hợp công nghệ sử dụng trong hệ thống Cloud}
...
\thesistabletitle{Bảng 4.12: Thiết bị đo lường sử dụng trong thử nghiệm phần cứng}
...
\thesisfigurecaption{Hình 4.29: Bố trí thiết bị đo lường trong phòng thí nghiệm}

\pandocbounded{\safeincludesvg{./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-21.svg}}

\thesisfigurecaption{Hình 4.30: Thiết bị tracker được lắp đặt trên xe thử nghiệm}
...
\thesistabletitle{Bảng 4.13: Cấu hình môi trường thử nghiệm phần mềm}
...
\thesisfigurecaption{Hình 4.31: Sơ đồ môi trường thử nghiệm tổng thể}
...
\thesistabletitle{Bảng 4.14: Kết quả đo dòng tiêu thụ năng lượng theo chế độ hoạt
động}
...
mA & ~25 mA & Heartbeat mỗi 10--30 phút \\
...
\thesisfigurecaption{Hình 4.32: Đồ thị dòng tiêu thụ theo thời gian trong một chu kỳ
...
\thesistabletitle{Bảng 4.15: Thời lượng pin dự phòng theo kịch bản sử dụng (ước tính
theo baseline 18650)}
...
\thesisfigurecaption{Hình 4.33: Đồ thị điện áp pin dự phòng theo thời gian theo
...
\thesistabletitle{Bảng 4.16: Kết quả kiểm thử nhiệt độ hoạt động}
...
\thesisfigurecaption{Hình 4.34: Đồ thị dòng tiêu thụ theo nhiệt độ môi trường}
...
\thesistabletitle{Bảng 4.17: Kết quả kiểm thử tương thích OBD2 trên các thương hiệu
xe}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}{|p{0.04\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.10\linewidth}|p{0.18\linewidth}|}
...
\thesistabletitle{Bảng 4.18: Kết quả kiểm thử mạch LVD}
...
\thesistabletitle{Bảng 4.19: Kết quả đo thời gian kết nối BLE OBD2}
...
\thesisfigurecaption{Hình 4.35: Biểu đồ phân bố thời gian kết nối BLE OBD2 (20 lần đo)}
...
\thesistabletitle{Bảng 4.20: Thời gian phản hồi OBD2 PID}
...
\thesistabletitle{Bảng 4.21: Thời gian bắt vệ tinh GPS (TTFF)}
...
~1 giây & 0.5 giây & 3 giây & 10--12 \\
\end{longtable}
}

\pandocbounded{\safeincludesvg{./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-27.svg}}

\thesisfigurecaption{Hình 4.36: Đồ thị thời gian bắt vệ tinh GPS trong các kịch bản
(Cold/Warm/Hot Start)}
...
\thesistabletitle{Bảng 4.22: Độ chính xác vị trí GPS}
...
\thesistabletitle{Bảng 4.23: Độ trễ truyền dữ liệu MQTT qua mạng 4G}
...
\thesisfigurecaption{Hình 4.37: Biểu đồ phân bố độ trễ MQTT trong điều kiện mạng 4G ổn
định}
...
\thesistabletitle{Bảng 4.24: Kết quả kiểm thử máy trạng thái firmware}
...
\thesistabletitle{Bảng 4.25: Kết quả kiểm thử reconnect MQTT}
...
\thesistabletitle{Bảng 4.25A: Đối chiếu luồng OTA theo mã nguồn triển khai}
...
\thesistabletitle{Bảng 4.26: Kết quả đo hiệu suất API Backend}
...
\hline
\endlastfoot
1 & GET /api/v1/vehicles & GET & ~45 ms & 38 ms & 95 ms &
150 ms & ~850 \\
2 & GET /api/v1/vehicles/:id & GET & ~35 ms & 28 ms & 72
ms & 120 ms & ~1100 \\
3 & POST /api/v1/vehicles & POST & ~65 ms & 55 ms & 130 ms
& 200 ms & ~600 \\
4 & GET /api/v1/devices/:id/telemetry & GET & ~55 ms & 45
ms & 110 ms & 180 ms & ~750 \\
5 & GET /api/v1/trips & GET & ~80 ms & 65 ms & 160 ms &
250 ms & ~500 \\
6 & GET /api/v1/alerts & GET & ~50 ms & 42 ms & 100 ms &
165 ms & ~800 \\
7 & POST /api/v1/auth/login & POST & ~120 ms & 100 ms &
200 ms & 350 ms & ~350 \\
8 & GET /api/v1/geofences & GET & ~40 ms & 32 ms & 85 ms &
140 ms & ~900 \\
\end{longtable}
}
...
\thesisfigurecaption{Hình 4.38: Biểu đồ thời gian phản hồi API (P50, P95, P99) cho các
endpoint chính}
...
\thesistabletitle{Bảng 4.27: Độ trễ end-to-end (Device -\textgreater{} Cloud -\textgreater{} Browser)}
...
\thesistabletitle{Bảng 4.28: Thông lượng ghi cơ sở dữ liệu}
...
\thesistabletitle{Bảng 4.29: Kết quả kiểm thử tải đồng thời}
...
\thesisfigurecaption{Hình 4.40: Đồ thị hiệu suất hệ thống theo số lượng thiết bị đồng
thời}
...
\thesistabletitle{Bảng 4.30: Kết quả đánh giá hiệu suất Frontend (Lighthouse)}
...
5 & Cumulative Layout Shift (CLS) & ~0.05 (đo trên bản
...
\thesisfigurecaption{Hình 4.41: Kết quả Lighthouse Performance Audit của trang
Dashboard}
...
\thesistabletitle{Bảng 4.31: Kết quả kiểm thử luồng dữ liệu chính}
...
\thesistabletitle{Bảng 4.32: Kết quả kiểm thử cảnh báo Geofence}
...
\thesistabletitle{Bảng 4.33: Kết quả kiểm thử lệnh điều khiển từ xa}
...
~15 giây (bao gồm thời gian restart) \\
4 & alert\_mode & Bật cảnh báo & Thành công &
~2s \\
\end{longtable}
}
...
\thesisfigurecaption{Hình 4.45: Screenshot giao diện gửi lệnh điều khiển từ Dashboard
...
\thesistabletitle{Bảng 4.34: Kết quả kiểm thử mất kết nối và phục hồi}
...
\thesisfigurecaption{Hình 4.46: Screenshot hành trình trên bản đồ, thể hiện giai đoạn
...
\thesistabletitle{Bảng 4.35: Tổng hợp kết quả đo lường phần cứng so với chỉ tiêu thiết
kế}
...
µA (~0.5 mA) & Đạt & IMU + RTC hoạt động \\
...
~2 giây & Đạt & Bao gồm init cơ bản \\
...
\thesistabletitle{Bảng 4.36: Tổng hợp kết quả đo lường firmware so với chỉ tiêu thiết
kế}
...
\thesistabletitle{Bảng 4.37: Tổng hợp kết quả đo lường Cloud/Backend so với chỉ tiêu
thiết kế}
...
~12,000 điểm/giây & Đạt & 12x mục tiêu \\
...
\thesistabletitle{Bảng 4.38: Tổng hợp kết quả đo lường Frontend so với chỉ tiêu thiết
kế}
...
~1--2 giây & Đạt & WebSocket + Leaflet \\
...
\thesistabletitle{Bảng 4.39: Bảng tổng hợp tổng thể --- So sánh kết quả với chỉ tiêu
thiết kế}
...
\thesisfigurecaption{Hình 4.47: Biểu đồ radar so sánh chỉ tiêu thiết kế và kết quả đạt
được (spider chart)}
...
\thesistabletitle{Bảng 4.40: Chỉ số ổn định vận hành thực tế}
...
\thesistabletitle{Bảng 5.1: Đánh giá hiệu năng quản lý năng lượng}
...
(~0.5 mA) trên thiết bị hiện tại & Đạt yêu cầu \\
...
\thesistabletitle{Bảng 5.2: Đánh giá hiệu năng firmware}
...
\thesistabletitle{Bảng 5.3: Đánh giá hiệu năng tổng hợp hệ thống đám mây}
...
\thesistabletitle{Bảng 5.4: Chi phí BOM thiết bị tracker}
...
\thesistabletitle{Bảng 5.5: So sánh chi phí với giải pháp thương mại}
...
(SIM 4G) & 1.710.000--2.470.000 VND & Cao (mã nguồn mở) \\
GPS Tracker đơn giản (Việt Nam) & 500.000--1.500.000 VND &
50.000--100.000 VND & 1.100.000--2.700.000 VND & Thấp \\
...
100.000--300.000 VND & 3.200.000--7.600.000 VND & Trung bình \\
\end{longtable}
}
...
\thesistabletitle{Bảng 5.6: Chi phí hạ tầng đám mây theo quy mô}
...
100--500 xe & Kubernetes cluster (3 node) & ~2.500.000 VND
...
\thesistabletitle{Bảng 5.7: Thang đo xác suất và tác động}
...
\thesistabletitle{Bảng 5.8: Ma trận đánh giá rủi ro và biện pháp giảm thiểu}
...
\thesistabletitle{Bảng 5.9: Tổng hợp mức độ rủi ro theo phân loại}
...
\thesistabletitle{Bảng 5.10: Lộ trình phát triển khuyến nghị}
...
\chapterheading{CHƯƠNG 6. PHẢN HỒI VÀ BÀI HỌC KINH NGHIỆM -- REFLECTION AND CASE STUDIES}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 6.3.1A: Tổng hợp thông tin về STT, Biện pháp, Mô tả chi tiết}
...
\subsection{6.4. Tổng kết và bài học kinh nghiệm -- General reflection and case studies}\label{64-tux1ed5ng-kux1ebft-vuxe0-buxe0i-hux1ecdc-kinh-nghiux1ec7m--general-reflection-and-case-studies}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 6.4.2A: Tổng hợp thông tin về Tầng, Ngôn ngữ / Công nghệ, Thách thức chính}
...
\thesistabletitle{Bảng PL-1.1: Bảng kê chi phí linh kiện (BOM)}
...
-\textgreater{} ~4V cấp modem SIM7600CE-T \\
...
\thesistabletitle{Bảng PL-1.2: Chi phí hạ tầng cloud}
...
\thesistabletitle{Bảng PL-1.3: Tổng hợp chi phí dự án}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.1A: Tổng hợp thông tin về Chuẩn cụ thể, Nội dung, Áp dụng}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.1B: Tổng hợp thông tin về PID (hex), Mô tả, Đơn vị}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.2A: Tổng hợp thông tin về Tính năng, Mô tả, Áp dụng}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.3A: Tổng hợp thông tin về Nguyên tắc, Hiện thực trong dự án}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 2.4A: Tổng hợp thông tin về Nguyên tắc, Mô tả, Ví dụ trong dự án}
...
\thesistabletitle{Bảng PL-3.1: Phân chia giai đoạn và thời gian thực hiện}
...
\thesisfigurecaption{Hình PL-3.1: Biểu đồ Gantt kế hoạch thực hiện dự án (24 tuần)}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3A: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3B: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3C: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3D: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3E: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 3.3F: Tổng hợp thông tin về Tuần, Công việc, Sản phẩm}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.2A: Tổng hợp thông tin về STT, Tên bảng, Mô tả}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.2B: Tổng hợp thông tin về Metric name mẫu, Labels, Mô tả}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.3A: Tổng hợp thông tin về Nhóm, Method, Endpoint}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.4A: Tổng hợp thông tin về Biến, Dịch vụ, Mô tả}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.5A: Tổng hợp thông tin về Yêu cầu, Phiên bản tối thiểu}
...
{\def\LTcaptype{none} % do not increment counter
\thesistabletitle{Bảng 4.5B: Tổng hợp thông tin về Dịch vụ, URL, Tài khoản mặc định}
