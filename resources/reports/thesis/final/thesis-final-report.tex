...
\item
  ...
  \texttt{util.c}).
\item
  \texttt{sdkconfig}, \texttt{CMakeLists.txt}: cấu hình build/runtime của
  dự án ESP-IDF.
\end{itemize}
...
hình: profile 12V dùng \(IGN_{\mathrm{ON}} \ge 13.0\,\mathrm{V}\) và
\(IGN_{\mathrm{OFF}} \le 12.0\,\mathrm{V}\); profile 24V dùng
\(IGN_{\mathrm{ON}} \ge 26.0\,\mathrm{V}\) và
\(IGN_{\mathrm{OFF}} \le 24.0\,\mathrm{V}\). Cách này kém chính xác hơn OBD2 nhưng vẫn bảo
...
\tightlist
\item
  \texttt{Tracking\_Backend/}: API server Express + TypeScript.
\item
  \texttt{Tracking\_Frontend/}: dashboard web Next.js.
\item
  \texttt{Tracking\_MqttBridge/}: dịch vụ ingest MQTT độc lập.
\item
...
\item
  ...
  \texttt{Tracking\_Grafana/}: khối quan trắc metrics/logs/dashboard.
\item
  \texttt{Tracking\_NPM/}: reverse proxy và quản trị SSL.
\item
  \texttt{Tracking\_Data/}: dữ liệu runtime (gitignored), gồm
...
\item
  MQTT Bridge subscribe, xác thực payload (Zod) và định tuyến dữ liệu.
\item
  Dữ liệu telemetry được ghi vào VictoriaMetrics; sự kiện/lỗi được ghi vào
  VictoriaLogs; trạng thái phiên được cập nhật vào PostgreSQL.
\item
  MQTT Bridge publish \texttt{internal/events/\#} để Backend listener nhận
  và phát Socket.IO cập nhật dashboard theo thời gian thực.
\end{enumerate}
...
\item
  ...
  và VictoriaLogs.
\item
  \texttt{src/mqtt/}: client MQTT và cấu hình subscription QoS.
\item
...
\item
  ...
  hóa schema dữ liệu, kiểu dữ liệu và tiện ích hỗ trợ.
\end{itemize}
...
\tightlist
\item
  \textbf{Runtime/Framework:} Node.js 20+ (LTS), Express 4.x, TypeScript
  5.x.
\item
  \textbf{Validation:} Zod (kết hợp runtime validation và type inference).
\item
  \textbf{Database:} PostgreSQL 16 (driver \texttt{pg}) cho dữ liệu quan
  hệ; VictoriaMetrics (PromQL/MetricsQL) cho chuỗi thời gian.
\item
  \textbf{Real-time:} Socket.IO 4.8 và MQTT.js tích hợp EMQX.
\item
  \textbf{Security/Auth:} Session-based authentication (database-backed token
  hash SHA-256), Helmet, CORS, Rate Limiting.
\item
  \textbf{API Docs:} swagger-ui-express + OpenAPI.
\end{itemize}
...
\tightlist
\item
  \texttt{index.ts}: điểm khởi động của Backend.
\item
...
\item
  \texttt{infrastructure/}: tích hợp cơ sở dữ liệu, logger, metrics và
  realtime.
\item
  \texttt{middleware/}: auth, error handling, rate limit, request-id,
  metrics, sentry.
\item
...
\tightlist
\item
  \texttt{app/}: App Router và các route dashboard bảo vệ.
\item
  \texttt{components/}: component tái sử dụng cho UI, layout, forms, map,
  charts và provider.
\item
  \texttt{features/}: module nghiệp vụ theo FSD
  (\texttt{vehicles}, \texttt{customers}, \texttt{trips},
  \texttt{alerts}, \texttt{map}).
\item
  \texttt{lib/}: API clients, Socket.IO realtime, Zustand store, constants,
  utils và shared hooks.
\end{itemize}
...
2 & Điện áp đóng lại (reconnect) & Profile 12V: \(12.2\,\mathrm{V}\); Profile 24V:
...
phòng được chuẩn hóa theo hai profile:
...
\item
  ...
  cứng kết hợp ADC firmware, dùng ngưỡng profile:
...
\tightlist
\item
  \texttt{iot-vehicle-tracking-system-cloud/}: chứa toàn bộ dịch vụ runtime
...
\item
  \texttt{README.md}: hướng dẫn triển khai và vận hành tổng quan.
\end{itemize}
