...
\textbf{ESP32-S3} được chọn làm vi điều khiển chính cho thiết bị theo dõi
vì đây là phương án cân bằng tốt giữa năng lực xử lý, mức độ tích hợp và
chi phí triển khai. Quyết định này dựa trên các điểm chốt sau:
...
2--3 lần), hệ thống tự động chuyển sang cơ chế dự phòng là đo điện áp
ắc quy qua ADC để phát hiện trạng thái IGN. Phương pháp
...
Firmware là lớp điều phối hành vi của thiết bị. Việc vị trí có được cập
nhật đều, cảnh báo có đến kịp thời hay không, và ắc quy xe có bị ảnh
hưởng hay không đều phụ thuộc vào lớp này. Do nằm ở điểm giao nhau giữa
phần cứng, truyền thông và yêu cầu nghiệp vụ, firmware không chỉ cần bảo
đảm thiết bị hoạt động được, mà còn phải giúp thiết bị vận hành đúng,
ổn định và tiết kiệm năng lượng trong môi trường thực tế.
...
\item
  \textbf{Độ bền vận hành cao}: có cơ chế thử lại, chuyển sang dự phòng và phục hồi
...
\item
  ...
  nối được OBD2, hệ thống chuyển sang đo điện áp ắc quy qua ADC theo cơ
...
Module này cho phép thiết bị theo dõi truy xuất trạng thái vận hành của
xe thông qua cổng OBD2 mà không cần dây nối trực tiếp tới vi điều khiển.
Adapter vgate iCar Pro được cắm vào cổng chẩn đoán OBD-II của xe, sau đó
 giao tiếp không dây với thiết bị theo dõi bằng BLE. Trong firmware,
NimBLE đảm nhiệm lớp Bluetooth, còn GATT là cơ chế để hai thiết bị tổ
chức dịch vụ và trường dữ liệu cho việc đọc và ghi thông tin.
...
\subparagraph{d) Xử lý lỗi và cơ chế dự phòng}\label{d-xux1eed-luxfd-lux1ed7i-vuxe0-cux1a1-chux1ebf-dux1ef1-phuxf2ng-fallback}
...
\endfoot
\endlastfoot
Không kết nối được adapter & Thử lại 2--3 lần với độ trễ 2 giây, sau đó
chuyển sang cơ chế dự phòng & 10 giây \\\hline
Mất kết nối giữa chừng & Kết nối lại 2--3 lần, nếu thất bại thì chuyển sang cơ chế dự phòng &
5 giây mỗi lần \\\hline
Adapter không phản hồi lệnh & Thử lại 1--2 lần, nếu thất bại thì chuyển sang cơ chế dự phòng
& 5 giây \\\hline
\end{longtable}
}


\textbf{Cơ chế dự phòng}: Khi không thể giao tiếp với OBD2
...
liệu'' mà là ``chuyển bản tin thành thông tin có thể khai thác''. Ở tầng
máy chủ và xử lý dữ liệu, một bản tin chỉ thật sự có giá trị khi được
 tiếp nhận ổn định, lưu đúng nơi, cập nhật kịp thời tới giao diện quản
trị và vẫn truy vết được khi có sự cố. Vì vậy, kiến trúc máy chủ phải
đồng thời giải quyết các bài toán sau:
...
trực tiếp vào API máy chủ và mọi dữ liệu dồn vào cùng một nơi lưu &
Triển khai ban đầu đơn giản & Không tối ưu cho dữ liệu phát sinh với tần
suất cao, khó mở rộng cập nhật thời gian thực, tải dễ dồn lên một dịch
vụ & Trung bình \\\hline
...
Đồ án chọn \textbf{PA-C: kiến trúc phân tầng kết hợp cơ chế xử lý theo sự kiện};
khi có bản tin mới, các lớp liên quan sẽ xử lý ngay, với chuỗi xử lý:

\texttt{Device\ -\textgreater{}\ EMQX\ -\textgreater{}\ MQTT\ Bridge\ -\textgreater{}\ PostgreSQL/}\\
\texttt{VictoriaMetrics/VictoriaLogs\ -\textgreater{}\ API\ máy\ chủ\ -\textgreater{}\ giao\ diện\ web}
...
và phân luồng, lớp lưu trữ ghi theo đúng đặc tính dữ liệu, còn lớp ứng
dụng phục vụ người dùng và nghiệp vụ. Nói cách khác, kiến trúc được
chọn vì phản ánh đúng luồng xử lý dữ liệu trong hệ thống thay vì dồn mọi
trách nhiệm về một dịch vụ duy nhất.
...
\tightlist
\item
  \textbf{Khớp bản chất dữ liệu IoT}: MQTT xử lý tốt mô hình
  publish/subscribe, tức mô hình ``một bên phát bản tin vào kênh chung,
  nhiều bên có thể nhận'', nên giảm mức phụ thuộc trực tiếp giữa thiết
  bị và tầng ứng dụng. Điều này giúp thiết bị chỉ tập trung gửi dữ liệu,
  thay vì phải biết chi tiết ai sẽ nhận và xử lý dữ liệu đó ở phía sau.
\item
  \textbf{Tối ưu hiệu năng theo vai trò}: Bridge xử lý luồng dữ liệu đi
  vào đầu tiên, API máy chủ tập trung vào nghiệp vụ, còn cơ sở dữ
...
khai cụ thể từng lớp của hệ thống máy chủ và dịch vụ xử lý. Cách trình
bày được sắp xếp theo luồng dữ liệu, từ lúc bản tin đi vào broker cho
đến khi thông tin xuất hiện trên giao diện của người vận hành.
...
\item
  ...
  tiếp đến người dùng thông qua WebSocket, là kênh cho phép máy chủ chủ
  động gửi thông tin mới lên trình duyệt.
\end{itemize}
...
Nếu mô tả theo luồng xử lý dữ liệu, hệ thống vận hành theo trình tự sau:
...
\item
  ...
  \texttt{v1/\{device\_id\}/rawdata}. Ở đây, EMQX Broker đóng vai trò là
  broker MQTT, nơi tiếp nhận và chuyển tiếp bản tin.
\item
...
\item
  ...
  đầu vào bằng lược đồ kiểm tra dữ liệu của Zod, sau đó
  ghi đồng thời vào nhiều nơi, nghĩa là một lần nhận dữ liệu nhưng lưu
...
\end{itemize}
\item
  \textbf{API máy chủ} (Express.js) là lớp để giao diện gọi vào khi cần
...
Cách tổ chức này giúp cấu trúc vận hành của đồ án bám sát cách các hệ
thống thật được chia dịch vụ. Hiểu đơn giản, mỗi dịch vụ được đặt trong
một ``hộp chạy riêng'' gọi là container; các hộp này có thể nói chuyện
với nhau qua mạng dùng chung, nhưng vẫn tách lỗi và tài nguyên tương đối
rõ ràng. Nhờ vậy, cấu trúc vận hành của đồ án bám sát cách các hệ thống
thật được chia dịch vụ trong thực tế:
...
\item
  ...
  biệt, được giới hạn CPU và RAM cụ thể (ví dụ: API máy chủ
...
\textit{Lưu ý:} Các số liệu về số kết nối đồng thời, thông lượng bản tin
và mức sử dụng tài nguyên trong bảng trên là giá trị tham chiếu tổng hợp
 từ tài liệu công bố và benchmark của nhà cung cấp, được dùng để so sánh
 tương đối giữa các broker chứ không phải số đo nội bộ của đồ án.
...
do MQTT Bridge và API máy chủ đảm nhiệm.
...
\item
  ...
  kiện cho API máy chủ.
\item
  API máy chủ ghi dữ liệu vào PostgreSQL và phát Socket.IO để cập
...
PostgreSQL 16 lưu trữ toàn bộ dữ liệu nghiệp vụ của hệ thống. Lược đồ
...
\textbf{Hiệu năng} & Tốt & Theo benchmark công bố, hiệu năng ghi có thể cao hơn đáng kể \\\hline
\textbf{Sử dụng RAM} & Cao & Thấp \\\hline
\textbf{Nén dữ liệu} & Tốt & Theo benchmark công bố, hiệu quả nén dữ liệu có thể tốt hơn \\\hline
...
khiển. Vì vậy, phần tiếp theo chuyển sang API máy chủ.

\paragraph{3.3.2.4. API máy chủ và kiến trúc phần mềm (API Server \&
Software
...
API máy chủ là lớp làm việc trực tiếp với người vận hành và các quy tắc
nghiệp vụ. Nếu MQTT Bridge chuyên nhận dòng dữ liệu thô từ thiết bị, thì
API máy chủ đảm nhiệm các chức năng nghiệp vụ mà người dùng sử dụng hằng ngày:
đăng nhập, quản lý xe, xem lịch sử, tạo cảnh báo, quản lý vùng cho phép
và điều phối OTA. Vì vai trò khác nhau, tiêu chí chọn framework ở đây cũng khác:
...
\caption{So sánh các khung phát triển cho API máy chủ}\label{tab:ch3-so-sanh-framework-backend}\\
\hline \textbf{Tiêu chí} & \textbf{Express + TypeScript} & \textbf{NestJS} & \textbf{Go + Gin} \\
\\\hline
\endfirsthead
\caption[]{So sánh các khung phát triển cho API máy chủ (tiếp theo)}\\
\hline \textbf{Tiêu chí} & \textbf{Express + TypeScript} & \textbf{NestJS} & \textbf{Go + Gin} \\
\\\hline
\endhead
\\\hline
...
API máy chủ được tổ chức theo kiến trúc Domain-Driven Design với ba
...
đồng giao tiếp'' giữa giao diện quản trị, API máy chủ và một số
...
\caption{Luồng điều phối OTA giữa API máy chủ, EMQX, thiết bị và MQTT Bridge}
\label{fig:ch3-luong-ieu-phoi-ota-giua-backend-emqx-thiet-bi-va-mqtt-bridge}
\end{figure}
}


API máy chủ vẫn duy trì một số alias tương thích như \texttt{/api/v1/device}
hoặc \texttt{/api/v1/export}, nhưng trong tài liệu, các endpoint được ưu
 tiên mô tả theo dạng tài nguyên số nhiều (\texttt{/devices},
\texttt{/vehicles}, \texttt{/customers}, \texttt{/trips},
\texttt{/alerts}, \texttt{/geofences}) để thống nhất
...
phân công firmware. Riêng luồng OTA được tách khá rõ: API máy chủ phát các
...
Token chỉ đóng vai trò khóa tham chiếu; API máy chủ mới là nơi lưu
...
Nếu tầng máy chủ là nơi xử lý dữ liệu, thì giao diện web là lớp trình bày
trực tiếp thông tin vận hành cho người sử dụng. Một màn hình tổng quan
hiệu quả không chỉ hiển thị đầy đủ thông tin, mà còn phải giúp người
vận hành nhận ra những trạng thái quan trọng, các dấu hiệu bất thường và
vị trí cần can thiệp. Vì vậy, thiết kế giao diện web cần đồng thời đáp
ứng các yêu cầu sau:
...
Đồ án chọn \textbf{PA-FE3: Next.js 15 + React 19} làm nền tảng giao diện
web chính vì bộ công cụ này phù hợp với yêu cầu của một giao diện tổng
quan cần hiển thị nhanh và phản hồi kịp thời khi dữ liệu và cảnh báo thay
đổi liên tục.
...
giao diện. Bộ công cụ này cung cấp các thành phần cơ bản có tính nhất
quán, hỗ trợ tốt cho thao tác bằng bàn phím và quản lý điểm lấy nét,
đồng thời vẫn cho phép nhóm chủ động kiểm soát cách triển khai từng
...
Ứng dụng giao diện web được tổ chức theo hướng chia mã nguồn theo từng
nhóm tính năng nghiệp vụ, thường gọi là Feature-Sliced Design (FSD). Mỗi
tính năng được đóng gói thành một module tương đối độc lập gồm thành
phần giao diện, hook, kiểu dữ liệu và tiện ích liên quan. Cách tổ chức
này giúp cấu trúc mã nguồn bám sát nghiệp vụ hơn, nhờ đó thuận lợi cho
việc bảo trì và mở rộng tính năng.
...
\item
  ...
  lấy từ API máy chủ qua HTTP. TanStack Query giúp lưu tạm và làm
...
\caption{Luồng dữ liệu một chiều giữa Next.js, Query, Zustand và API máy chủ}
...
\tightlist
\item
  \textbf{Đăng nhập:} Người dùng nhập email và mật khẩu, API máy chủ tạo bản
...
\item
  ...
  đồng thời chuyển tiếp đến API máy chủ.
\item
  \textbf{API máy chủ} phát sự kiện \texttt{vehicle:\{id\}:location} qua
  Socket.IO đến tất cả trình duyệt đang đăng ký nhận dữ liệu.
\item
  \textbf{Giao diện web} nhận sự kiện và cập nhật tọa độ điểm đánh dấu
  trên bản đồ. Kỹ thuật nội suy (interpolation) được dùng để điểm đánh
  dấu di chuyển mượt hơn giữa hai điểm tọa độ liên tiếp.
\end{enumerate}
...
API máy chủ & Express.js + TypeScript & Nhẹ, linh hoạt, dễ tổ chức theo
...
\item
  ...
  hợp như PostgreSQL và VictoriaMetrics, sau đó API máy chủ xử lý
...
Trên nguyên mẫu đã chế tạo, ESP32-S3 giữ vai trò bộ điều phối trung tâm
và liên kết trực tiếp với các khối phần cứng còn lại. Cụ thể, vi điều
khiển giao tiếp với modem qua UART, làm việc với adapter OBD2 qua BLE,
đọc cảm biến rung qua I2C và đo điện áp ắc quy qua ADC. Phần này tập
trung vào cách các khối đã được bố trí và kết nối trong thiết bị thực tế,
không lặp lại phần phân tích lý do lựa chọn đã trình bày ở Chương 3.
...
Trên bản triển khai hiện tại, modem được nối với ESP32-S3 qua UART1 để
xử lý cả luồng LTE và GNSS bằng AT command. Cách nối này bám đúng pin
mapping và kiến trúc đã chốt ở Chương 3.

\textbf{Giao tiếp BLE với OBD2 adapter:} ESP32-S3 dùng BLE 5.0 tích hợp
để kết nối với adapter vgate iCar Pro (BLE 4.0), từ đó đọc các thông số
OBD-II như trạng thái IGN, RPM, tốc độ xe, mức nhiên liệu và mã lỗi chẩn
đoán trong quá trình thử nghiệm.

\textbf{Giao tiếp I2C với cảm biến LIS3DSH:} Cảm biến gia tốc 3 trục
LIS3DSH được nối qua bus I2C (GPIO2 SDA, GPIO1 SCL). Trong bản phần cứng
hiện tại, cảm biến này chịu trách nhiệm phát hiện chuyển động khi xe đỗ
và tạo tín hiệu đánh thức cho ESP32-S3 theo cấu hình firmware đang dùng.
...
cơ chế dự phòng}\label{f-xux1eed-luxfd-lux1ed7i-vuxe0-fallback}
...
\textit{Lưu ý:} Bảng dưới đây mô tả hợp đồng topic đang được triển khai ở
firmware và hệ thống máy chủ; đây không phải bảng chỉ số hiệu năng.
...
\caption{Cấu trúc topic MQTT trong firmware và hệ thống máy chủ}\label{tab:ch4-bang-bo-sung-chi-muc-4-2-3-trien-khai-firmware}\\
\hline \textbf{Topic} & \textbf{Hướng truyền} & \textbf{Mục đích} \\
\\\hline
\endfirsthead
\caption[]{Cấu trúc topic MQTT trong firmware và hệ thống máy chủ (tiếp theo)}\\
\hline \textbf{Topic} & \textbf{Hướng truyền} & \textbf{Mục đích} \\
\\\hline
\endhead
\\\hline
...
Tính năng OTA mới được tích hợp xuyên suốt từ API máy chủ đến firmware.
API máy chủ tạo \texttt{jobId}, tra cứu firmware theo
...
Hệ thống máy chủ là nơi dữ liệu từ thiết bị được tiếp nhận, chuẩn hóa,
lưu trữ và đưa tới giao diện quản trị. Trên cơ sở phương án đã chọn ở
Chương 3, phần này tập trung vào cách tầng máy chủ đã được triển khai
thực tế theo từng khối: hạ tầng Docker, MQTT Bridge, API quản trị, giao
diện web và lớp giám sát vận hành.

Trong bản triển khai hiện tại, luồng dữ liệu chính đi qua EMQX, MQTT
Bridge, VictoriaMetrics, VictoriaLogs, PostgreSQL, API máy chủ và giao
 diện quản trị. Trọng tâm của mục này là mô tả cấu hình và cách các thành
phần đang vận hành, không lặp lại phần phân tích lựa chọn kiến trúc đã
...
Ở giai đoạn triển khai, mỗi dịch vụ được tách thành một cấu hình Docker
Compose riêng. Cách tổ chức này phản ánh trực tiếp mô hình phân tách đã
được đề xuất ở Chương 3, đồng thời tạo thuận lợi cho việc khởi động,
cập nhật và kiểm thử từng thành phần trong môi trường thực tế. Việc mỗi
dịch vụ chạy trong một container độc lập cũng giúp hệ thống dễ quản lý
hơn khi cần điều chỉnh từng khối riêng lẻ. Cách tổ chức này có một số
lợi điểm rõ ràng:
...
\tightlist
\item
  \texttt{Tracking\_Backend/}: API máy chủ viết bằng Express +
  TypeScript.
\item
...
API máy chủ & tracking-backend & 4000 & REST API và kênh WebSocket \\\hline
...
\nolinkurl{api.thingdock.dev} & 443 & tracking-backend & 4000 & API máy chủ, Swagger và Socket.IO đi qua lớp chuyển tiếp Nginx Proxy Manager \\\hline
...
\item
  Cuối cùng, MQTT Bridge phát sự kiện nội bộ để API máy chủ nhận và đẩy cập
...
\paragraph{4.2.4.3. Triển khai API máy chủ}\label{4243-triux1ec3n-khai-backend-api-backend-api-implementation}

\subparagraph{a) Lựa chọn công
nghệ}\label{a-lux1ef1a-chux1ecdn-cuxf4ng-nghux1ec7-1}

API máy chủ được xây dựng trên nền tảng Node.js với Express.js và
...
Bridge gánh riêng, API máy chủ không phải ôm cả dòng dữ liệu kỹ
...
\caption{So sánh các khung phát triển API máy chủ cho hệ thống IoT}\label{tab:ch4-so-sanh-framework-backend-cho-he-thong-iot}\\
\hline \textbf{Tiêu chí} & \textbf{Express + TypeScript} & \textbf{NestJS} & \textbf{Go + Gin} \\
\\\hline
\endfirsthead
\caption[]{So sánh các khung phát triển API máy chủ cho hệ thống IoT (tiếp theo)}\\
\hline \textbf{Tiêu chí} & \textbf{Express + TypeScript} & \textbf{NestJS} & \textbf{Go + Gin} \\
\\\hline
\endhead
\\\hline
...
\item
  ...
  từ đầu vào. Đây là lớp bảo vệ quan trọng vì API máy chủ nhận dữ
...
API máy chủ được tổ chức theo kiến trúc Domain-Driven Design với
...
\tightlist
\item
  \texttt{index.ts}: điểm khởi động của API máy chủ.
\item
...
\caption{Cấu trúc thư mục API máy chủ theo kiến trúc DDD}
...
Firmware & \nolinkurl{/api/v1/firmware} & Có (admin) & Quản lý firmware
OTA \\\hline
\end{longtable}
}


Riêng nhóm dữ liệu IoT (\texttt{/api/v1/iot}) không đi theo luồng đăng nhập
người dùng thông thường. Các thiết bị gửi dữ liệu trực tiếp bằng MQTT
hoặc qua cơ chế dự phòng HTTP, nên lớp bảo vệ chính nằm ở cơ chế nhận
diện thiết bị và ACL của EMQX thay vì ở phiên đăng nhập web.
...
tục, API máy chủ chỉ phát sự kiện đúng vào nhóm chức năng liên
...
OTA từ API máy chủ; tiến trình thực thi chi tiết vẫn được theo dõi chủ yếu
...
middleware, cho phép đối chiếu lỗi giữa log của giao diện web và log của API máy chủ.
...
Giao diện web quản trị được xây dựng trên nền tảng Next.js 15 với React
19. Khác với Chương 3, phần này không phân tích lại lý do lựa chọn công
nghệ, mà tập trung vào cấu trúc triển khai hiện tại của giao diện và cách
các thành phần đang phối hợp với API máy chủ.

Trong bản triển khai hiện nay, giao diện web có thể chia thành ba nhóm
vai trò chính: khung ứng dụng và điều hướng trang, lớp quản lý trạng thái
và đồng bộ dữ liệu với máy chủ, cùng lớp hiển thị trực tiếp cho người
vận hành như bản đồ, biểu đồ và cập nhật thời gian thực.
...
Giao diện web được tổ chức theo kiến trúc chia mã nguồn theo từng nhóm
 tính năng. Trong mỗi khu vực, các thành phần UI, hook, kiểu dữ liệu và
 tiện ích liên quan được đặt gần nhau. Phần này ghi nhận cấu trúc mã
 nguồn đang được sử dụng trong bản triển khai hiện tại, thay vì lặp lại
 phần lập luận lựa chọn kiến trúc đã nêu ở Chương 3.
...
\caption{Giao diện trang tổng quan hệ thống trong bản triển khai thực tế}
...
\item
  \textbf{Socket.IO Client}: Nhận các cập nhật thời gian thực do API máy
  chủ phát ra. Giao diện web không kết nối trực tiếp tới MQTT broker;
  mọi dữ liệu thời gian thực đều được API máy chủ chuẩn hóa trước khi
  đẩy ra giao diện.
\end{itemize}
...
nối log giữa giao diện web, API máy chủ và các dịch vụ liên quan.
...
\item
  ...
  API máy chủ và hạ tầng đang hoạt động ra sao, ví dụ số yêu cầu HTTP, độ
...
dịch vụ trong hệ thống. API máy chủ sử dụng \texttt{victorialogs-transport}
...
\item
  \textbf{Service}: Tên dịch vụ phát sinh log (API máy chủ, mqtt-bridge,
  giao diện web)
\item
...
API máy chủ tích hợp Sentry thông qua hai middleware:
...
Mỗi yêu cầu HTTP đến API máy chủ đều được gán một mã truy vết duy
...
API máy chủ, đến cơ sở dữ liệu và MQTT Bridge, giúp việc dò lỗi
...
các hệ lưu trữ, đồng thời API máy chủ và giao diện web đáp ứng yêu
...
MQTT \\\hline
API máy chủ & Express + TypeScript & 4.x & Cung cấp API, đẩy dữ
...
thống (phần cứng, firmware, máy chủ), kết quả kiểm thử tích hợp toàn tuyến
và đối chiếu với các tiêu chí thiết kế ở Chương 1 và Chương 2. Nói cách
khác, đây là phần kiểm chứng xem hệ thống sau khi được triển khai có
thực sự đáp ứng được những gì đã đề ra ở các chương trước hay không.
Toàn bộ giá trị được ghi nhận trên thiết bị phần cứng đã chế tạo và lắp
đặt thực tế trong điều kiện vận hành bám sát bối cảnh khai thác.
...
\item
  ...
  phần cứng, firmware, API máy chủ và các thành phần giao diện web.
\item
...
\item
  ...
  liệu toàn tuyến, từ thiết bị đến giao diện người dùng.
\end{enumerate}
...
& ~2 mA (ước tính bước đầu, cần đo lặp lại) & Chỉ IMU + RTC hoạt
động \\\hline
...
trong đó \(\eta\) là hiệu suất chuyển đổi của mạch boost converter
(~85--90\%). Các giá trị dưới đây là \textbf{giá trị quy đổi theo bộ pin
tham chiếu 18650}, không phải số đo thực nghiệm trực tiếp trên cell dùng
trong thử nghiệm hiện tại.
...
\hline \textbf{STT} & \textbf{Kịch bản sử dụng} & \textbf{Dòng trung bình} & \textbf{Thời lượng ước tính} & \textbf{Nguồn giá
trị} & \textbf{Ghi chú} \\
\\\hline
\endfirsthead
...
~2.6--3.0 giờ & Quy đổi thiết kế & Trường hợp xấu nhất, phụ thuộc hiệu
suất boost \\\hline
2 & Sleep + heartbeat 30 phút & ~15 mA & ~64
giờ & Quy đổi thiết kế & Xe đậu bình thường \\\hline
3 & Ngủ sâu, chỉ chờ IMU đánh thức & ~0.5 mA &
~1,940 giờ (~81 ngày) & Quy đổi thiết kế & Chế độ tiết kiệm tối đa \\\hline
4 & Hỗn hợp (50\% xe chạy + 50\% xe đỗ) & ~182 mA &
~5.0 giờ & Quy đổi thiết kế & Mô phỏng sử dụng thực tế \\\hline
\end{longtable}
}
...
\textbf{Nhận xét:} Với bộ pin tham chiếu 18650, thời lượng pin dự phòng
trong chế độ theo dõi liên tục khi xe chạy được quy đổi vào khoảng
2.6--3.0 giờ, tùy hiệu suất chuyển đổi thực tế. Trong kịch bản xe đỗ và
chỉ gửi heartbeat định kỳ, thiết bị vẫn có thể duy trì hoạt động trong
nhiều ngày; tuy nhiên, các giá trị này cần được kiểm thử lại trên mẫu
pin 18650 thực tế nếu sử dụng cho giai đoạn chế tạo tiếp theo.
...
~15 mét (đo bước đầu, cần xác nhận lại) & ~10 mét (đo bước đầu, cần
xác nhận lại) \\\hline
4 & Bãi đỗ xe có mái che & ~8.5 mét (ước tính ban đầu) &
~20 mét (đo bước đầu, cần xác nhận lại) & ~15 mét (đo bước đầu, cần
xác nhận lại) \\\hline
\end{longtable}
}
...
Khác với phần kết nối lại được đánh giá bằng các phép đo định lượng,
luồng OTA ở giai đoạn này được đánh giá chủ yếu bằng đối chiếu mã nguồn
triển khai và kiểm thử tích hợp theo kịch bản giữa API máy chủ, firmware
và MQTT Bridge. Mục tiêu là xác nhận toàn bộ đường đi của lệnh cập nhật,
trạng thái phản hồi và cơ chế lưu vết đã được nối thông.


{\thesissettableid{4.25A}{tab-ch4-oi-chieu-luong-ota-theo-ma-nguon-trien-khai}
\def\LTcaptype{table}
\begin{longtable}{|L{0.224\linewidth}|L{0.580\linewidth}|L{0.160\linewidth}|}
\caption{Đối chiếu mức độ hoàn thiện luồng OTA theo mã nguồn và kiểm thử tích hợp}\label{tab:ch4-oi-chieu-luong-ota-theo-ma-nguon-trien-khai}\\
...
\textbf{Nhận xét:} Luồng OTA cơ bản đã khép kín từ API máy chủ đến thiết bị
...
\paragraph{4.3.4.1. Hiệu suất API máy
...
\caption{Kết quả đo hiệu suất API máy chủ}\label{tab:ch4-ket-qua-o-hieu-suat-api-backend}\\
...
\\\hline
\endfirsthead
\caption[]{Kết quả đo hiệu suất API máy chủ (tiếp theo)}\\
\hline \textbf{STT} & \textbf{Endpoint} & \textbf{Phương thức} & \textbf{Thời gian trung bình} & \textbf{P50} & \textbf{P95} & \textbf{P99} & \textbf{Số yêu cầu/giây} \\
\\\hline
\endhead
\\\hline
...
~35--120 ms cho các thao tác CRUD, đáp ứng mục tiêu thiết kế
(\textless{} 200 ms cho P95). Endpoint POST /api/auth/login có thời gian
cao hơn (~120 ms) do phải băm và xác thực mật khẩu.
Tổng thể, API máy chủ xây dựng bằng Express.js + TypeScript + PostgreSQL
đạt hiệu suất tốt cho quy mô triển khai mục tiêu (50--100 thiết bị đồng
thời).

\paragraph{4.3.4.2. Độ trễ WebSocket
(Socket.IO)}\label{4342-ux111ux1ed9-trux1ec5-websocket-socketio}

Độ trễ toàn tuyến của dữ liệu thời gian thực được đo từ lúc thiết bị
phát bản tin MQTT đến khi giao diện web nhận được sự kiện cập nhật qua
WebSocket (Socket.IO).
...
~15 ms & Mạng Docker nội bộ \\\hline
3 & Bridge -\textgreater{} VictoriaMetrics (ghi dữ liệu) & ~10
ms & ~30 ms & HTTP write API \\\hline
4 & Bridge -\textgreater{} API máy chủ (sự kiện nội bộ) & ~5
ms & ~12 ms & HTTP nội bộ / sự kiện nội bộ \\\hline
5 & API máy chủ -\textgreater{} Giao diện web (WebSocket) & ~15 ms
& ~40 ms & Phát sự kiện bằng Socket.IO \\\hline
6 & \textbf{Tổng toàn tuyến} & \textbf{~185 ms} &
\textbf{~400 ms} & \textbf{Từ thiết bị đến giao diện quản trị} \\\hline
\end{longtable}
}
...
\caption{Đồ thị phân bố độ trễ toàn tuyến (1000 bản tin mẫu)}
\label{fig:ch4-o-thi-phan-bo-o-tre-end-to-end-1000-ban-tin}
\end{figure}
}


\textbf{Nhận xét:} Độ trễ toàn tuyến trung bình ~185 ms
...
nội bộ (EMQX -\textgreater{} Bridge -\textgreater{} API máy chủ
-\textgreater{} giao diện web) có độ trễ rất thấp (~35 ms
tổng), nhờ sử dụng mạng Docker nội bộ và cơ chế phát sự kiện của
Socket.IO ngay khi có dữ liệu mới.
...
~3.5 GB & 0.1\% (ước tính tại tải biên) & ~350
ms (ước tính tại tải biên) \\\hline
\end{longtable}
}
...
\endfoot
\endlastfoot
1 & Điểm Lighthouse tổng hợp & 87/100 & \textgreater{} 85 & Đạt \\\hline
2 & First Contentful Paint (FCP), tức thời gian thấy nội dung đầu tiên & ~1.2 giây & \textless{} 1.5 giây & Đạt \\\hline
3 & Largest Contentful Paint (LCP), tức thời gian hiện khối nội dung lớn nhất & ~2.1 giây & \textless{} 2.5 giây & Đạt \\\hline
4 & Total Blocking Time (TBT), tức thời gian trang bị chặn phản hồi & ~120 ms & \textless{} 200 ms & Đạt \\\hline
5 & CLS, tức mức xê dịch bố cục khi trang đang tải & ~0.05 & \textless{} 0.1 & Đạt \\\hline
6 & Time to Interactive (TTI), tức thời điểm trang phản hồi tốt với thao tác & ~2.5 giây & \textless{} 3.5 giây & Đạt \\\hline
7 & Kích thước gói tải xuống đã nén & ~380 KB & \textless{} 500 KB & Đạt \\\hline
\end{longtable}
}
...
1.2 giây cho thấy trang hiển thị nội dung đầu tiên khá nhanh. Các chỉ số
Core Web Vitals đều đạt ngưỡng tốt. Các giá trị trong bảng được đo trên
bản dựng thử nghiệm ở chế độ desktop. Việc sử dụng Next.js 15 với React
Server Components cùng kỹ thuật tách nhỏ mã nguồn khi tải giúp tối ưu
kích thước gói tải xuống và thời gian mở trang.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{4.3.5. Kiểm thử tích hợp toàn hệ thống (kiểm thử toàn
 tuyến)}\label{435-kiux1ec3m-thux1eed-tuxedch-hux1ee3p-touxe0n-hux1ec7-thux1ed1ng-end-to-end-integration-test}

Mục này trình bày kết quả kiểm thử luồng dữ liệu toàn tuyến, từ thiết bị
...
API máy chủ phát sự kiện WebSocket -\textgreater{} giao diện web cập nhật bản đồ
và biểu đồ.
...
~0.02 giây \\\hline
8 & Đẩy WebSocket & API máy chủ phát sự kiện đến giao diện web & Thành công &
~0.02 giây \\\hline
9 & Cập nhật UI & Bản đồ và biểu đồ cập nhật & Thành công &
~0.05 giây \\\hline
& \textbf{Tổng thời gian toàn tuyến} & \textbf{Từ dữ liệu cảm biến đến
...
\caption{Minh họa giao diện trang tổng quan hiển thị vị trí xe đang di chuyển trên bản đồ Leaflet}
...
giây sau khi API máy chủ tạo cảnh báo. Tổng thời gian từ xe vượt ranh giới đến
...
\caption{Minh họa luồng gửi lệnh điều khiển từ trang tổng quan và phản hồi từ thiết bị}
...
~2.6--3.0 giờ (quy đổi thiết kế theo bộ pin tham chiếu 18650) & Đạt theo quy
...
khả năng xử lý đồng thời, theo dõi vùng cho phép, cảnh báo bất thường và
khả năng tự kết nối lại MQTT. Những điểm cần tiếp tục tối ưu chủ yếu nằm
ở dòng tiêu thụ khi phát 4G liên tục, ngưỡng hiệu chuẩn LVD profile 12V
và cơ chế phát lại dữ liệu vận hành từ bộ nhớ cục bộ microSD nếu mục
tiêu là tăng mức độ bảo toàn dữ liệu trong các khoảng mất sóng kéo dài.
...
\endfoot
\endlastfoot
1 & Tỷ lệ sẵn sàng của API máy chủ (7 ngày) & 99.6\% & Đạt, phù hợp môi trường
...
sleep toàn hệ thống xấp xỉ 500 µA; nếu quy đổi theo cùng giả định tiêu
thụ sang bộ pin dự phòng tham chiếu 18650 1S (3500mAh), thời lượng dự
phòng tương ứng vào khoảng 24--36 giờ ở kịch bản cảnh báo gián đoạn và
khoảng 2.6--3.0 giờ ở chế độ theo dõi liên tục khi xe chạy. Kết quả quy
đổi này cho thấy thiết bị đã đạt điều kiện tối thiểu để theo dõi liên tục
mà không gây áp lực quá lớn lên nguồn điện của xe.
...
(mục tiêu với bộ pin tham chiếu 18650) & ~24--36 giờ (quy đổi thiết kế theo
...
\textbf{Dịch vụ API máy chủ (Tracking\_Backend):}
...
qua Socket.IO giữa API máy chủ và giao diện web giúp vị trí xe được cập nhật gần
...
lý ở API máy chủ và thời gian giao diện vẽ lại dữ liệu; phần còn lại là độ
...
\endfoot
\endlastfoot
API máy chủ & Thời gian phản hồi trung bình & 20--80 ms (đơn giản),
100--300 ms (phức tạp) & Tốt \\\hline
API máy chủ & Số yêu cầu xử lý cùng lúc & 100--500 yêu cầu/giây & Đạt yêu cầu \\\hline
...
\hline \textbf{Giải pháp} & \textbf{Chi phí thiết bị} & \textbf{Phí dịch vụ hàng tháng} & \textbf{Tổng chi phí năm
đầu (ước tính)} & \textbf{Khả năng tùy biến} \\
\\\hline
\endhead
\\\hline
\endfoot
...
LVD ngắt nguồn từ ắc quy. Với bộ pin tham chiếu 18650, thời lượng dự
phòng vẫn đủ cho cảnh báo gián đoạn ngắn hạn; tuy nhiên, các giá trị
quy đổi cho vận hành liên tục cần được hiểu là quy đổi thiết kế và nên
được đo lại nếu triển khai phần cứng theo đúng cell mới. Ngưỡng LVD
profile 12V vì vậy vẫn cần được hiệu chuẩn thêm để bám sát giá trị thiết
kế.
...
\item
  ...
  container hóa được dùng để đóng gói từng dịch vụ như API máy chủ,
...
\item
  ...
  truy vấn xác thực.
\item
  \emph{Triển khai tách từng dịch vụ}: Mỗi dịch vụ như API máy chủ,
...
phương tiện có thể tạo ra những giá trị xã hội rõ ràng, chứ không chỉ
đơn thuần là một công cụ kiểm soát. Một số tác động tích cực có thể nhận
thấy gồm:
...
\item
  ...
  khi mất kết nối hoặc thiết bị hỏng, không để việc mất kết
...
Frontend & TypeScript, giao diện web tổng quan & Phải giữ giao diện dễ dùng, cập
...
\subsection{1.2. Chi phí hạ tầng đám mây (ước tính hàng
...
\caption{Chi phí hạ tầng đám mây}\label{tab:app1-chi-phi-ha-tang-cloud}\\
\hline \textbf{STT} & \textbf{Hạng mục} & \textbf{Thông số} & \textbf{Chi phí hàng tháng (VND)} & \textbf{Ghi chú} \\
\\\hline
\endfirsthead
\caption[]{Chi phí hạ tầng đám mây (tiếp theo)}\\
\hline \textbf{STT} & \textbf{Hạng mục} & \textbf{Thông số} & \textbf{Chi phí hàng tháng (VND)} & \textbf{Ghi chú} \\
\\\hline
\endhead
\\\hline
...
Hạ tầng đám mây (3 tháng phát triển + 3 tháng kiểm thử) & 2.370.000--3.570.000 & 6
...
\textbf{GĐ3} & Xây dựng hạ tầng đám mây & Tuần 5--8 (4 tuần) & Docker
...
\textbf{Ghi chú:} Các giai đoạn có sự chồng chéo có chủ đích
để tối ưu hóa thời gian. Ví dụ, GĐ3 (hạ tầng đám mây) bắt đầu trước khi
...
\caption{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 1}\label{tab:ch3-tong-hop-thong-tin-ve-tuan-cong-viec-san-pham}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endfirsthead
\caption[]{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 1 (tiếp theo)}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endhead
\\\hline
...
\caption{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 2}\label{tab:ch3-tong-hop-thong-tin-ve-tuan-cong-viec-san-pham-ca9bd3}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endfirsthead
\caption[]{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 2 (tiếp theo)}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endhead
\\\hline
...
\textbf{Giai đoạn 3 --- Hạ tầng đám mây (Tuần 5--8):}
...
\caption{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 3}\label{tab:ch3-tong-hop-thong-tin-ve-tuan-cong-viec-san-pham-b7165b}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endfirsthead
\caption[]{Kế hoạch công việc và sản phẩm đầu ra của giai đoạn 3 (tiếp theo)}\\
\hline \textbf{Tuần} & \textbf{Công việc} & \textbf{Sản phẩm} \\
\\\hline
\endhead
\\\hline
...
\textbf{Giai đoạn 4 --- API máy chủ (Tuần 7--14):}
...
\endfoot
\endlastfoot
17--18 & Tích hợp firmware - hạ tầng máy chủ & Hệ thống toàn tuyến \\\hline
...
\subsection{4.2. Tham chiếu cơ sở dữ liệu (Lược đồ cơ
sở dữ liệu)}\label{42-tham-chiux1ebfu-cux1a1-sux1edf-dux1eef-liux1ec7u-database-schema}

Lược đồ PostgreSQL đầy đủ được lưu tại các tệp SQL khởi tạo trong dịch vụ
