...
Switch\_OFF=12.0V, Switch\_ON=12.2V; 24V dùng
Switch\_OFF=24.0V, Switch\_ON=24.4V, qua đó bảo vệ ắc quy không bị rút
cạn quá mức {[}7{]}.
...
Hệ thống tracker sử dụng ESP32-S3 làm bộ xử lý trung tâm. Có thể hình
dung đây là "bộ não" của thiết bị, chịu trách nhiệm thu thập dữ liệu,
điều phối các khối phần cứng và quyết định trạng thái vận hành. Vi điều
khiển này giao tiếp UART với modem 4G/GNSS, BLE với OBD2, I2C với cảm
biến gia tốc và ADC để giám sát điện áp ắc quy. Toàn bộ hệ thống được
cấp nguồn bởi mạch quản lý năng lượng thông minh, có khả năng tự động
chuyển đổi giữa ắc quy xe và pin dự phòng.
...
Ở mức chức năng, khối modem đảm nhiệm hai việc: đưa dữ liệu của xe lên
mạng 4G và trả về vị trí GNSS của xe. ESP32-S3 sử dụng UART1 (GPIO16 TX,
GPIO17 RX) để giao tiếp với SIMCom SIM7600CE-T thông qua tập lệnh AT.
Modem được cấu hình Auto mode (\texttt{AT+CNMP=2}) để tự động chuyển
giữa LTE/UMTS/GSM, sử dụng APN mặc định \texttt{internet}, và cung cấp
dữ liệu GNSS qua \texttt{AT+CGNSINF} (có thể thêm stream NMEA
\texttt{AT+CGNSTST} nếu cần) trên cùng UART. Tốc độ baud mặc định 115200
bps, dữ liệu 8-bit, 1 stop bit, không parity. Kiến trúc này giảm số khối
phần cứng rời và giữ pin mapping hiện tại.
...
tracker không chỉ biết xe đang ở đâu mà còn biết xe đang ở trạng thái
...
LIS3DSH được kết nối qua bus I2C (GPIO47 SDA, GPIO48 SCL). Vai trò chính
của cảm biến này là giúp hệ thống "ngủ mà vẫn canh", tức là phát hiện
chuyển động khi xe đang đỗ và đánh thức ESP32-S3 từ chế độ deep sleep
thông qua ngắt ngoài (interrupt) khi có rung động bất thường.
...
\begin{figure}[htbp]
\centering
\pandocbounded{\safeincludesvgnarrow{./assets/figures/07-chuong-4-trien-khai-hardware-hinh-4-10.svg}}
...
Thiết bị tracker có thể được lắp ở nhiều vị trí khác nhau trong xe, tuy
...
Hệ thống Cloud là nơi biến dữ liệu từ thiết bị thành khả năng quan sát
...
từng khối: hạ tầng Docker, MQTT Bridge, Backend API, Frontend Dashboard
và lớp giám sát vận hành.

Nếu nhìn theo hành trình dữ liệu, triển khai thực tế diễn ra như sau:
thiết bị IoT gửi MQTT vào EMQX, MQTT Bridge phân luồng dữ liệu sang
VictoriaMetrics, VictoriaLogs và PostgreSQL, còn Backend/API cùng
Frontend Dashboard đảm nhiệm truy vấn, điều khiển và hiển thị thời gian
thực. Cách mô tả này giúp người đọc nối được kiến trúc ở Chương 3 với
các cấu hình triển khai cụ thể ở phần sau.
...
Ở giai đoạn triển khai, nhóm chọn mô hình per-service Docker Compose
theo quy ước IVM26. Đây là cách tổ chức phù hợp với đồ án vì vẫn đủ đơn
giản để cài đặt, nhưng đã phản ánh khá rõ tư duy chia dịch vụ trong một
hệ thống thật. Theo đó, mỗi dịch vụ có một file
\texttt{docker-compose.yml} riêng thay vì gom toàn bộ vào một file lớn.
Cách tổ chức này có một số lợi điểm rõ ràng:
...
được đưa vào mạng dùng chung \texttt{tracking-network}; nhờ vậy, dù nằm
...
NPM & tracking-npm & 80, 81, 443 & Reverse proxy \\\hline
\end{longtable}
}

\textbf{Mapping truy cập cloud UAT:}

{\thesissettableid{4.5-cloud}{tab-ch4-mapping-domain-va-port-cong-khai-cloud-uat}
\def\LTcaptype{table}
\begin{longtable}{|p{0.21\linewidth}|p{0.12\linewidth}|p{0.20\linewidth}|p{0.13\linewidth}|p{0.24\linewidth}|}
\caption{Mapping subdomain, port public và service nội bộ của hệ thống cloud UAT}\label{tab:ch4-mapping-domain-va-port-cong-khai-cloud-uat}\\
\hline \textbf{Subdomain public} & \textbf{Port public} & \textbf{Service nội bộ} & \textbf{Upstream port} & \textbf{Vai trò} \\
\hline
\endfirsthead
\caption[]{Mapping subdomain, port public và service nội bộ của hệ thống cloud UAT (tiếp theo)}\\
\hline \textbf{Subdomain public} & \textbf{Port public} & \textbf{Service nội bộ} & \textbf{Upstream port} & \textbf{Vai trò} \\
\hline
\endhead
\nolinkurl{thingdock.dev} & 443 & tracking-frontend & 4001 & Frontend web qua NPM reverse proxy \\\hline
\nolinkurl{api.thingdock.dev} & 443 & tracking-backend & 4000 & Backend API, Swagger, Socket.IO qua NPM reverse proxy \\\hline
\nolinkurl{mqtt.thingdock.dev} & 8883 & tracking-emqx & 8883 & MQTT TLS trực tiếp cho thiết bị qua NPM stream TCP \\\hline
\nolinkurl{grafana.thingdock.dev} & 443 & tracking-grafana & 4002 & Grafana qua NPM reverse proxy \\\hline
\nolinkurl{emqx.thingdock.dev} & 443 & tracking-emqx & 18083 & EMQX Dashboard qua NPM reverse proxy \\\hline
\nolinkurl{npm.thingdock.dev} & 443 & tracking-npm & 81 & Nginx Proxy Manager Admin qua NPM reverse proxy \\\hline
\nolinkurl{(internal only)} & 8428 & tracking-victoriametrics & 8428 & VictoriaMetrics, chỉ dùng nội bộ \\\hline
\end{longtable}
}

\textbf{Ghi chú:} Trong triển khai UAT hiện tại, Cloudflare chỉ giữ vai
trò DNS trỏ domain về public IP của VPS; sau đó Nginx Proxy Manager mới
làm reverse proxy đến các container nội bộ. Vì vậy, người dùng chỉ truy
cập \texttt{thingdock.dev}, \texttt{api.thingdock.dev},
\texttt{grafana.thingdock.dev}, \texttt{emqx.thingdock.dev} và
\texttt{npm.thingdock.dev} qua cổng HTTPS mặc định. Các port
\texttt{4001}, \texttt{4000}, \texttt{4002}, \texttt{18083} và
\texttt{81} là port nội bộ phía sau NPM, không xuất hiện trong URL
public. Riêng MQTT TLS cho thiết bị đi trực tiếp tới EMQX qua
\texttt{mqtt.thingdock.dev}.
VictoriaMetrics (\texttt{8428}) được giữ ở chế độ nội bộ, không public
qua subdomain.
...
MQTT Bridge (\texttt{Tracking\_MqttBridge/}) là nơi hệ thống ``dịch''
dữ liệu thô từ broker thành các bản ghi có cấu trúc phù hợp với từng lớp
lưu trữ và lớp ứng dụng phía sau. Việc tách dịch vụ này khỏi Backend API
giúp đường ingest telemetry gọn hơn, dễ scale riêng và tránh trộn lẫn
với lớp nghiệp vụ web.
...
Sau khi dữ liệu đã được tiếp nhận và xử lý ở tầng cloud, bước tiếp theo
là biến chúng thành giao diện đủ rõ ràng để người vận hành có thể quan
sát và ra quyết định. Vì vậy, phần frontend không chỉ được đánh giá ở
khía cạnh công nghệ, mà còn ở khả năng tổ chức thông tin theo đúng nhu
cầu sử dụng thực tế.
...
Components. Việc lựa chọn stack này bám theo mục tiêu đã nêu từ Chương
3: giao diện cần tải nhanh, cập nhật thời gian thực ổn định và vẫn đủ
dễ tổ chức khi số lượng tính năng quản trị tăng dần.
...
phần UI, hooks, types, và utilities liên quan. Cách tiếp cận này khiến
cấu trúc mã nguồn phản chiếu khá sát cấu trúc nghiệp vụ, nên người đọc
không cần đi quá sâu vào code vẫn có thể hình dung hệ thống web đang
...
Để dashboard thực sự hữu ích trong vận hành, giao diện không được dừng ở
...
Với dashboard thời gian thực, nếu không tách bạch rõ loại dữ liệu nào là
trạng thái giao diện, loại nào là dữ liệu từ máy chủ và loại nào là cập
nhật sự kiện, mã nguồn sẽ rất dễ rối. Vì vậy, frontend sử dụng chiến
lược quản lý trạng thái kết hợp:
...
Khi hệ thống đã có đủ các lớp phần cứng, firmware, cloud và frontend,
...
logs (nhật ký) và traces (truy vết). Cách tổ chức này giúp người vận
hành không chỉ biết ``có lỗi hay không'', mà còn lần ra lỗi xuất phát từ
đâu, ảnh hưởng tới dịch vụ nào và đang diễn ra ở quy mô nào.
...
\begin{figure}[htbp]
\centering
\pandocbounded{\safeincludesvgchart{./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-23.svg}}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\safeincludesvgchart{./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-24.svg}}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\safeincludesvgnarrow{./assets/figures/10-chuong-4-ket-qua-do-luong-hinh-4-37.svg}}
...
thực tế. Thiết bị tracker đã được chế tạo thành một bộ phần cứng hoàn
chỉnh; firmware ESP-IDF/FreeRTOS hiện thực đầy đủ các mô-đun giao tiếp,
máy trạng thái năng lượng và cơ chế tự phục hồi kết nối; hạ tầng cloud
cùng backend/frontend cũng vận hành đồng bộ theo kiến trúc đề xuất.
...
machine đều đạt hoặc vượt mục tiêu tổng hợp. Dù vậy, kết quả cũng chỉ ra
những điểm còn cần làm sâu thêm ở vòng tối ưu tiếp theo, đáng chú ý là
ngưỡng LVD profile 12V, mức dòng ở pha truyền 4G liên tục và cơ chế
replay telemetry từ flash cho các tình huống mất sóng kéo dài. Đây sẽ là
cơ sở để Chương 5 chuyển từ phần "đã làm được gì" sang phần "đánh giá
chất lượng của những gì đã làm".
...
trong trạng thái chờ. Mạch buck converter đầu vào 12V/24V (dải 7--40V)
hạ áp xuống 5V, sau đó qua LDO 3.3V cấp cho ESP32-S3, đạt hiệu suất tổng
trên 90\% ở điều kiện thử nghiệm profile 12V. Logic chuyển nguồn dự
phòng được chuẩn hóa theo hai profile: 12V
(\texttt{Switch\_OFF=12.0V}, \texttt{Switch\_ON=12.2V},
\texttt{IGN\_ON\textgreater{}=13.0V},
\texttt{IGN\_OFF\textless{}=12.0V}) và 24V
(\texttt{Switch\_OFF=24.0V}, \texttt{Switch\_ON=24.4V},
\texttt{IGN\_ON\textgreater{}=26.0V},
...
2.6--3.0 giờ ở chế độ tracking liên tục. Nói cách khác, thiết bị đã đạt
được điều kiện tối thiểu để theo dõi liên tục mà không gây áp lực quá
lớn lên nguồn điện của xe.
...
nhận được cho ứng dụng tracking xe (tham chiếu
...
điều phối thực thi, độ ổn định của máy trạng thái và khả năng tự phục
hồi kết nối kèm OTA.
...
Cách tổ chức này cho thấy một lựa chọn thiết kế khá thực dụng: ưu tiên
giữ luồng điều khiển dễ hiểu và dễ debug, thay vì theo đuổi một cấu trúc
đa task phức tạp nhưng khó kiểm chứng trong giai đoạn đồ án.
...
Nhìn tổng thể, tầng cloud được đánh giá không chỉ qua hiệu năng của từng
dịch vụ riêng lẻ mà qua khả năng phối hợp giữa chúng để giữ cho dữ liệu
đi từ thiết bị đến dashboard đủ nhanh và ổn định. Trên cơ sở đó, từng
thành phần được xem xét như sau:
...
người dùng mượt mà. Ở góc nhìn vận hành, kết quả này có ý nghĩa vì nó
cho thấy dashboard không trở thành ``điểm nghẽn cuối cùng'' sau khi dữ
liệu đã được thu thập và xử lý ở các lớp phía dưới. Dashboard hiển thị
thông tin tổng quan đội xe với thời gian tải trang dưới 2 giây (First
Contentful Paint). Bản đồ Leaflet cập nhật vị trí xe trơn tru khi nhận
dữ liệu WebSocket, không bị giật lag khi hiển thị 50--100 xe đồng thời.
Biểu đồ ECharts hiển thị dữ liệu telemetry (tốc độ, RPM, nhiệt độ) trực
quan, hỗ trợ zoom và pan mượt mà.
...
Chi phí Bill of Materials (BOM) của thiết bị tracker IoT dưới đây dùng
...
Khối nguồn (TP4056, boost/buck, LDO, BMS, LVD) & Quản lý năng lượng &
93.000 \\\hline
...
trong vùng chi phí chấp nhận được so với nhóm GPS tracker phổ thông
...
PostgreSQL đã bao gồm các bảng dữ liệu cho các tính năng Phase 2. Điều
...
Khi hệ thống đã có khả năng thu thập telemetry liên tục và khá ổn định,
giá trị tiếp theo không chỉ nằm ở việc ``nhìn thấy dữ liệu'', mà ở việc
biến dữ liệu đó thành dự báo và khuyến nghị. Vì vậy, AI/ML là một hướng
...
vào tối ưu DFM, EMI, độ gọn cơ khí và độ tin cậy:
...
tăng chiều sâu hardening: watchdog, bảo mật MQTT/TLS và kiểm thử vận
...
kiến thức mạng quyết định dữ liệu đó có đi đúng nơi và đến đủ nhanh hay
không. Vì vậy, nhóm kiến thức này xuất hiện xuyên suốt từ thiết bị, cloud
đến giao diện thời gian thực:
...
mà để tổ chức dữ liệu theo đúng đặc tính sử dụng của hệ thống. Đây là
một điểm học được rất rõ khi đồ án phải đồng thời xử lý dữ liệu telemetry
và dữ liệu nghiệp vụ:
...
Đường ống dữ liệu thời gian thực là nơi nhiều ràng buộc gặp nhau cùng
lúc: tốc độ, độ tin cậy, chi phí lưu trữ và khả năng mở rộng. Hệ thống
cần xử lý luồng dữ liệu telemetry liên tục từ nhiều thiết bị IoT (GPS,
trạng thái nguồn, IMU; sẵn sàng mở rộng thêm OBD2) với các yêu cầu:
...
Bài toán cloud của đồ án không dừng ở việc ``đưa dữ liệu lên server'',
...
Bên cạnh các lợi ích trên, hệ thống vẫn đi kèm những lo ngại cần được
xác định sớm để tránh biến công cụ hỗ trợ vận hành thành công cụ gây áp
lực hoặc xâm phạm quyền riêng tư.
...
lẻ, mà ở chỗ mọi lớp đều có ràng buộc riêng và chỉ bộc lộ rõ khi ghép
lại với nhau. Hệ thống này bao gồm bốn lớp công nghệ khác nhau:
...
2 & LTE + GNSS & \nolinkurl{SIM7600CE-T} & 1 bộ & 650.000 & 650.000 & Modem
...
6 & IC sạc pin & TP4056 + mạch phụ trợ & 1 & 37.000 & 37.000 & Sạc 1S
...
\hline
\endhead
Phần cứng (1 bộ tracker) & 1.514.000 & Theo BOM ở bảng PL-1.1 \\\hline
...
chi phí phần cứng cho mỗi bộ tracker khoảng 1.514.000 VND, thấp hơn đáng
...
\tightlist
\item
  \textbf{Domain (cloud/UAT):} \texttt{api.thingdock.dev} (đường dẫn
  Swagger: \texttt{/api-docs})
\item
...
\hline
\endhead
Frontend Web & \nolinkurl{thingdock.dev} & admin / Admin@2026 \\\hline
Backend API & \nolinkurl{api.thingdock.dev} & --- \\\hline
Swagger Docs & \nolinkurl{api.thingdock.dev (/api-docs)} & --- \\\hline
MQTT TLS Broker & \nolinkurl{mqtt.thingdock.dev} & Theo tài khoản MQTT của thiết bị \\\hline
Grafana & \nolinkurl{grafana.thingdock.dev} & admin / Anmh1205 \\\hline
EMQX Dashboard & \nolinkurl{emqx.thingdock.dev} & admin / Anmh1205 \\\hline
NPM Admin & \nolinkurl{npm.thingdock.dev} & Tài khoản admin cấu hình khi bootstrap \\\hline
\end{longtable}
}

\textbf{Ghi chú cloud UAT:} VictoriaMetrics được giữ ở chế độ nội bộ,
không công khai qua subdomain. Nếu cần truy cập phục vụ vận hành, dịch vụ
này vẫn nằm tại \texttt{tracking-victoriametrics:8428} trong Docker
network hoặc có thể xuất bản thêm subdomain riêng ở giai đoạn sau.
