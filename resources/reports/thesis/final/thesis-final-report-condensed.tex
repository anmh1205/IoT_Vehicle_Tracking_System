...
\item
  \textbf{Giai đoạn 4 --- Phát triển API máy chủ}: Chuyển dữ liệu kỹ thuật
...
\item
  \textbf{Tầng máy chủ và dữ liệu}: broker bản tin, dịch vụ API, lưu trữ dữ
...
\hline
\endlastfoot
Số rail nguồn chính & 1 rail dùng chung cho logic và modem & 2 rail chính và dự phòng cơ bản & Bus 5 V, rail 3,3 V logic, rail modem, rail dự phòng \\\hline
Cách nuôi modem SIM7600CE-T & Dùng chung với logic, dễ sụt áp khi phát xung dòng & Có cải thiện nhưng chưa tách hẳn & Có rail riêng khoảng 4 V cho modem \\\hline
...
Ảnh hưởng của xung dòng modem lên rail logic & Xung dòng đi chung rail, dễ kéo tụt nguồn MCU & Giảm bớt nhưng vẫn còn chia sẻ một phần đường cấp & Được cô lập nhờ rail modem riêng \\\hline
Khả năng thêm các nhánh 3,3 V, 4 V, sạc và dự phòng & Khó, vì mọi tải bám một rail & Thêm được nhưng ranh giới chưa rõ & Thuận lợi, mỗi miền nguồn có nhiệm vụ riêng \\\hline
...
Rail logic 3,3 V & AP2112-3.3 & ESP32-S3, LIS3DH, RTC và mức logic & Giữ nguồn sạch cho vi điều khiển và cảm biến \\\hline
Rail riêng cho modem & TPS54231 & SIM7600CE-T & Tách tải động lớn của modem khỏi miền logic \\\hline
...
\hline
\endlastfoot
Loại linh kiện & Bộ hạ áp chuyển mạch TPS54231 & Tạo rail riêng cho modem \\\hline
...
Nguồn & MP2482, AP2112-3.3, TPS54231, TP5100, SX1308, 18650 1S, mạch chuyển nguồn, ADC & Tách nhánh nguồn logic, rail modem và rail dự phòng; sạc pin bằng TP5100 {[}30{]} & Ổn định nhánh nguồn, bảo vệ ắc quy bằng chiến lược phần mềm và duy trì khi mất nguồn ngắn \\\hline
\end{longtable}
}
...
Tổ chức tác vụ cho modem, BLE, IMU và ADC & Chủ yếu theo vòng lặp chính tuần tự, khó tách tải khi ngoại vi tăng & Có FreeRTOS, hàng đợi, semaphore và nhóm sự kiện {[}10{]} & Có RTOS đầy đủ \\\hline
...
Khả năng giữ cấu trúc mã khi ghép đồng thời modem, BLE và IMU & Khởi đầu nhanh nhưng dễ phình logic trong một luồng chính & Giữ cấu trúc rõ giữa trình điều khiển, tác vụ nền và lõi trạng thái & Cần thêm thời gian chuẩn hóa thư viện và gỡ lỗi \\\hline
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"data"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"latitude"}\FunctionTok{:} \FloatTok{10.823371}\FunctionTok{,}
    \DataTypeTok{"longitude"}\FunctionTok{:} \FloatTok{106.740750}\FunctionTok{,}
    \DataTypeTok{"speed"}\FunctionTok{:} \FloatTok{67.13}\FunctionTok{,}
    \DataTypeTok{"course"}\FunctionTok{:} \FloatTok{320.22}\FunctionTok{,}
    ...
    \DataTypeTok{"vibration"}\FunctionTok{:} \FloatTok{0.63}\FunctionTok{,}
  ...
  \DataTypeTok{"diagnostics"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"signals"}\FunctionTok{:} \FunctionTok{\{}
      ...
      \DataTypeTok{"obd\_speed\_kph"}\FunctionTok{:} \DecValTok{67}\FunctionTok{,}
    ...
    \DataTypeTok{"reported\_dtc\_count"}\FunctionTok{:} \DecValTok{1}\FunctionTok{,}
    \DataTypeTok{"dtc"}\FunctionTok{:} \FunctionTok{\{}
      \DataTypeTok{"stored"}\FunctionTok{:} \OtherTok{[}\StringTok{"P0500"}\OtherTok{]}\FunctionTok{,}
      \DataTypeTok{"pending"}\FunctionTok{:} \OtherTok{[]}\FunctionTok{,}
      \DataTypeTok{"permanent"}\FunctionTok{:} \OtherTok{[}\StringTok{"P0171"}\OtherTok{]}
    \FunctionTok{\}}
  \FunctionTok{\},}
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"timestamp"}\FunctionTok{:} \DecValTok{1710000005123}\FunctionTok{,}
  \DataTypeTok{"state"}\FunctionTok{:} \FunctionTok{\{}
    \DataTypeTok{"ignition\_state"}\FunctionTok{:} \StringTok{"ON"}\FunctionTok{,}
    \DataTypeTok{"motion\_state"}\FunctionTok{:} \StringTok{"MOVING"}\FunctionTok{,}
    \DataTypeTok{"vehicle\_state"}\FunctionTok{:} \StringTok{"MOVING\_ON"}\FunctionTok{,}
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"timestamp"}\FunctionTok{:} \DecValTok{1710000009000}
\FunctionTok{\}}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{\{}
  ...
  \DataTypeTok{"targetVersion"}\FunctionTok{:} \StringTok{"v2.3.1"}\FunctionTok{,}
  \DataTypeTok{"currentVersion"}\FunctionTok{:} \StringTok{"v2.3.0"}\FunctionTok{,}
...
{\thesissetfigureid{4.6}{condensed-hinh-4-6-minh-hoa-luoc-do-co-so-du-lieu-postgresql-dang-trien-khai}
\begin{figure}[H]
\centering
\pandocbounded{\safeincludesvg{./assets/figures/05-chuong-3-giai-phap-backend-hinh-3-14.svg}}
\caption{Minh họa lược đồ cơ sở dữ liệu PostgreSQL đang triển khai}
\label{fig:condensed-hinh-4-6-minh-hoa-luoc-do-co-so-du-lieu-postgresql-dang-trien-khai}
\end{figure}
}
