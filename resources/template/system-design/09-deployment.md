...
services:
  ...
  postgres:
    ...
    environment:
      ...
      POSTGRES_DB: ${POSTGRES_DB:-iot_database}
    volumes:
  ...
  victoriametrics:
    ...
    container_name: iot-victoriametrics
    command:
      - "-retentionPeriod=30d"
  ...
  victorialogs:
    ...
    container_name: iot-victorialogs
    command:
      - "-retentionPeriod=7d"
      - "-storageDataPath=/storage"
      - "-httpListenAddr=:9428"
  ...
  backend:
    ...
    environment:
      ...
      POSTGRESQL_DATABASE: ${POSTGRES_DB:-iot_database}
  ...
  mqtt-bridge:
    build:
      context: ./backend
      dockerfile: Dockerfile
    container_name: iot-mqtt-bridge
...
```bash
# Backup database
docker exec iot-postgres pg_dump -U postgres iot_database > backup.sql

# Restore database
docker exec -i iot-postgres psql -U postgres iot_database < backup.sql
    ...
    steps:
      ...
      - uses: docker/build-push-action@v5
        with:
          context: ./backend
          push: true
          tags: registry/iot-backend:${{ github.sha }}
