...
- [ ] Env vars: respects hierarchy `process.env` > `$CODEX_HOME/skills/${SKILL}/.env` (global) > `$CODEX_HOME/skills/.env` (global) > `$CODEX_HOME/.env` (global) > `./.codex/skills/${SKILL}/.env` (cwd) > `./.codex/skills/.env` (cwd) > `./.codex/.env` (cwd)
