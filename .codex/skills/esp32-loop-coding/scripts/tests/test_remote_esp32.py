...
def test_scp_args_disable_host_key_checks_when_requested():
    args = remote_esp32._scp_args("100.96.198.126", "anmh1", "keyfile", 15, False)
    assert args[0] == "scp"
    assert "StrictHostKeyChecking=no" in args
    assert "UserKnownHostsFile=/dev/null" in args
...
def test_wait_for_port_requires_stable_reprobe(monkeypatch):
    args = make_args(ready_settle_ms=1, ready_probes=2)
    monkeypatch.setattr(
        remote_esp32,
        "_detect_remote_ports",
        lambda *_a, **_k: [{"DeviceID": "COM13", "Name": "USB Serial Device", "PNPDeviceID": "USB\\VID_303A&PID_1001"}],
    )
    probe_results = iter(
        [
            (True, ""),
            (False, "device not stable yet"),
            (True, ""),
            (True, ""),
        ]
    )
    monkeypatch.setattr(remote_esp32, "_probe_remote_port", lambda *_a, **_k: next(probe_results))
    monkeypatch.setattr(remote_esp32.time, "sleep", lambda *_a, **_k: None)
    port, attempts, _elapsed, _ports, last_error = remote_esp32._wait_for_port(args, 5, 115200)
    assert port == "COM13"
    assert attempts == 2
    assert last_error == "device not stable yet"


def test_build_esptool_args_supports_native_usb_overrides(tmp_path):
    ...
    args = make_args(before="no_reset_no_sync", after="no_reset", no_stub=True, flasher_args=str(flasher_args))
    argv = remote_esp32._build_esptool_args(args, "COM13")
    assert "--before" in argv and "no_reset_no_sync" in argv
    assert "--after" in argv and "no_reset" in argv
    assert "--no-stub" in argv
    assert "hard_reset" not in argv


def test_remote_scp_path_converts_windows_path():
    remote_path = remote_esp32._remote_scp_path(remote_esp32.PureWindowsPath(r"C:\Users\anmh1\esp32-remote-flash\bootloader.bin"))
    assert remote_path == "/C:/Users/anmh1/esp32-remote-flash/bootloader.bin"


def test_stage_flash_artifacts_copies_local_build_outputs(monkeypatch, tmp_path):
    bootloader = tmp_path / "bootloader"
    partition = tmp_path / "partition_table"
    bootloader.mkdir()
    partition.mkdir()
    (bootloader / "bootloader.bin").write_bytes(b"boot")
    (partition / "partition-table.bin").write_bytes(b"part")
    (tmp_path / "Tracking_Firmware.bin").write_bytes(b"app")
    (tmp_path / "ota_data_initial.bin").write_bytes(b"ota")
    flasher_args = tmp_path / "flasher_args.json"
    flasher_args.write_text(
        json.dumps(
            {
    ...
    args = make_args(flasher_args=str(flasher_args), stage_dir=r"C:\stage")
    copied: list[tuple[str, str]] = []
    monkeypatch.setattr(remote_esp32, "_run_remote", lambda *_a, **_k: subprocess.CompletedProcess([], 0, "", ""))
    monkeypatch.setattr(
        remote_esp32,
        "_run_scp",
        lambda local_path, remote_path, _args: copied.append((Path(local_path).name, str(remote_path)))
        or subprocess.CompletedProcess([], 0, "", ""),
    )
    remote_esp32._stage_flash_artifacts(args)
    assert ("flasher_args.json", r"C:\stage\flasher_args.json") in copied
    assert ("bootloader.bin", r"C:\stage\bootloader\bootloader.bin") in copied
    assert ("partition-table.bin", r"C:\stage\partition_table\partition-table.bin") in copied
    assert ("Tracking_Firmware.bin", r"C:\stage\Tracking_Firmware.bin") in copied


def test_cmd_flash_stage_only_json(monkeypatch, capsys):
    args = make_args(json=True)
    args.stage_only = True
    monkeypatch.setattr(remote_esp32, "_stage_flash_artifacts", lambda *_a, **_k: None)
    exit_code = remote_esp32.cmd_flash(args)
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert exit_code == 0
    assert payload["status"] == "stage-ok"


def test_run_remote_wait_flash_parses_json_payload(monkeypatch):
    args = make_args()
    monkeypatch.setattr(remote_esp32, "_stage_text_file", lambda *_a, **_k: None)
    monkeypatch.setattr(
        remote_esp32,
        "_run_remote_python",
    ...
    result = remote_esp32._run_remote_wait_flash(args)
    assert result.status == "flash-ok"
    assert result.port == "COM13"
...
def test_cmd_flash_json_suppresses_raw_flash_output(monkeypatch, capsys):
    args = make_args(json=True)
    args.stage_artifacts = False
    monkeypatch.setattr(
        remote_esp32,
        "_run_remote_wait_flash",
