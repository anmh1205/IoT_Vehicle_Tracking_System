...
def find_api_key() -> Optional[str]:
    ...
    Uses $CODEX_HOME/scripts/resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. PROJECT/.codex/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/.codex/skills/.env (shared skills)
    4. PROJECT/.codex/.env (project global)
    5. $CODEX_HOME/skills/ai-multimodal/.env (user skill-specific)
    6. $CODEX_HOME/skills/.env (user shared)
    7. $CODEX_HOME/.env (user global)
    """
...
def batch_process(
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python $CODEX_HOME/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
        print("3. Quick setup: export GEMINI_API_KEY='your-key'")
        print("4. Create .env: cd $CODEX_HOME/skills/ai-multimodal && cp .env.example .env")
