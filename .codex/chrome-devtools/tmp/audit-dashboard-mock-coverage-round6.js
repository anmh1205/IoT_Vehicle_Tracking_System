...
const OUTPUT_DIR = path.resolve('.codex/chrome-devtools/screenshots/mock-audit-round6');
const REPORT_PATH = path.resolve('.codex/chrome-devtools/screenshots/mock-audit-round6-report.json');
    ...
    ({ labels: targetLabels, selector: targetSelector }) => {
      ...
      for (const label of targetLabels) {
        ...
        const hit = nodes.find((node) =>
          normalize(node.textContent ?? '').includes(normalizedLabel),
        );
        if (hit) {
