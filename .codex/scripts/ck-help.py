...
CATEGORY_GUIDES = {
    ...
    "config": {
        ...
        "workflow": [
            ("Global", "Set user prefs in `$CODEX_HOME/.ck.json`"),
    ...
    "notifications": {
        ...
        "workflow": [
            ("1. Set env vars", "Add `DISCORD_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID` to `$CODEX_HOME/.env`"),
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .codex/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_config_guide() -> None:
    ...
    print("- Global: `$CODEX_HOME/.ck.json` (user preferences)")
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`$CODEX_HOME/.ck.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```bash")
    print("# $CODEX_HOME/.ck.json - applies everywhere")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `$CODEX_HOME/.ck.json` - personal preference")
...
def main():
    # Find .codex/commands directory
    script_path = Path(__file__).resolve()
    claude_dir = script_path.parent.parent  # .codex/scripts -> .codex
    commands_dir = claude_dir / "commands"

    if not commands_dir.exists():
        print("Error: .codex/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .codex/commands/")
        sys.exit(1)
