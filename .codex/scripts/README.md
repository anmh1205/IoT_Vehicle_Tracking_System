...
2. **PROJECT/.codex/skills/\<skill\>/.env** - Project skill-specific
3. **PROJECT/.codex/skills/.env** - Project shared across skills
4. **PROJECT/.codex/.env** - Project global defaults
5. **$CODEX_HOME/skills/\<skill\>/.env** - User skill-specific
6. **$CODEX_HOME/skills/.env** - User shared across skills
7. **$CODEX_HOME/.env** - User global defaults (LOWEST)

### CLI Usage

```bash
# Resolve a variable for a specific skill
python $CODEX_HOME/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python $CODEX_HOME/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python $CODEX_HOME/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python $CODEX_HOME/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python $CODEX_HOME/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
if not api_key:
    ...
    print("Run: python $CODEX_HOME/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python $CODEX_HOME/scripts/resolve_env.py TEST_VAR --verbose
...
python $CODEX_HOME/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --find-all
```
...
python .codex/scripts/generate_catalogs.py --skills

# Generate commands catalog (outputs to stdout)
python .codex/scripts/generate_catalogs.py --commands

# Generate both catalogs (outputs to stdout)
python .codex/scripts/generate_catalogs.py

# Write to file instead of stdout
python .codex/scripts/generate_catalogs.py --skills --output guide/SKILLS.yaml

# View help
python .codex/scripts/generate_catalogs.py --help
```
