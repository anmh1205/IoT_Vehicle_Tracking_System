 ...
 *
 * Extracted from dev-rules-reminder.cjs for reuse in both Claude hooks and Codex plugins.
...
function resolveRulesPath(filename, configDirName = '.codex') {
  ...
  if (fs.existsSync(globalRulesPath)) return `$CODEX_HOME/rules/${filename}`;
  ...
  if (fs.existsSync(globalWorkflowsPath)) return `$CODEX_HOME/workflows/${filename}`;
...
function resolveScriptPath(filename, configDirName = '.codex') {
  ...
  if (fs.existsSync(globalPath)) return `$CODEX_HOME/scripts/${filename}`;
  return null;
}
...
function resolveSkillsVenv(configDirName = '.codex') {
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ? '~\\.codex\\skills\\.venv\\Scripts\\python.exe'
      : '$CODEX_HOME/skills/.venv/bin/python3';
  }
  return null;
}
