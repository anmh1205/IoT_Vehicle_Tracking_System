...
git diff --cached --name-only | awk -F'/' '{
  ...
  else if ($0 ~ /\.codex\/(skills|agents|commands|rules)/) print "config:"$0
...
**Special cases:**
- `.codex/` skill updates: `perf(skill): improve git-manager token efficiency`
...
- **Git history persists** across Codex sessions
...
```
Files: .codex/commands/new.md, src/feature.ts, package.json
Split into:
