...
**IMPORTANT**: Analyze skills at `.codex/skills/*` and activate as needed.
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .codex/.mcp.json .gemini/settings.json
...
```bash
npx tsx .codex/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
