...
1. Scan the codebase and calculate the number of files with LOC in each directory (skip credentials, cache or external modules directories, such as `.codex`, `.codex`, `.git`, `tests`, `node_modules`, `__pycache__`, `secrets`, etc.)
