...
/* Detected LIS3DH I2C address (0x18 or 0x19). */
...
esp_err_t imu_init(void) {
    ...
    ESP_GOTO_ON_ERROR(i2c_new_master_bus(&bus_cfg, &s_bus_handle), fail, TAG, "i2c_new_master_bus failed");

    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = LIS3DH_ADDR_PRIMARY,
        .scl_speed_hz = LIS3DH_I2C_FREQ_HZ,
    };
    ...
    if (err != ESP_OK || who_am_i != 0x33) {
        i2c_master_bus_rm_device(s_dev_handle);
        s_dev_handle = NULL;

        dev_cfg.device_address = 0x19;
        ESP_GOTO_ON_ERROR(i2c_master_bus_add_device(s_bus_handle, &dev_cfg, &s_dev_handle),
                          fail,
                          TAG,
                          "fallback I2C address add failed");

        ESP_GOTO_ON_ERROR(imu_read_reg(LIS3DH_WHO_AM_I_REG, &who_am_i), fail, TAG, "WHO_AM_I read failed");
        ESP_GOTO_ON_FALSE(who_am_i == 0x33, fail, TAG, "Unexpected WHO_AM_I: 0x%02X", who_am_i);
        s_lis3dh_addr = 0x19;
    }

    /* 10Hz normal mode with XYZ enabled. */
    ESP_GOTO_ON_ERROR(imu_write_reg(LIS3DH_CTRL_REG1, 0x27), fail, TAG, "CTRL_REG1 write failed");
    /* High-resolution mode, +/-2g full scale. */
    ...
    ESP_LOGI(TAG, "LIS3DH initialized at I2C addr 0x%02X", s_lis3dh_addr);
    return ESP_OK;
...
esp_err_t imu_configure_motion_interrupt(uint8_t threshold_mg, uint8_t duration_ms) {
    ...
    ESP_RETURN_ON_FALSE(imu_write_reg(LIS3DH_CTRL_REG5, 0x08) == ESP_OK, ESP_FAIL, TAG, "CTRL_REG5 write failed");
