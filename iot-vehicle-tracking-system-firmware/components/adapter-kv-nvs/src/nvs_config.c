...
esp_err_t nvs_config_save_session_context(const session_persist_context_t *context) {
    // Persist config save session context here so later boots, retries, or recovery paths can resume cleanly.
    if (context == NULL) {
...
esp_err_t nvs_config_load_session_context(session_persist_context_t *out_context, bool *out_found) {
    // Rehydrate config load session context here so later logic reads one coherent snapshot after reset or sleep.
...
esp_err_t nvs_config_clear_session_context(void) {
    // Reset config clear session context here so stale data does not leak into the next cycle.
    nvs_handle_t handle = 0;
    esp_err_t err = nvs_open(TRACKER_NVS_NAMESPACE, NVS_READWRITE, &handle);
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        return ESP_OK;
