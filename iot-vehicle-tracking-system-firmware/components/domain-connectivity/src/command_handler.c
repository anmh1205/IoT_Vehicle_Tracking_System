...
/*
 * Recent accepted cloud command IDs. The MQTT connection is persistent, so
 * QoS1 DUP delivery may cross reconnect/reboot boundaries. Keep the bounded
 * RAM ring mirrored in NVS before allowing the FSM to consume side effects.
 */
...
static esp_err_t command_handler_persist_recent_command_ids_locked(void) {
    ...
    s_recent_command_persist_dirty = err != ESP_OK;
    if (err != ESP_OK) {
        ESP_LOGW(TAG,
                 "event=command_dedupe_persist_failed err=%s",
                 esp_err_to_name(err));
    }
    return err;
}
...
static void command_handler_remember_command_id(uint64_t command_id) {
    ...
    s_recent_command_persist_dirty = true;
    (void)command_handler_persist_recent_command_ids_locked();
...
esp_err_t command_handler_init(config_t *config) {
    ...
    ESP_RETURN_ON_FALSE(dedupe_err == ESP_OK,
                        dedupe_err,
                        TAG,
                        "command dedupe context load failed");
...
esp_err_t command_handler_process(const char *command_json,
    ...
    bool duplicate_command = command_handler_is_recent_command_id_locked(parsed_command_id);
    command_handler_give_lock();
...
command_action_t command_handler_consume_action(uint64_t *out_command_id) {
    ...
    if (s_recent_command_persist_dirty &&
        command_handler_persist_recent_command_ids_locked() != ESP_OK) {
        command_handler_give_lock();
        return COMMAND_ACTION_NONE;
    }
