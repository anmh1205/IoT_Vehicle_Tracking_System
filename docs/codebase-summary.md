...
- **Tracking_Frontend**: Next.js 16.1 app (React 19.2.3) rooted in the App Router. It now has a public marketing entry at `src/app/page.tsx` backed by `src/features/marketing/*` plus the existing protected dashboard under `src/app/dashboard/*`. The dashboards continue to lean on shared UI atoms in `src/components/common` (for example `stat-card.tsx`, `data-table.tsx`, `empty-state.tsx`) while domain-specific pages under `src/features/*` (notifications, statistics, devices, fuel analytics, maintenance, exports, geofences, trips, system-status) reuse the same layout, spacing, and helpers such as `src/lib/utils/date/format.ts` to normalize numbers and dates.
