...
import { getVehiclePositionSnapshot } from './vehicle-allowed-zone.repository';

describe('vehicle-allowed-zone.repository', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(findOne).mockResolvedValue(null);
  });

  it('uses latest device coordinates when building a legacy allowed-zone preview', async () => {
