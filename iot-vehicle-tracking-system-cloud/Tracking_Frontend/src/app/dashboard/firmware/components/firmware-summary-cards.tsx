    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          subtitle={
            latestFirmware
              ? `Mới nhất ${getFirmwareDisplayVersion(latestFirmware.version)}`
          ...
          subtitle={
            activeFirmware
              ...
              : hasCompletedDelivery
                ? 'Kho firmware chưa gắn cờ stable'
                : 'Chưa có bản stable'
          }
          isLoading={isLoading}
        />
        <StatCard
          ...
          trend={{
            value: `${averageProgress}% tiến độ trung bình`,
            positive: averageProgress >= 50,
          }}
          isLoading={isLoading}
        />
        <StatCard
          ...
          subtitle={
            latestDeployment
              ? `Cập nhật ${formatDateTime(latestDeployment.updatedAt)}`
              : 'Chưa có lịch sử'
          }
          trend={{
            value: `${deployments.length ? deployments.length - failedDeployments : 0} đợt ổn định`,
            positive: failedDeployments === 0,
          }}
          isLoading={isLoading}
        />
      </div>
      ...
      <Card className="border-dashed bg-gradient-to-r from-background via-muted/30 to-background">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Tổng dung lượng lưu trữ: {formatBytes(totalSize)} · Cờ stable phản ánh kho firmware, còn
            OTA phản ánh lịch sử triển khai trên thiết bị.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-lg border bg-background p-3">
            <p className="text-xs text-muted-foreground">Bản stable hiện tại</p>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {activeFirmware
                ...
                : hasCompletedDelivery
                  ? 'Thiết bị có OTA nhưng kho chưa gắn cờ stable'
                  : 'Chưa có bản stable'}
            </p>
          </div>
          <div className="rounded-lg border bg-background p-3">
          ...
          <div className="rounded-lg border bg-background p-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {failedDeployments > 0
                ? `${failedDeployments} thiết bị/bản ghi lỗi`
                : 'Không ghi nhận lỗi trong đợt gần nhất'}
            </p>
          </div>
        </CardContent>
      </Card>
