...
export const getDeploymentBadgeVariant = (status: string | null | undefined) =>
  status === 'failed' || status === 'stuck_timeout' ? ('destructive' as const) : ('outline' as const);
...
export const sortDeploymentsByRecentActivity = (left: FirmwareDeployment, right: FirmwareDeployment) => {
  const rightTime =
