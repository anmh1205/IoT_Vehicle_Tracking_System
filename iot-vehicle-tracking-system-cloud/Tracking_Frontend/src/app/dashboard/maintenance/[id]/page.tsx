...
const MaintenanceDetailPage = ({
  ...
  const typeLabel = TYPE_LABELS[detail.maintenanceType] ?? detail.maintenanceType ?? 'Khác';
  const pageTitle = detail.title ?? `Bảo trì #${id}`;
  ...
  return (
    <PageContainer pageTitle={pageTitle} pageDescription="Theo dõi tiến trình bảo trì và chi phí khai thác">
        ...
        <CardContent className="space-y-4 p-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-2xl font-semibold tracking-tight">
                  {detail.title ?? 'Công việc bảo trì chưa có tiêu đề'}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {detail.vehicleId ? `Phương tiện: ${detail.vehicleId}` : 'Chưa gắn phương tiện'}
                </p>
              </div>
            </div>
          ...
          {missingSignals.length > 0 ? (
            <div className="rounded-2xl border border-dashed px-4 py-3 text-sm text-muted-foreground">
              {missingSignals.join(' • ')}. Phiếu bảo trì hiện thiếu ngữ cảnh để theo dõi end-to-end.
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            <InfoRow label="Phương tiện" value={detail.vehicleId ?? 'Chưa có'} />
            ...
            <InfoRow label="Mô tả" value={detail.description ?? 'Chưa có mô tả'} />
        ...
        <Card>
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            ...
            <InfoRow
              label="Ngày nhắc bảo trì tiếp theo"
