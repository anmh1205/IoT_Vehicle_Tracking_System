...
import {
  ...
  type DeviceWorkspaceActions,
  type DeviceWorkspaceFallbackPaths,
  type DeviceWorkspaceSection,
...
export const DeviceWorkspaceShell = ({
  ...
  return (
    <DialogContent
      ...
      className="!inset-0 !left-0 !top-0 !z-[var(--layer-dialog)] !flex !h-[100dvh] !max-h-[100dvh] !w-screen !max-w-none !translate-x-0 !translate-y-0 flex-col overflow-hidden rounded-none border-0 bg-background p-0"
    >
      <DialogTitle className="sr-only">{device?.deviceName ?? 'Không gian làm việc thiết bị'}</DialogTitle>
      <DialogDescription className="sr-only">
        Không gian làm việc vận hành thiết bị gồm xe, thiết bị, cảnh báo, vùng và dữ liệu vận hành trong cùng một hộp thoại toàn màn hình.
      </DialogDescription>
      ...
      <div className="flex min-h-0 flex-1 flex-col bg-[radial-gradient(circle_at_top_right,rgba(56,189,248,0.14),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(59,130,246,0.12),transparent_32%)]">
        <header className="shrink-0 border-b border-border/70 bg-background/88 px-4 py-4 backdrop-blur-xl sm:px-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <p className="truncate text-xl font-semibold">
                  {device?.vehiclePlate ?? device?.deviceName ?? 'Không gian làm việc thiết bị'}
                </p>
                <Badge variant="outline">Xe: {vehicle.value}</Badge>
                <Badge variant="outline">Thiết bị: {runtime.value}</Badge>
                <Badge variant={DEVICE_STATUS_VARIANTS[device?.currentStatus ?? ''] ?? 'outline'}>
                  {connectivity.label}: {DEVICE_STATUS_LABELS[device?.currentStatus ?? ''] ?? connectivity.value}
                </Badge>
                {device?.lastSeenAt ? (
                  <Badge variant="secondary">Cập nhật {formatRelative(device.lastSeenAt)}</Badge>
                ) : null}
              </div>
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          <main className="flex min-h-0 min-w-0 flex-1 flex-col">
            <div className="shrink-0 border-b border-border/60 bg-background/72 px-4 py-3 backdrop-blur md:hidden">
            ...
            <div
              className={cn(
                'min-h-0 min-w-0 flex-1 px-4 py-4 sm:px-6',
                activeSection === 'route' || activeSection === 'raw'
                  ? 'overflow-hidden'
                  : 'overflow-y-auto',
              )}
            >
              {loading ? (
                ...
                <ErrorBox description={error.message} />
              ) : (
                <DeviceWorkspaceCanvas
