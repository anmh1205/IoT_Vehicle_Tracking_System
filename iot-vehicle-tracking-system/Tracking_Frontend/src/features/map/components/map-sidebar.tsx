...
export const MapSidebar = () => {
  ...
  const rows = useMemo(
    () =>
      Array.from(positions.values()).filter((item) =>
      ...
      <Tabs defaultValue="devices" className="h-full">
        <div className="p-3">
          <TabsList className="w-full">
            ...
            <TabsTrigger value="geofences" className="flex-1">
              Vùng giám sát
            </TabsTrigger>
          </TabsList>
        </div>
            ...
            {rows.map((item) => (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">
                  {STATUS_LABELS[item.status] ?? item.status} - {item.speed} km/h
                </div>
              </button>
            ))}
          </div>
          ...
          {selected && (
            <Card className="mt-3">
              ...
              <CardContent className="space-y-1 text-xs">
                ...
                <div>
                  Vĩ độ/Kinh độ: {selected.lat}, {selected.lon}
                </div>
              </CardContent>
            </Card>
          )}
        </TabsContent>
        <TabsContent value="geofences" className="m-0 p-3 text-sm text-muted-foreground">
          Danh sách vùng giám sát được quản lý tại trang Vùng giám sát.
        </TabsContent>
      </Tabs>
    </div>
  );
